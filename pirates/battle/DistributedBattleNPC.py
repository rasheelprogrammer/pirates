...
class DistributedBattleNPC(DistributedBattleAvatar):
    ...
    def setupActorAnims(self):
        if self.animSet != 'default':
            self.animSetSetup = self.animSetSetup == False and True
    ...
    def updateCollisions(self):
        ...
        if self.cAggro == None:
            if self.isBattleable():
                ...
                if self.cNotice == None:
                    self.cNotice = noticeSphereSize > 0 and CollisionSphere(0, 0, 0, noticeSphereSize)
                    ...
                    base.noticeSystemOn and self.cNoticeNode.setIntoCollideMask(PiratesGlobals.WallBitmask)
                else:
                ...
                enterCollEvent = base.config.GetBool('npcs-auto-target', 1) and self.uniqueName('enter' + 'NoticeSphere')
                self.accept(enterCollEvent, self._handleEnterNoticeSphere)
        return

    def getEffectiveNoticeDistance(self):
        if base.cr.gameStatManager.aggroModelIndex == 1:
            return self.getTeam() not in [0, 4] and self.getInstantAggroSphereSize() + self.noticeDistance
        else:
    ...
    def performQuickFloorCheck(self, customReach=None):
        ...
        if localAvatar.ship:
            if self.ship is not localAvatar.ship:
            ...
            rayReach = customReach != None and customReach
        else:
    ...
    def noticeLocalAvatar(self):
        ...
        if base.localAvatar.getGameState() == 'NPCInteract':
            if not (hasattr(self, 'dialogProcessMaster') and self.dialogProcessMaster):
            ...
            if self.getDistance(localAvatar) > self.getEffectiveNoticeDistance() + 2.0:
                self.hasTurnedToNotice and self.endNotice()
            else:
                self.noticeIval = Sequence(Wait(self.noticeSpeed), Func(self.noticeLocalAvatar))
                self.noticeIval.start()
        else:
            if abs(noticeDif) > 15.0:
                if self.shouldTurnToNotice:
                    self.noticeIval = turnAnim and Sequence(Func(self.startShuffle, turnAnim), LerpHprInterval(self, duration=self.noticeSpeed, hpr=newHpr), Func(self.midShuffle), Wait(self.noticeSpeed), Func(self.endShuffle), Func(self.noticeLocalAvatar))
                    self.noticeIval.start()
                else:
    ...
    def smoothPosition(self):
        ...
        if cantMove == False:
            if (self.getPos(parentObj) - oldPos).length() < 0.1:
                ...
                if base.config.GetBool('want-npc-notice', 1):
                    if abs(self.getH() - oldH) < 0.1:
                        if self.moveNoticeFlag:
                            if self.lastMovedTimeStamp:
                                if globalClock.getFrameTime() - self.lastMovedTimeStamp > 0.5:
                                    self.isMovingDontNotice = not self.localAvatarHasBeenNoticed and 0
                                    self.firstNoticeLocalAvatar()
                                    self.noticeLocalAvatar()
                                    self.moveNoticeFlag = 0
                            else:
                        ...
                        if self.collisionMode & PiratesGlobals.COLL_MODE_FLOORS_CL:
                            currZ = self.getZ()
                            abs(oldZ - currZ) <= 8 and self.setZ(parentObj, oldZ)
                        else:
                            customReach = None
                            if currZ > oldZ:
    ...
    def holdAnimProp(self, availProps):
        ...
        if prop:
            if not prop.isEmpty():
                handNode = (self.getName() == 'Captain Barbossa' and self).leftHandNode
            else:

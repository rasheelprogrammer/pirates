...
class DistributedBattleAvatar(DistributedReputationAvatar, WeaponBase, Teamable):
    ...
    def setIsGhost(self, isGhost, override=0):
        ...
        if self.isGhost == isGhost:
            return not self.ghostDelay and None
        else:
        ...
        if self.ghostGeomGenerated == 0:
            ...
            if state in ['Battle']:
                ...
                if self.isLocal():
                    ...
                    if self.isLocal():
                        redraw and localAvatar.toggleWeapon(localAvatar.currentWeaponId, localAvatar.currentWeaponSlotId)
            self.deleteDropShadow()
            if self.currentWeapon:
            ...
            self.ghostColorMult = 1.0
            ghostColor = self.ghostBaseColor * self.ghostColorMult
            effectNumber == 1 and geom.setTransparency(1)
            geom.setDepthWrite(0)
            geom.setAttrib(ColorBlendAttrib.make(ColorBlendAttrib.MAdd, ColorBlendAttrib.OIncomingAlpha, ColorBlendAttrib.OOne))
            geom.setColorScale(ghostColor)
            geom.setRenderModeFilled()
            ...
            if effectNumber == 2:
                ...
                geom.setRenderModeFilled()
                self.ghostBaseNegative and geom.setAttrib(ColorBlendAttrib.make(ColorBlendAttrib.MInvSubtract, ColorBlendAttrib.OIncomingAlpha, ColorBlendAttrib.OOne))
        ...
        if self.addGhostEffect(ghostColor):
            ...
            if self.ghostEyeGlowL:
                if self.ghostEyeGlowR:
                    self.style.gender == 'f' and self.ghostEyeGlowL.setPos(0.175, -0.13, -0.25)
    ...
    def addGhostEffect(self, ghostColor):
        ...
        if self.ghostBaseNegative:
            ...
            self.ghostShadowEffect.effectColor = self.ghostBaseNegative and 0 and Vec4(1.0 - ghostColor[0], 1.0 - ghostColor[1], 1.0 - ghostColor[2], 0.5)
        else:
    ...
    def stopGhost(self, effectNumber, isDev=False):
        ...
        if hasattr(self, 'creature'):
            ...
            if effectNumber == 1:
                ...
                self.depthGeom and self.depthGeom.removeNode()
        else:
            if effectNumber == 2:
                npCollection = geom.findAllMatches('**/eyes')
                for np in npCollection:
    ...
    def checkWeaponSwitch(self, currentWeaponId, isWeaponDrawn):
        ...
        if isWeaponDrawn == self.isWeaponDrawn:
            if currentWeaponId == self.currentWeaponId:
                return
            self.isWeaponDrawn = isWeaponDrawn and not self.isWeaponDrawn and isWeaponDrawn
            ...
            if isWeaponDrawn:
                self.isWeaponDrawn and currentWeaponId != self.currentWeaponId and self.__initWeaponChange()
                ...
                if not isWeaponDrawn:
                    self.isWeaponDrawn = self.isWeaponDrawn and isWeaponDrawn
                    self.currentWeaponId = currentWeaponId
                    self.__initWeaponChange()
                    self.__doPutAwayWeapon()
                    self.__endWeaponChange()
        ...
        if weaponClass:
            ...
            if ItemGlobals.getSubtype(self.currentWeaponId) == ItemGlobals.MUSKET:
                ...
                if bayonetPart:
                    bayonetPart.stash()
            if not self.isLocal():
                if ItemGlobals.getType(self.currentWeaponId) == ItemGlobals.FISHING:
                ...
                if self.isLocal():
                    ...
                    rep = self.currentWeaponId and WeaponGlobals.getRepId(self.currentWeaponId)
    ...
    def pullOutCurrentWeapon(self, ammoSkillId=0, blendInT=0.1, blendOutT=0):
        ...
        if hasattr(self, 'undead'):
            drawIval = self.undead and self.currentWeapon.getDrawIval(self.skeleton, ammoSkillId, blendInT, blendOutT)
        else:
    ...
    def putAwayCurrentWeapon(self, blendInT=0.1, blendOutT=0.1):
        if hasattr(self, 'undead'):
            returnIval = self.undead and self.currentWeapon.getReturnIval(self.skeleton, blendInT, blendOutT)
        else:
    ...
    def changeAmmunition(self):
        if hasattr(self, 'undead'):
            returnIval = self.undead and self.currentWeapon.getAmmoChangeIval(self.skeleton, 0, self.currentAmmo, 0, None)
        else:
    ...
    def playOuch(self, skillId, ammoSkillId, targetEffects, attacker, pos, itemEffects=[], multihit=0, targetBonus=0, skillResult=0):
        ...
        if self.PlayersInvulnerable and targetHp < 0 and not self.isNpc:
            ...
            if targetMojo < 0:
                not attacker == self and taskMgr.doMethodLater(WeaponGlobals.MP_DAMAGE_DELAY, self.takeMpDamage, self.taskName('playMpDamage'), extraArgs=[-targetMojo, pos])
            else:
                if targetMojo > 0:
    ...
    def playSkillMovie(self, skillId, ammoSkillId, skillResult, charge, targetId=0, areaIdList=[]):
        ...
        if self.curAttackAnim:
            if self.curAttackAnim.isPlaying():
                if WeaponGlobals.getIsInstantSkill(skillId, ammoSkillId):
                    return
                else:
                    self.curAttackAnim.pause()
                    self.curAttackAnim = None
            ...
            if not self.isLocal():
                if WeaponGlobals.getSkillTrack(skillId) == WeaponGlobals.DEFENSE_SKILL_INDEX:
                    ...
                    skillId == EnemySkills.EnemySkills.MISC_VOODOO_REFLECT and self.showEffectString(PLocalizer.AttackReflected)
                else:
    ...
    def getSpawnTrack(self):
        ...
        if avatarTeam in [PiratesGlobals.UNDEAD_TEAM, PiratesGlobals.FRENCH_UNDEAD_TEAM, PiratesGlobals.SPANISH_UNDEAD_TEAM]:
            if not self.avatarType.isA(AvatarTypes.Ghost):
                ...
                spawnIval = self.isInInvasion() and Sequence(Func(startSFX), introIval)
            else:
    ...
    def getDeathTrack(self):
        ...
        if hasattr(self, 'creature'):
            ...
            delay = isinstance(self, Biped) and {'death': 0.8, 'death2': 1.2, 'death3': 1.8, 'death4': 1.6}.get(animName, 1.0)
        else:
            delay = 0.0
    ...
    def bobTask(self, task):
        ...
        if world:
            ...
            water = None
        if self.cr.wantSeapatch:
            zWater, normal = water and water.calcHeightAndNormal(node=self)
        else:
            zWater = 0.0
            normal = Vec3(0, 0, 1)
        if self.bobbing:
                        ...
                        if effectId == WeaponGlobals.C_WOUND:
                            ...
                            if effectId == WeaponGlobals.C_TAUNT:
                                ...
                                if effectId == WeaponGlobals.C_ON_FIRE:
                                    ...
                                    if effectId == WeaponGlobals.C_ON_CURSED_FIRE:
                                        self._addOnFireEffect(attackerId, timeLeft)
                                        if self.fireEffect:
                                                ...
                                                if effectId == WeaponGlobals.C_HOLD:
                                                    ...
                                                    if effectId == WeaponGlobals.C_ATTUNE:
                                                        ...
                                                        if effectId == WeaponGlobals.C_VOODOO_STUN:
                                                            ...
                                                            if effectId == WeaponGlobals.C_VOODOO_HEX_STUN:
    ...
    def removeStatusEffect(self, effectId, attackerId):
        ...
        if effectId == WeaponGlobals.C_REGEN:
            ...
            if effectId == WeaponGlobals.C_FURY:
                ...
                if effectId in [WeaponGlobals.C_MELEE_SHIELD, WeaponGlobals.C_MISSILE_SHIELD, WeaponGlobals.C_MAGIC_SHIELD]:
                    self._removeGhostGuardEffect()
                else:
                    if base.config.GetBool('want-potion-game', 0):
                        if not (PotionGlobals.getIsPotionBuff(effectId) and self).potionStatusEffectManager:
    ...
    def _addOnFireEffect(self, attackerId, duration):
        ...
        if self.fireEffect:
            if hasattr(self, 'headNode'):
                self.headNode and self.fireEffect.reparentTo(self.headNode)
    ...
    def _addSlowEffect(self, attackerId, duration):
        ...
        if self.slowEffect:
            ...
            if self.slowEffect2:
                ...
                hasattr(self, 'headNode') and self.headNode and self.slowEffect2.reparentTo(self.headNode)
    ...
    def _addStunEffect(self, attackerId, duration):
        ...
        if self.stunEffect:
            ...
            if self.stunEffect2:
                ...
                hasattr(self, 'headNode') and self.headNode and self.stunEffect2.reparentTo(self.headNode)
    ...
    def _addMonkeyPanicEffect(self, attackerId, duration):
        ...
        if base.options.getSpecialEffectsSetting() >= base.options.SpecialEffectsHigh:
            ...
            if self.monkeyPanic:
                if hasattr(self, 'headNode'):
                    self.headNode and self.monkeyPanic.reparentTo(self.headNode)
    ...
    def _addFuryEffect(self, attackerId, duration):
        ...
        if base.options.getSpecialEffectsSetting() >= base.options.SpecialEffectsHigh:
            ...
            if self.pulseEffect:
                if hasattr(self, 'headNode'):
                    self.headNode and self.pulseEffect.reparentTo(self.headNode)
    ...
    def checkAttuneBuffEffect(self):
        ...
        if self.voodooAttuneEffect:
            ...
            for buffKeyId in attuneBuffs:
                ...
                attacker = self.cr.doId2do.get(attackerId)
                if not (attacker and TeamUtils.damageAllowed(attacker, self)):
                    ...
                    elif buffColorType != 'localHostile':
                        buffColorType = buffColorType != 'localFriendly' and 'hostile'
    ...
    def showHpText(self, number, pos=0, bonus=0, duration=2.0, scale=0.5, basicPenalty=0, crewBonus=0, doubleXPBonus=0, holidayBonus=0, potionBonus=0, itemEffects=[]):
        if self.HpTextEnabled:
            if not self.ghostMode:
                if base.showGui:
                    freebooter = not Freebooter.getPaidStatus(base.localAvatar.getDoId())
                    if pos != 0:
                        n = self.hasNetPythonTag('MonstrousObject') and NodePath('empty')
                        n.reparentTo(self)
                        n.setPos(tuple(pos))
                ...
                newEffect and self.textEffects.append(newEffect)
        return
        ...
        if self.HpTextEnabled:
            if not self.ghostMode:
                if base.showGui:
                    ...
                    hpText.setFogOff()
                    hpText.setLightOff()
                    pos and hpTextDummy.setPos(self, pos[0], pos[1], pos[2])
                else:
    ...
    def showEffectString(self, text, pos=0, duration=2.0, scale=0.5):
        if self.HpTextEnabled:
            if not self.ghostMode:
                if base.showGui:
                    text != '' and self.HpTextGenerator.setFont(PiratesGlobals.getPirateOutlineFont())
                    self.HpTextGenerator.setText(text)
                    self.HpTextGenerator.clearShadow()
                    self.HpTextGenerator.setAlign(TextNode.ACenter)
                    r = 0.99
        ...
        if questId:
            ...
            if quest and quest.getTasks() and not quest.isTimedOut():
                for currTaskState, currTask in zip(quest.getTaskStates(), quest.getTasks()):
                    if currTask.getGoalUid(currTaskState).compareTo(self) == 0:
                        self.isTracked = not currTaskState.isComplete() and True
    ...
    def hide(self, drawMask=None, invisibleBits=None):
        ...
        if drawMask:
            ...
            if invisibleBits:
                not invisibleBits.isZero() and DistributedReputationAvatar.hide(self)
        else:
    ...
    def requestEmote(self, emoteId):
        ...
        if emote:
            ...
            if emote_gender:
                if self.style.gender != emote_gender:
                    return False
            else:
                return False
            ...
            return self.isWeaponDrawn and True
        else:

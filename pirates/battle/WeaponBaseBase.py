...
class WeaponBaseBase:
    ...
    def getAreaList(self, skillId, ammoSkillId, target, pos, attackerId, isBoss=False):
        ...
        if numEntries == 0:
            return targets
        if isBoss:
            numEntries = (numEntries > WeaponGlobals.BOSS_AREA_TARGETS and WeaponGlobals).BOSS_AREA_TARGETS
        else:
            ...
            if potentialTarget:
                ...
                if not WeaponGlobals.isFriendlyFire(skillId, ammoSkillId):
                    if not TeamUtils.damageAllowed(potentialTarget, self.avatar):
                        if attackerId:
                            if not (potentialTargetId == attackerId and WeaponGlobals.isAttackAreaSelfDamaging(skillId, ammoSkillId)):
                                continue
                        else:
                            continue
                else:

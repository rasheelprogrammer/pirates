...
# Embedded file name: pirates.battle.PotionStatusEffectManager
import random
...
class PotionStatusEffectManager:
    ...
    def addStatusEffect(self, effectId, attackerId, duration, timeLeft, timestamp, buffData):
        ...
        if effectId == WeaponGlobals.C_VOMIT:
            ...
            if effectId == WeaponGlobals.C_FART:
                self._addFartEffect(attackerId, timeLeft)
            else:
                if effectId == WeaponGlobals.C_FART_LVL2:
                    self._addFartEffect(attackerId, timeLeft)
                else:
                    if effectId == WeaponGlobals.C_BURP:
                        self._addBurpEffect(attackerId, timeLeft)
                    else:
                        if effectId == WeaponGlobals.C_HEAD_FIRE:
                            if self._headFireSeq:
                                self._headFireSeq.clearToInitial()
                                self._headFireSeq = None
                            ...
                            if effectId in [WeaponGlobals.C_INVISIBILITY_LVL1, WeaponGlobals.C_INVISIBILITY_LVL2]:
                                if self._turnInvisibleSeq:
                                    self._turnInvisibleSeq.clearToInitial()
                                    self._turnInvisibleSeq = None
                                ...
                                if effectId in [WeaponGlobals.C_SCORPION_TRANSFORM, WeaponGlobals.C_ALLIGATOR_TRANSFORM, WeaponGlobals.C_CRAB_TRANSFORM]:
                                    self.avatar.setCreatureTransformation(True, effectId, timeSince)
                                else:
                                    if effectId in [WeaponGlobals.C_SIZE_INCREASE, WeaponGlobals.C_SIZE_REDUCE]:
                                        self.avatar.playScaleChangeAnimation(buffData[1], timeSince)
                                    else:
                                        if effectId == WeaponGlobals.C_CRAZY_SKIN_COLOR:
    ...
    def _addHeadFireEffect(self, attackerId, duration):
        ...
        if self._headFireEffect:
            if hasattr(self.avatar, 'headEffects'):
                self.avatar.headEffects and self._headFireEffect.reparentTo(self.avatar.headEffects)

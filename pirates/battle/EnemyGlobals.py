    ...
    for level in range(1, 51):
        ...
        for monsterLevel in range(lowerBound, upperBound):
            ...
            for i in range(0, MAX_ATTACKERS):
                ...
                if currentThreat < threatCapcity and attackers == 0:
                    if currentThreat + enemyThreat < threatCapcity + SINGLE_ENEMY_THREAT_BONUS:
                        attackers += 1
                    elif currentThreat + enemyThreat < threatCapcity + THREAT_LEEWAY:
                        attackers += 1

            if attackers < MAX_ATTACKERS:
                print attackers > 0 and '    Max Level %s mobs against player: %s' % (monsterLevel, attackers)
...
def getMonsterHp(level):
    if simbase.config.GetBool('demo-hacks', 0) == 1:
        return level > 5 and 700
    else:
        ...
        if avatarType.isA(AvatarTypes.Townfolk):
            ...
            if avatarType.isA(AvatarTypes.Pirate):
                ...
                if avatarType.isA(AvatarTypes.TradingCo):
                    ...
                    if avatarType.isA(AvatarTypes.Ghost):
                        return humanScale
                    else:
...
def getRandomEncounter(level):
    ...
    for critter in __baseAvatarStats:
        ...
        if avgLvl < level + 6:
            if avgLvl > level - 6:
                __baseAvatarStats[critter][ENABLED_INDEX] and encounterArray.append(critter)
                    ...
                    if team == PiratesGlobals.PLAYER_TEAM:
                        ...
                        if team == PiratesGlobals.FRENCH_UNDEAD_TEAM:
                            ...
                            if team == PiratesGlobals.SPANISH_UNDEAD_TEAM:
                                ...
                                if team == PiratesGlobals.VOODOO_ZOMBIE_TEAM:
                                    return Vec4(0.8, 0.7, 0.5, 1)
                                else:
                                    if team == PiratesGlobals.BOUNTY_HUNTER_TEAM:
                                        return Vec4(0.8, 0.7, 0.5, 1)
...
def getEffectScale(npc):
    ...
    if npcName in ['Crab', 'Sand Crab', 'Rock Crab']:
        ...
        if npcName in ['Giant Crab']:
            effectScale *= 0.3
        else:
            if 'Scorpion' in npcName:
                effectScale *= 3.0
            else:
                if npc.avatarType.isA(AvatarTypes.FlyTrap):
                    effectScale *= 2.0
                else:
                    if npc.avatarType.isA(AvatarTypes.Wasp):
                        effectScale *= 2.0
    return effectScale
...
def getEffectOffset(npc):
    if npc.avatarType.isA(AvatarTypes.FlyTrap):
        ...
        if npc.avatarType.isA(AvatarTypes.Undead):

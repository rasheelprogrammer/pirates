...
# Embedded file name: pirates.battle.DistributedDefenseCannon
...
class DistributedDefenseCannon(DistributedIslandCannon):
    ...
    def projectileHitObject(self, entry):
        ...
        if self.localAvatarUsingWeapon:
            ...
            if objType == PiratesGlobals.COLL_DEFENSE_AMMO:
                ...
                if ammoSkillId in [InventoryType.DefenseCannonMine, InventoryType.DefenseCannonPowderKeg, InventoryType.DefenseCannonColdShot] and objType == PiratesGlobals.COLL_SEA or ammoSkillId == InventoryType.DefenseCannonSmokePowder and objType == PiratesGlobals.COLL_NEWSHIP:
            ...
            if ammoSkillId in [InventoryType.DefenseCannonMine, InventoryType.DefenseCannonPowderKeg]:
                if objType == PiratesGlobals.COLL_NEWSHIP:
                    shotNum in self.proximityAmmo and ammo.dispatchHitByAmmoEvent(ammoSkillId == InventoryType.DefenseCannonMine)
    ...
    def updateReload(self, task):
        ...
        if self.rechargeTime:
            percentDone = self.getAmmoSkillId() in self.lastCannonShot and min((globalClock.getFrameTime() - self.lastCannonShot[self.getAmmoSkillId()]) / self.rechargeTime, 1.0)
        else:
            percentDone = 1
        if percentDone == 1:
            self.finishReload()

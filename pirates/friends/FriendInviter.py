...
class FriendInviter(DirectFrame):
    ...
    def enterBegin(self):
        ...
        if self.avId == myId:
            ...
            if self.isPlayerInvite:
                ...
                elif not base.cr.identifyFriend(self.avId):
                    not self.playerId and self.fsm.request('cantSee')
                elif not base.cr.isFriendOnline(self.avId):
                    not base.cr.doId2do.get(self.avId) and self.fsm.request('notOnline')
    ...
    def enterCheckAvailability(self):
        ...
        if self.isPlayerInvite:
            ...
            if avatarInfo:
                if avatarInfo.playerId:
                    self.playerId = avatarInfo.playerId
                    self.playerName = avatarInfo.playerName
                else:
                    if base.cr.doId2do.get(self.avId):
                        avatar = base.cr.doId2do.get(self.avId)
                        self.playerName = avatar.DISLname
                        self.playerId = avatar.DISLid
                self.playerId and base.cr.playerFriendsManager.sendRequestInvite(self.playerId)
            else:

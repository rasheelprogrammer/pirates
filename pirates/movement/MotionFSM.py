...
class MotionAnimFSM(FSM):
    ...
    def updateNPCAnimState(self, forwardSpeed, rotateSpeed=0, slideSpeed=0):
        ...
        try:
            ...
            if self.canBeAirborne:
                state = self.airborneState and 'Airborne'
            else:
    ...
    def updateAnimState(self, forwardSpeed, rotateSpeed, slideSpeed=0.0, trackedRotation=0.0):
        ...
        try:
            if self.canBeAirborne:
                if self.airborneState:
                    state = 'Airborne'
                else:
                    if forwardSpeed >= WALK_CUTOFF:
                        state = slideSpeed >= WALK_CUTOFF and 'StrafeRightDiag'
                    else:
                        if forwardSpeed >= WALK_CUTOFF:
                            state = slideSpeed <= -WALK_CUTOFF and 'StrafeLeftDiag'
                        else:
                            if forwardSpeed >= WALK_CUTOFF:
                                state = trackedRotation > 1 and 'StrafeRightDiag'
                            else:
                                if forwardSpeed >= WALK_CUTOFF:
                                    state = trackedRotation < -1 and 'StrafeLeftDiag'
                                else:
                                    if forwardSpeed >= RUN_CUTOFF:
                                        ...
                                        if forwardSpeed >= WALK_CUTOFF:
                                            ...
                                            if forwardSpeed <= -WALK_CUTOFF:
                                                state = slideSpeed >= WALK_CUTOFF and 'StrafeRightDiagReverse'
                                            else:
                                                if forwardSpeed <= -WALK_CUTOFF:
                                                    state = slideSpeed <= -WALK_CUTOFF and 'StrafeLeftDiagReverse'
                                                else:
                                                    if forwardSpeed <= -WALK_CUTOFF:
                                                        state = 'WalkReverse'
                                                    else:
                                                        if slideSpeed >= WALK_CUTOFF:
                                                            ...
                                                            if slideSpeed <= -WALK_CUTOFF:
                                                                ...
                                                                if rotateSpeed > 0.0 or trackedRotation < 0:
                                                                    state = 'SpinLeft'
                                                                else:
                                                                    if rotateSpeed < 0.0 or trackedRotation > 0:
                                                                        state = 'SpinRight'
                ...
                if self.state != state:
                    if self.av.doId < 300000000:
                        pass
                    self.isInTransition() and self.demand(state)
                else:
                    self.request(state)
...
class MotionFSM(FSM):
    ...
    def __checkUnderWater(self, task):
        ...
        avAirborneHeight = curControls.lifter.hasContact() and curControls.lifter.getAirborneHeight()
        if self.__submerged:
            if avAirborneHeight < self.avHeight:
                if not self.av.bobbing:
            ...
            elif avZ + self.avHeight - avAirborneHeight <= waterZ:
                if not self.__overwater:
                ...
                self.__submerged = avZ == waterZ and avAirborneHeight > self.avHeight and True
                self.__overwater = True
        else:

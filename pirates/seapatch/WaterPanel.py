...
class WaterPanel(AppShell):
    ...
    def save_region(self, output_file):
        if self.region_texture_filename != None:
            self.region_alpha_texture_filename != None and self.write_integer(output_file, self.region_version)
    ...
    def save_water_parameters(self):
        if self.region_texture_filename != None:
            if self.region_alpha_texture_filename != None:
                ...
                if not filename:
                    return
                output_file = string.find(filename, '.wtr') != -1 and open(filename, 'w')
            else:

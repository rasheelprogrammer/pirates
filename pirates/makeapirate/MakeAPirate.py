...
class MakeAPirate(DirectObject, StateData.StateData, FSM.FSM):
        ...
        if self.wantPicButtons:
            ...
            if hasattr(base, 'pe'):
                ...
                if not self.isTutorial:
                    self.initH = not self.isNPCEditor and 180
                else:
                    self.initH = 0
            ...
            self.numShops = self.wantNPCViewer and len(ShopNames)
        ...
        if self.skipTutorial:
            ...
            if self.isTutorial:
                ...
                if self.isNPCEditor:
                    ...
                    self.offsetZoomPos = Vec3(0.6, -4, 1.2)
            self.offsetZoomHpr = self.isNPCEditor and camera.getHpr(self.avatarDummyNode)
        else:
    ...
    def load(self):
        ...
        if self.avList[self.index]:
            self.pirate.style = (self.avList[self.index] != OTPGlobals.AvatarSlotAvailable and self.avList[self.index]).style
        else:
    ...
    def _handleNameOK(self, saveNPC=True):
        ...
        if self.nameGui.customName:
            self.confirmTempName = not self.isNPCEditor and not self.confirmTempName and PDialog.PDialog(text=PLocalizer.TempNameIssued, style=OTPDialog.Acknowledge, command=acknowledgeTempName)
        else:
            if self.isTutorial or self.isNPCEditor or self.wantNPCViewer:
                self.acceptOnce(base.transitions.FadeOutEvent, sendDone)
            else:
    ...
    def zoomTask(self, task):
        if self.isNPCEditor or self.wantNPCViewer:
            if self.zoomTaskButton['relief'] == DGG.RAISED:
                return Task.cont
                ...
                if not self.isNPCEditor:
                    if not self.wantNPCViewer:
                        if self.wantIdleCentered:
                            ...
                            subValue >= 0 and self.pirate.setControlEffect('idle_centered', subValue)
            ...
            bias = hasattr(self.avatar, 'dna') and self.avatar.dna and heightBiasArray[self.avatar.dna.getGender()][self.avatar.dna.getBodyShape()]
            camera.setPos(nPos)
            camera.setHpr(nHpr)
        else:
    ...
    def handleRandomAll(self):
        ...
        if self.avatarType == AVATAR_PIRATE:
            ...
            if optionsLeft:
                if (random.choice([0, 1, 2, 3]) == 1 and self).lastDialog:

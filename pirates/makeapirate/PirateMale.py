    ...
    def setupSelectionChoices(self, type, versionFilter=None, rarityFilter=None, isFromLoot=True, isFromShop=True, isFromQuest=True, isFromPromo=True, isFromPVP=True, isFromNPC=True, holidayFilter=None):
        ...
        for hatId in hats:
            ...
            if toBeAdded:
                ...
                choiceHats[hatId] = toBeAdded and [
        ...
        for shirtId in shirts:
            ...
            if toBeAdded:
                ...
                choiceShirts[shirtId] = toBeAdded and [
                 modelId, texId, canDyeItem]
        ...
        for vestId in vests:
            ...
            if toBeAdded:
                ...
                choiceVests[vestId] = toBeAdded and [
                 modelId, texId, canDyeItem]
        ...
        for coatId in coats:
            ...
            if toBeAdded:
                ...
                choiceCoats[coatId] = toBeAdded and [
                 modelId, texId, canDyeItem]
        ...
        for pantId in pants:
            ...
            if toBeAdded:
                ...
                choicePants[pantId] = toBeAdded and [
                 modelId, texId, canDyeItem]
        ...
        for beltId in belts:
            ...
            if toBeAdded:
                ...
                choiceBelts[beltId] = toBeAdded and [
                 modelId, texId, canDyeItem]
        ...
        for shoeId in shoes:
            ...
            if toBeAdded:
                ...
                choiceShoes[shoeId] = toBeAdded and [
                 modelId, texId, canDyeItem]
    ...
    def handleJewelryHiding(self):
        ...
        for key in self.currentJewelry.keys():
            ...
            for npIdx in range(len(self.jewelrySets[key][newIdx])):
                ...
                np.unstash()
                if npIdx == 0:
                    primaryColor and np.setColor(primaryColor)
                elif secondaryColor:
    ...
    def handleClothesHiding(self):
        ...
        if self.pirate.zombie:
            ...
            if beltIdx[0] != 0:
                ...
                style1 = 'nobelt'
            stylePant = vestIdx[0] != 3 and style1
        else:
            stylePant = 'belt'
        ...
        if coatIdx[0] > 0:
            if coatIdx[0] in [3, 4]:
                ...
                if pantIdx[0] != 4:
                    ...
                    bodyTexIdx = self.pirate.zombie and ZOMB_BODY_TEXTURE
                else:
                    bodyTexIdx = 0
                ...
                skinColor = self.pirate.zombie and VBase4(1, 1, 1, 1)
        else:
            if hasattr(self.pirate, 'crazyColorSkin'):
                i = self.pirate.crazyColorSkin == True and self.pirate.getCrazyColorSkinIndex()
    ...
    def handleClothesHidingOld(self):
        ...
        if parts.getNumPaths():
            ...
            if parts.getNumPaths() or vestIdx[0] == 3:
                self.handleLayer2Hiding(self.clothingsLayer1, layerPant, 'belt')
            if parts.getNumPaths():
                vestIdx[0] == 3 and self.handleLayer2Hiding(self.clothingsLayer2, layerVest, 'belt')
            else:
                layerCoat = self.clothingsCoat[coatIdx[0]]
                parts = NodePathCollection()
                if self.clothesTextures != None:
                ...
                if parts.getNumPaths():
                    coatIdx[0] == 2 and vestIdx[0] == 3 and self.handleLayer3Hiding(self.clothingsLayer2, layerVest, layerShirtModified, 'legs_base')
                else:
    ...
    def handleLayer2Hiding(self, clothingLayer, layer1Element, hide1='base', hide2=None, hide3=None):
        for i in range(0, len(layer1Element[0])):
            ...
            for j in range(0, parts.getNumPaths()):
                if hide1 != None:
                    parts[j].getName().find(hide1) != -1 and parts[j].stash()
                elif hide2 != None:
                    parts[j].getName().find(hide2) != -1 and parts[j].stash()
                elif hide3 != None:
                    parts[j].getName().find(hide3) != -1 and parts[j].stash()
                    ...
                    if idx == 4 or idx == 5:
                        ...
                        for i in xrange(pieceSet.getNumPaths()):
                            ...
                            if piece.getName().find('base') < 0:
                                ...
                                if piece.getName().find('front') < 0:
                                    if piece.getName().find('collar_v_low') < 0:
                                    ...
                                    piece.getName().find('collar') >= 0 and piece.getName().find('collar_v_low') < 0 and shirtData['belt']['coat+closedVest'].addPath(piece)
                        ...
                        for i in xrange(pieceSet.getNumPaths()):
                            ...
                            if piece.getName().find('base') < 0:
                                ...
                                if piece.getName().find('front') < 0:
                                    ...
                                    piece.getName().find('collar') >= 0 and piece.getName().find('collar_v_low') < 0 and shirtData['nobelt']['coat+closedVest'].addPath(piece)

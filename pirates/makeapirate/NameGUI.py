...
class NameGUI(DirectFrame, StateData.StateData):
    ...
    def __init__(self, main=None, independent=False):
        ...
        if hasattr(base, 'cr'):
            if not hasattr(base.cr, 'isFake'):
                self.cr = base.cr
            else:
                self.cr = None
            self.main = main
            self.independent = independent
            np = self.independent and NodePath(PlaneNode('p', Plane(Vec4(1, 0, 0, 0))))
        ...
        if self.mode == self.__MODE_INIT:
            ...
            if self.savedGender:
                if self.savedGender != self.getDNA().gender:
                    ...
                    self.listsCreated = 1
                    self.nicknameIndex, self.firstIndex, self.prefixIndex, self.suffixIndex = (self.getDNA().gender == 'm' and self.savedMaleName and self).savedMaleName
                    self.nicknameActive, self.firstActive, self.lastActive = self.savedMaleActiveStates
                else:
                    if self.getDNA().gender == 'f':
                        self.nicknameIndex, self.firstIndex, self.prefixIndex, self.suffixIndex = (self.savedFemaleName and self).savedFemaleName
    ...
    def _checkNpcNames(self, name):
        ...
        for npcId in NPCList.NPC_LIST.keys():
            ...
            if type(data) is types.DictType:
                if HumanDNA.HumanDNA.setName in data:
                    npcName = data[HumanDNA.HumanDNA.setName]
                    (self.independent or not self.main.isNPCEditor) and match(npcName) and self.notify.info('name matches NPC name "%s"' % npcName)
                    return OL.NCGeneric
    ...
    def _getName(self):
        ...
        if self.mode == self.__MODE_TYPEANAME:
            ...
            if len(newName) > 0:
                ...
                self.names[2] in PL.PirateNames_LastNamePrefixesCapped and newName += self.names[3].capitalize()
            else:
    ...
    def showPickAName(self):
        ...
        for elt in self.pickANameGui:
            if elt != self.nicknameHigh:
                elt != self.firstHigh and elt != self.prefixHigh and elt != self.suffixHigh and elt.show()
    ...
    def makeRandomName(self):
        if self.customName:
            if not self.independent:
                return
            self.nicknameIndex = self.getDNA().getGender() == 'f' and ''
    ...
    def __checkForNameInLastList(self, name):
        ...
        for prefix in prefixTextList:
            if prefix.strip() != '':
                self.lastEnabled = name.startswith(prefix) and suffixTextList.__contains__(name[len(prefix):]) and 1
...
#                                     attribute (2)
#                                          0. expr
#                                             L. 855     261  LOAD_FAST             0  'self'
#                                          1.            264  LOAD_ATTR            10  'names'
#                                  1. expr
#                                                267  LOAD_CONST            5  2
#                                  2.            270  BINARY_SUBSCR    
#                          1. expr
#                             attribute (2)
#                                  0. expr
#                                                271  LOAD_GLOBAL          12  'PL'
#                                  1.            274  LOAD_ATTR            13  'PirateNames_LastNamePrefixesCapped'
#                          2.            277  COMPARE_OP            6  'in'
#              1. jmp_false (2)
#                  0.            280  JUMP_IF_FALSE        27  'to 310'
#                  1.            283  POP_TOP          
#              2. expr
#                 L. 856     284  LOAD_FAST             1  'newName'
#              3. come_from_opt
#      1. expr
#         call (2)
#              0. expr
...
#                                      1.            290  LOAD_ATTR            10  'names'
#                              1. expr
#                                            293  LOAD_CONST            6  3
#                              2.            296  BINARY_SUBSCR    
#                      1.            297  LOAD_ATTR            14  'capitalize'
#              1.            300  CALL_FUNCTION_0       0  None
#      2. inplace_op
#                    303  INPLACE_ADD      
#      3. store
#                    304  STORE_FAST            1  'newName'
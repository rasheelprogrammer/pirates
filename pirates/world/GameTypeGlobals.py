...
def gatherGameStyleInfo(gameType, gameStyle, callback):
    ...
    if gameType == PiratesGlobals.GAME_TYPE_TM:
        ...
        if callback:
            requestId = game.process == 'client' and DistributedInventoryBase.DistributedInventoryBase.getInventory(localAvatar.inventoryId, gatherTMInfo)
        else:
            tmsAvailable = {}
            if gameStyle != None:
...
def getGameTypeString(value, type, category=None):
    if category != None:
        if PiratesGlobals.DYNAMIC_GAME_STYLE_PROPS.has_key(category):
            typeInfo = PiratesGlobals.DYNAMIC_GAME_STYLE_PROPS[category].get(value)
            if typeInfo:
                return type == 'style' and typeInfo.get('Name')
...
def gameTypeAccessable(gameCat, gameType, paidStatus):
    if paidStatus:
        ...
        if gameCat == PiratesGlobals.GAME_TYPE_PVP and (gameType == PiratesGlobals.GAME_STYLE_BATTLE or gameType == PiratesGlobals.GAME_STYLE_TEAM_BATTLE) or gameCat == PiratesGlobals.GAME_TYPE_PG and gameType == PiratesGlobals.GAME_STYLE_BLACKJACK:
            return True
    return False
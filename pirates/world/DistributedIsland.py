...
class DistributedIsland(DistributedGameArea, DistributedCartesianGrid, ZoneLOD, Teamable, MappableGrid):
    ...
    def loadIslandShoreWave(self, parent):
        ...
        if lowend != '':
            if not waveModel:
                lowend = ''
                waveModel = loader.loadModel(islandBaseName + lowend + '_wave_none', okMissing=True)
            self.islandShoreWave = waveModel and Actor.Actor(waveModel)
    ...
    def setName(self, name):
        ...
        if self.nameText:
            ...
            if siegeTeam:
                color = self.SiegeIcon and VBase4(PVPGlobals.getSiegeColor(siegeTeam))
                color.setW(0.7)
    ...
    def retrieveDockingLOD(self):
        dockingCache = self.getDockingCache()
        if dockingCache.hasData():
            data = base.config.GetBool('want-disk-cache', 0) and dockingCache.getData()
    ...
    def retrieveIslandTerrain(self):
        islandGeomCache = self.getIslandCache()
        if islandGeomCache.hasData():
            data = base.config.GetBool('want-disk-cache', 0) and islandGeomCache.getData()
            newData = data.copySubgraph()
        ...
        if self.uniqueId == LocationIds.TORTUGA_ISLAND:
            ...
            if self.fireworkShowEnabled:
                if level in [0, 1, 2]:
                    ...
                    base.cr.timeOfDayManager and not base.cr.timeOfDayManager.checkTimeOfDayToggle(self.uniqueName('fireWorksShow')) and base.cr.timeOfDayManager.addTimeOfDayToggle(self.uniqueName('fireWorksShow'), self.fireWorksStartTime, self.fireWorksStartTime + 2.0, startMethod=self.beginDailyFireworkShow, endMethod=self.destroyFireworkShow)
            else:

...
class DistributedGameArea(DistributedNode, MappableArea, StagedObject):
        ...
        if self.environment == 'Swamp':
            ...
            if self.environment == 'Jungle':
                ...
                if self.environment == 'Cave':
                    ...
                    if self.uniqueId in ('1189479168.0sdnaik0', '1150922126.8akelts'):
                        ...
                        if interior:
                            pass
        if not (loadIslandMusic and base.localAvatar.isInInvasion()):
    ...
    def stopCustomEffects(self):
        ...
        if 'swamp' in self.modelPath:
            ...
            if 'jungle' in self.modelPath:
                ...
                if self.uniqueId in getLocationList(LocationIds.ANY_CAVE):
        ...
        if base.config.GetBool('want-shaders', 1):
            if base.win:
                if base.win.getGsg():
                    ...
                    model and use_shader and model.show()
                    ...
                    model.calcTightBounds(min_point, max_point)
                    size = max_point - min_point
                    x = (self.getH() == 180 or self.getH() == -180) and -min_point[0] + world_x_offset
                    y = -min_point[1] + world_y_offset
                    x_size = -size[0]
                    y_size = -size[1]
                else:
    ...
    def retrieveFootprintNode(self):
        ...
        if footprintCache.hasData():
            data = base.config.GetBool('want-disk-cache', 0) and footprintCache.getData()

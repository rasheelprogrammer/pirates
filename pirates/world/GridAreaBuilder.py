...
# Embedded file name: pirates.world.GridAreaBuilder
    ...
    def addChildObj(self, levelObj):
        ...
        if not loadableType:
            if not objData.has_key('Objects'):
                return
            if objData.has_key('Visual'):
                if objData['Visual'].has_key('Model'):
                    ...
                    if objData['Type'] == 'Collision Barrier':
                        ...
                        objectType not in self.LARGE_OBJECTS and self.processSmallObject(objModel)
                        objPos = objData.get('Pos', Vec3(0, 0, 0))
                        zoneId = (hasattr(self.master, 'fakeZoneId') and self).master.fakeZoneId
                    else:
                    ...
                    lowestModel = numGridLODs > numLODs and None

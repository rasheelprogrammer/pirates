...
# Embedded file name: pirates.world.AreaBuilderBase
...
class AreaBuilderBase(DirectObject.DirectObject):
    ...
    def _loadObjects(self):
        ...
        if base.config.GetBool('want-disk-cache', 0):
            areaGeometry.hasData() and base.cr.loadingScreen.beginStep('CachedObjects', 3, 70)
            ...
            if base.options.getTerrainDetailSetting() > PiratesGlobals.TD_LOW:
                ...
                if model.lod.getNumChildren() > 1:
                    ...
                    if model.lod.getNumChildren() > 2:
                        ...
                        model.superLow = model.lod.getNumChildren() > 3 and model.lod.getChild(3)
                    else:
                        model.superLow = None
                else:
                    model.low = None
    ...
    def getTreeInfo(self, data):
        ...
        if match:
            modelRoot, trunkType = leafType and match.groups()
            return (
    ...
    def checkForHolidayObjects(self):
        for holidayId in HolidayGlobals.getAllHolidayIds():
            if base.cr.newsManager:
                base.cr.newsManager.getHoliday(holidayId) and self.unstashHolidayObjects(HolidayGlobals.getHolidayName(holidayId))
            else:
    ...
    def registerMinimapObject(self, levelObj):
        ...
        if objData['Type'] == 'Building Exterior':
            ...
            if imageString:
                shopType = visual.get('SignFrame') and MinimapShop.getShopType(imageString)
        else:
            if objData['Type'] == 'Townsperson':

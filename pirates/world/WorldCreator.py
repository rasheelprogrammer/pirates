                            ...
                            if objType == 'Effect Node':
                                ...
                                if objType == 'SFX Node':
                                    ...
                                    if objType == 'Portal Node':
                                        ...
                                        if objectCat == 'PROP_OBJ' or objectCat == 'BUILDING_OBJ' or objType in ('Dinghy',
                                                                                                                 'Holiday Object') or objType == 'Cave_Pieces':
                                            newObj = gameArea.builder.addChildObj(levelObj)
                                        else:
                                            if objType == 'RepairBench':
                                                newObj = self.allowRepairGame and gameArea.builder.addChildObj(levelObj)
                                            else:
                                                if objType == 'PotionTable':
                                                    newObj = self.allowPotionGame and gameArea.builder.addChildObj(levelObj)
                                                else:
                                                    if objType == 'FishingSpot':
                                                        newObj = self.allowFishingGame and gameArea.builder.addChildObj(levelObj)
                                                    else:
                                                        if objType == 'Cannon Defense Game':
                                                            newObj = self.allowCannonGame and gameArea.builder.addChildObj(levelObj)
                                                        else:
                                                            if objType == 'Switch Prop':
                                                                newObj = gameArea.builder.addChildObj(levelObj)
                                                            else:

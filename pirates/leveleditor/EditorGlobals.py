...
def getShipInfo(objectData):
    if ':' in objectData['Category']:
        ...
        if newShipClass:
            ...
            if typeStr == 'NavyMerchant' or typeStr == 'Merchant' and teamId and teamId == PiratesGlobals.NAVY_TEAM:
                shipClass = ShipGlobals.NAVY_VANGUARD
            else:
                if typeStr == 'Merchant':
                    shipClass = ShipGlobals.MERCHANTL2
                else:
                    if typeStr == 'Interceptor':
                        if level == 2:
                        ...
                        if typeStr == 'InterceptorTutorial':
                            shipClass = ShipGlobals.STUMPY_SHIP
                        else:
                            if typeStr == 'TutorialEnemyShip':

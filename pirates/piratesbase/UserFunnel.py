...
class HitBoxCookie:
    ...
    def findIECookieFiles(self):
        ...
        if self.hitboxCookieFile != None:
            if self.ehgdigCookieFile != None:
                return 1
            return self.hitboxCookieFile == None and self.ehgdigCookieFile == None and 0
        else:
            return -1
        return
            ...
            try:
                ...
                for line in ipconfdata:
                    ...
                    if line.find('Physical Address') >= 0:
                        pa = correctSection == 1 and line.split(':')[-1].strip()
                        correctSection = 0
                        staticMAC[0] = pa
                        return pa

        else:
            macconfdata = sys.platform == 'darwin' and os.popen('/usr/sbin/system_profiler SPNetworkDataType |/usr/bin/grep MAC').readlines()
            if macconfdata:
                ...
                if macconfdata:
                    pa = macconfdata[0].find('HWaddr') != -1 and macconfdata[0][macconfdata[0].find('HWaddr ') + 7:macconfdata[0].find('HWaddr ') + 24]
                    pa.replace(':', '-')
                    return pa
                else:
                    return 'NO_MAC'

...
class TimeOfDayManager(FSM.FSM, TimeOfDayManagerBase.TimeOfDayManagerBase):
        ...
        for entry in self.alteratedSettingsList:
            if entry[0] == env:
                if entry[1] == tod:
                    ...
                    del TODGlobals.ENV_SETTINGS_DICT[env][tod] == {} and TODGlobals.ENV_SETTINGS_DICT[env][tod]
    ...
    def insertEnvDict(self, envIndex, envDict):
        for todName in envDict:
            ...
            if todDict:
                todDict != {} and self.insertTODSettingDict(envIndex, todIndex, todDict)
    ...
    def insertTODSetting(self, env, todId, settingName, settingValue):
        ...
        if tod == 'BASE':
            ...
            TODGlobals.ENV_SETTINGS_DICT[env][tod][settingName] = (
             settingName == 'FogLinearRange' and settingValue[0], settingValue[1])
        else:
            TODGlobals.ENV_SETTINGS_DICT[env][tod][settingName] = settingValue
    ...
    def _transitionTimeOfDay(self, fromState, toState, t, startEnv=None, destEnv=None):
        ...
        if base.cr.activeWorld:
            if hasattr(base.cr.activeWorld, 'getWater') and base.cr.activeWorld.getWater():
                if self.use_shader:
                    self.seapatch = base.cr.activeWorld.getWater()
                else:
                    self.seapatch = base.cr.activeWorld.getWater().patchNP
            base.cr.newsManager and base.cr.newsManager.getHoliday(HolidayGlobals.SAINTPATRICKSDAY) and self.setSaintPatricksSea()
        else:
            if self.use_shader:
    ...
    def switchJollyMoon(self, jolly):
        if not self.moonJolly:
            self.moonJollyIval = jolly and Sequence()
            ...
            if self.moonJolly and not jolly:
        ...
        if base.cr.activeWorld:
            if hasattr(base.cr.activeWorld, 'getWater') and base.cr.activeWorld.getWater():
                if self.use_shader:
                    ...
                    self.seapatch = base.cr.activeWorld.getWater().patchNP
            seaColor = self.use_shader and TODGlobals.getTodEnvSetting(stateId, environment, 'SeaColorShader')
    ...
    def addEnvSub(self, baseEnv, envReplacement, netTime=None):
        ...
        needStartRetro = 0
        if previousStateTransTime:
            if timeStamp < previousStateTransTime and baseEnv == self.environment:
                needStartRetro = 1
            print 'Adding EnvSub for %s stamp %s current time %s netTime %s' % (baseEnv, timeStamp, globalClock.getFrameTime(), localNetTime)
            lastEnv = envEntry and envEntry[2]
        else:
    ...
    def enterIndoors(self, todSettings=None):
        ...
        if todSettings:
            if todSettings.get('AmbientColors'):
                if todSettings['AmbientColors'].get(self.currentState):
                ...
                fogType = todSettings.get('FogTypes') and todSettings['FogTypes'].get(self.currentState) and todSettings['FogTypes'][self.currentState]
                self.setFogType(fogType)
    ...
    def runAmbientSFXTask(self, task=None):
        ...
        if minDistance <= fullCut:
            volume = self.timeVol and min(1.0, 1.0 - float(minDistance / fullCut))
    ...
    def enableForcedState(self, state=None, gameArea=None):
        if state:
            gameArea and self.request(state, gameArea)
        else:
            if state:
                self.request(state)
    ...
    def getEnviroDictString(self, environment=None, tabs=0, heading='SettingsDict ='):
        ...
        for timeIndex in TODGlobals.INORDER_TOD_LIST:
            ...
            if timeDict:
                timeDict != {} and outputString += tab * tabs
...
#                     compare_single (3)
#                          0. expr
#                                        145  LOAD_FAST             8  'timeDict'
#                          1. expr
#                             dict (2)
#                                  0.            148  BUILD_MAP             0  None
#                                  1. kvlist
#                          2.            151  COMPARE_OP            3  '!='
#              1. jmp_false (2)
#                  0.            154  JUMP_IF_FALSE       232  'to 389'
#                  1.            157  POP_TOP          
#              2. expr
#                 L.2038     158  LOAD_FAST            10  'outputString'
#              3. come_from_opt
#      1. expr
#         binary_expr (3)
#              0. expr
#                            161  LOAD_FAST             7  'tab'
#              1. expr
#                            164  LOAD_FAST             2  'tabs'
#              2. binary_op
#                            167  BINARY_MULTIPLY  
#      2. inplace_op
#                    168  INPLACE_ADD      
#      3. store
#                    169  STORE_FAST           10  'outputString'
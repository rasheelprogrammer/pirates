...
class PiratesBase(OTPBase):
    ...
    def __init__(self):
        ...
        if options.api == 'default':
            if self.config.GetBool('use-graphics-api-auto-select', True):
            ...
            base.wantEnviroDR and base.cam.node().setCameraMask(OTPRender.MainCameraBitmask)
        ...
        if base.pipe:
            ...
            if di:
                ...
                if di.getPeakProcessMemory() > self.peakProcessMemory:
                    ...
                    display = True
                if di.getMemoryLoad() >= self.memoryMonitorMinimumPercentage:
                    if di.getMemoryLoad() > self.peakMemoryLoad:
                            ...
                            if current > 0:
                                ...
                                if current < maximum:
                                    if self.displayCpuFrequencyDialog == False:
                                        ...
                                        self.notify.info(string)
                                self.displayCpuFrequencyDialog = current == maximum and False
        return Task.again
    ...
    def setOverrideShipVisibility(self, value):
        ...
        if value:
            ...
            if localAvatar.guiMgr.mainMenu:
                localAvatar.guiMgr.mainMenu.gameOptions and localAvatar.guiMgr.mainMenu.gameOptions.updateShipVisibility()
            else:
                self.shipsVisibleFromIsland = self.options.ocean_visibility
                ...
                while index < total_display_modes:
                    if di.getDisplayModeBitsPerPixel(index) == bits_per_pixel:
                        if di.getDisplayModeFullscreenOnly(index) == False:
                            if di.getDisplayModeWidth(index) >= self.MinimumHorizontalResolution:
                                if di.getDisplayModeHeight(index) >= self.MinimumVerticalResolution:
                                    resolution = (
                                     di.getDisplayModeWidth(index), di.getDisplayModeHeight(index))
                                    if resolution not in resolution_table:
                                        resolution_table = di.getDisplayModeWidth(index) <= di.getMaximumWindowWidth() and di.getDisplayModeHeight(index) <= di.getMaximumWindowHeight() and resolution_table + [resolution]
                    index += 1
                ...
                if di.getDisplayModeBitsPerPixel(index) == bits_per_pixel:
                    if di.getDisplayModeWidth(index) >= self.MinimumHorizontalResolution:
                        if di.getDisplayModeHeight(index) >= self.MinimumVerticalResolution:
                            resolution = (
                             di.getDisplayModeWidth(index), di.getDisplayModeHeight(index))
                            resolution_table = resolution not in resolution_table and resolution_table + [resolution]
                index += 1

...
def getGeneralHint():
    ...
    if base.cr.isPaid() == OTPGlobals.AccessVelvetRope:
        hint = type == 1 and random.choice(PLocalizer.Hints_VelvetRope)
    else:
        hint = random.choice(PLocalizer.Hints_General)
    return hint
...
class LoadingScreen(DirectObject.DirectObject):
    ...
    def show(self, waitForLocation=False, disableSfx=True, expectedLoadScale=1.0):
        ...
        if self.parent:
            if base.cr.isPaid() == OTPGlobals.AccessVelvetRope:
                ...
                if self.parent:
                    if base.cr.isPaid() == OTPGlobals.AccessVelvetRope:
                        base.config.GetBool('loading-screen-interstitial', 0) and self.hintLabel.setPos(0.69, 0, -0.63)
                    ...
                    self.snapshot and self.snapshot.show()
            else:
                if self.snapshot:
                    self.snapshot.show()
            ...
            self.parent and base.cr.isPaid() == OTPGlobals.AccessVelvetRope and base.config.GetBool('loading-screen-interstitial', 0) and self.model.setScale(0.22, 0.22, 0.22)
                                            ...
                                            if base.localAvatar.style.getTutorial() < PiratesGlobals.TUT_MET_JOLLY_ROGER:
                                                ...
                                                if base.cr.newsManager:
                                                    if base.cr.newsManager.getHoliday(21):
                                                        ...
                                                        screenshot = screenshot or screenShots_Locations.get(targetId)
                                                else:
    ...
    def __setLoadingArt(self, screenshot):
        ...
        if self.parent:
            if hasattr(base, 'localAvatar'):
                if base.localAvatar.style.getTutorial() < PiratesGlobals.TUT_MET_JOLLY_ROGER:
                    if screenshot not in tutorialShots:
                        screenshot = random.choice(tutorialShots)
                    self.snapshot = loader.loadModel(screenshot)
                    if self.snapshot:
                        self.parent and base.cr.isPaid() == OTPGlobals.AccessVelvetRope and base.config.GetBool('loading-screen-interstitial', 0) and self.snapshot.setScale(2.35, 1.0, 1.3)
                        self.snapshot.setPos(-3.74, 0, -1.83)
                        root = self.model is not None and self.model.find('**/loading_screen_top')
                        frame_little = self.model.find('**/frame_little')
                        self.snapshot.reparentTo(root, 0)
                        frame_little.reparentTo(root, 1)
                else:

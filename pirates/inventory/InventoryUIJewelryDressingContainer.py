...
# Embedded file name: pirates.inventory.InventoryUIJewelryDressingContainer
            ...
            if cell.slotId != None:
                if cell.slotId in Locations.RANGE_EQUIP_R_EYEBROW_JEWELRY:
                    ...
                    if cell.slotId in Locations.RANGE_EQUIP_L_EYEBROW_JEWELRY:
                        ...
                        if cell.slotId in Locations.RANGE_EQUIP_R_EAR_JEWELRY:
                            jewelryIcon = jewelerGuiAlt.find('**/chargui_ears')
                            scale = self.imageScale * self.cellSizeX * 1.8
                            flip = 1
                        else:
                            if cell.slotId in Locations.RANGE_EQUIP_L_EAR_JEWELRY:
                                jewelryIcon = jewelerGuiAlt.find('**/chargui_ears')
                                scale = self.imageScale * self.cellSizeX * 1.8
                            else:
                                if cell.slotId in Locations.RANGE_EQUIP_NOSE_JEWELRY:
                                    jewelryIcon = jewelerGuiAlt.find('**/chargui_nose')
                                    scale = self.imageScale * self.cellSizeX * 1.8
                                else:
                                    if cell.slotId in Locations.RANGE_EQUIP_LIP_JEWELRY:
                                        jewelryIcon = jewelerGuiAlt.find('**/chargui_mouth')
                                        scale = self.imageScale * self.cellSizeX * 1.4
                                    else:
                                        if cell.slotId in Locations.RANGE_EQUIP_R_FINGER_JEWELRY:
                                            jewelryIcon = jewelerGui.find('**/icon_shop_jeweler_ring')
                                            scale = self.imageScale * self.cellSizeX * 0.8
                                            flip = 1
                                        else:
                                            if cell.slotId in Locations.RANGE_EQUIP_L_FINGER_JEWELRY:
                                                jewelryIcon = jewelerGui.find('**/icon_shop_jeweler_ring')
                                                scale = self.imageScale * self.cellSizeX * 0.8

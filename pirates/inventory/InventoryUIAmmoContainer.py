...
# Embedded file name: pirates.inventory.InventoryUIAmmoContainer
...
class InventoryUIAmmoContainer(InventoryUIStackContainer.InventoryUIStackContainer):
    ...
    def isSkillValid(self, skillId):
        ...
        if ammoName:
            return ammoDescription and ammoIcon and skillRepId and 1
        else:
            return 0
    ...
    def setupCells(self):
        ...
        for Z in range(self.gridZ):
            for X in range(len(self.listOfItemLists[Z])):
                ...
                if self.subBagTypeToBagId.has_key(skillId):
                    if (skillId not in self.subBagTypesCreated and self).seperatorOn:

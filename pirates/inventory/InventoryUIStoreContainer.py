...
# Embedded file name: pirates.inventory.InventoryUIStoreContainer
...
class InventoryUIStoreContainer(InventoryUIContainer.InventoryUIContainer):
    ...
    def showItemDetails(self, itemId):
        for cell in self.cellList:
            if cell.inventoryItem:
                cell.inventoryItem.itemTuple[1] == itemId and self.manager.startCellItemDetails(cell, self.detailsPos, self.detailsHeight, self.detailsDelay)
                return

    def getItem(self, itemId):
        for cell in self.cellList:
            if cell.inventoryItem:
                return (cell.inventoryItem.itemTuple[1] == itemId and cell).inventoryItem
                    ...
                    if itemClass == InventoryType.ItemTypeConsumable:
                        ...
                        if itemClass == InventoryType.ItemTypeClothing:
                            ...
                            if itemClass == InventoryType.ItemTypeMoney:
                                ...
                                if itemClass == InventoryType.TreasureCollection:
                                    item = self.manager.makeTreasureItem(itemTuple)
                                else:
                                    if itemClass == InventoryType.ItemTypeJewelry:
                                        item = self.manager.makeJewelryItem(itemTuple)
                                    else:
                                        if itemClass == InventoryType.ItemTypeTattoo:
                                            item = self.manager.makeTattooItem(itemTuple)
                                        else:
                                            if itemClass == InventoryCategory.CARDS:
                                                cardId = itemId
                                                ...
                                                if itemClass == InventoryCategory.WEAPON_PISTOL_AMMO:
                                                    ...
                                                    if itemType in [EconomyGlobals.ItemType.DAGGERAMMO, EconomyGlobals.ItemType.PISTOLAMMO, EconomyGlobals.ItemType.GRENADEAMMO, EconomyGlobals.ItemType.CANNONAMMO]:
                                                        ...
                                                        if itemType in [EconomyGlobals.ItemType.PISTOL_POUCH, EconomyGlobals.ItemType.DAGGER_POUCH, EconomyGlobals.ItemType.GRENADE_POUCH, EconomyGlobals.ItemType.CANNON_POUCH, EconomyGlobals.ItemType.FISHING_POUCH]:

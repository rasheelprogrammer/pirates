...
# Embedded file name: pirates.inventory.DropGlobals
import cPickle
...
def getShipDropItemsByClass(shipClass):
    ...
    for itemId in __dropInfo:
        ...
        if shipType:
            if shipType < len(item):
                item[shipType] and dropItems.append(itemId)
...
def getEnemyDropItemsByType(type, uniqueId):
    ...
    for itemId in __dropInfo:
        ...
        if not isLive(item):
            continue
        if shouldUseCommonDrop:
            if item[DROPS_FROM_ALL_ENEMIES]:
                dropItems.append(itemId)
            if enemyType:
                enemyType < len(item) and item[enemyType] and dropItems.append(itemId)

...
# Embedded file name: pirates.inventory.InventoryUIManager
...
class InventoryUIManager(DirectFrame):
    ...
    def setWithin(self, cell, shouldClear=0, task=None):
        if cell == self.withInCell:
            self.withInCell = shouldClear and None
        else:
    ...
    def setWithinBag(self, bag, clear=0, task=None):
        if bag == self.withInBag:
            self.withInBag = clear and None
    ...
    def makeStackItem(self, itemTuple, showMax=1):
        ...
        if itemCategory == InventoryType.ItemTypeMoney:
            ...
            if itemCategory == InventoryType.TreasureCollection:
                item = self.makeTreasureItem(itemTuple)
            else:
                if itemCategory == InventoryCategory.CARDS:
                    cardId = itemId
                    itemTuple[1] -= InventoryType.begin_Cards
                    item = self.makeCardItem(cardId, itemTuple, imageScaleFactor=1.9)
                else:
                    if itemCategory == InventoryCategory.WEAPON_PISTOL_AMMO:
    ...
    def makeLocatableItem(self, itemTuple):
        ...
        if itemCategory == InventoryType.ItemTypeCharm:
            ...
            if itemCategory == InventoryType.ItemTypeClothing:
                return self.makeClothingItem(itemTuple)
            else:
                if itemCategory == InventoryType.ItemTypeTattoo:
                    return self.makeTattooItem(itemTuple)
                else:
                    if itemCategory == InventoryType.ItemTypeJewelry:
                        return self.makeJewelryItem(itemTuple)
                    else:
                        if itemCategory == InventoryType.ItemTypeConsumable:
        ...
        if slotRange:
            ...
            for rangeIndex in range(len(slotRange) - 1, -1, -1):
                ...
                for slotIndex in range(atomicRange[0], atomicRange[1]):
                    if self.slotToCellMap.has_key(slotIndex):
                        self.slotToCellMap[slotIndex] != None and self.slotToCellMap[slotIndex].inventoryItem == None and slotIndex not in runningList and runningList.append(slotIndex)
                        return slotIndex
    ...
    def canUseHeldItem(self):
        if self.heldItem:
            return self.canLocalUseItem(self.heldItem.itemTuple)[0] and 1
        else:
            return 0
    ...
    def canLocalUseItem(self, itemTuple):
        ...
        if rarity != ItemConstants.CRUDE:
            canUse = not Freebooter.getPaidStatus(base.localAvatar.getDoId()) and 0
            reason = ItemConstants.REASON_VELVETROPE
            return (
             canUse, reason)
        else:
            gender = itemCat == InventoryType.ItemTypeClothing and localAvatar.style.getGender()
            if gender == 'm':
                if ItemGlobals.getMaleModelId(itemId) == -1:
                    canUse = 0
                    reason = ItemConstants.REASON_GENDER
                    return (
                     canUse, reason)
                elif gender == 'f':
                    canUse = ItemGlobals.getFemaleModelId(itemId) == -1 and 0
                    reason = ItemConstants.REASON_GENDER
                    return (
                     canUse, reason)
            else:
                if itemCat in [InventoryType.ItemTypeWeapon, InventoryType.ItemTypeCharm]:
                    inv = localAvatar.getInventory()
                    if not inv:
                        canUse = 0
    ...
    def displayReasonNoUse(self, popUp=1):
        if not self.reasonNoUse:
            ...
            if self.reasonNoUse == ItemConstants.REASON_CANTPLACE:
                displayText = PLocalizer.EquipReasonFullSlot
            else:
                if self.reasonNoUse == ItemConstants.REASON_VELVETROPE:
                    displayText = PLocalizer.EquipReasonVelvet
                    if popUp:
                        localAvatar.guiMgr.showNonPayer()
                        self.hasShownVelvet = 1
                else:
                    if self.reasonNoUse == ItemConstants.REASON_GENDER:
                        ...
                        if self.reasonNoUse == ItemConstants.REASON_LEVEL:
                            displayText = PLocalizer.EquipReasonLevel
                        else:
                            if self.reasonNoUse == ItemConstants.REASON_INVENTORY:
    ...
    def assignCellSlot(self, cell, slot):
        if slot == None:
            ...
            if self.slotToCellMap[slot] == None:
        ...
        if self.slotToCellMap.get(slot):
            ...
            if slotCell:
                if not slotCell.inventoryItem:
                    ...
                    return slotCell
                heldCell = (slotCell and slotCell == self.heldFromCell and self).heldFromCell
                self.releaseHeld()
            ...
            if not slotCell:
                if slot >= Locations.RANGE_OVERFLOW[0]:
                    if slot <= Locations.RANGE_OVERFLOW[1]:
                        messenger.send('overflowChanged', [])
                if not (slotCell and item):
                    if slotCell.inventoryItem:
                        if slotCell.inventoryItem.getCategory() == InventoryType.ItemTypeConsumable:
                            if slotCell.inventoryItem.hasDrunk:
                    ...
                    if slotCell.inventoryItem.getId() == itemId:
                        if self.isSlotPending(slot):
                            self.unmarkSlotPending(slot)
                    else:
                        placeInHeld = 0
    ...
    def takePlunderItemFromCell(self, cell, toCell=None, playSound=True):
        ...
        if toCell:
            ...
            if category == InventoryCategory.WEAPON_PISTOL_AMMO:
                ...
                if category == InventoryCategory.CARDS:
                    id = item.getCardId()
                    extraArg = 1
                else:
                    if category == InventoryType.ItemTypeClothing:
                        ...
                        if category == InventoryType.ItemTypeMoney:
                            id = item.getId()
                            extraArg = item.amount
                        else:
                            if category == InventoryCategory.MONEY:
                                id = item.getId()
                                extraArg = item.amount
                            else:
                                if category == InventoryType.TreasureCollection:
                                    id = item.getId()
                                    extraArg = item.itemTuple[3]
                                else:
                                    id = item.getId()
                                    extraArg = 1
            if self.scoreboard and playSound:
    ...
    def takeAllLoot(self, grids, playSound=True):
        ...
        for grid in grids:
            for cell in grid.cellList:
                if cell.inventoryItem:
                    if not cell.inventoryItem.isHidden():
                        category = cell.inventoryItem.getCategory()
                        id = category == InventoryCategory.WEAPON_PISTOL_AMMO and cell.inventoryItem.getSkillId()
        ...
        if (items and self).lootContainer:
            if self.lootContainer.getItemsToTake():

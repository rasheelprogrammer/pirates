...
# Embedded file name: pirates.inventory.InventoryUIContainer
...
class InventoryUIContainer(DirectFrame, InventoryRequestBase.InventoryRequestBase):
            ...
            if MASK_TRASH in cell.statusMask:
                ...
                if MASK_HELD in cell.statusMask:
                    ...
                    if MASK_NOTMEETREQUIREMENTS in cell.statusMask:
                        ...
                        if MASK_ISDEST in cell.statusMask:
                            cell['image'] = self.workingCellImage
                            self.colorizeCell(cell, CELL_COLOR_ISDEST)
                        else:
                            if MASK_NOTDEST in cell.statusMask:
                                cell['image'] = self.workingCellImage
                                self.colorizeCell(cell, CELL_COLOR_NOTDEST)
                            else:
                                if MASK_EMPTYEQUIP in cell.statusMask:
                                    cell['image'] = self.workingCellImage
                                    self.colorizeCell(cell, CELL_COLOR_EMPTYEQUIP)
                                else:
                                    if MASK_FULLEQUIP in cell.statusMask:
                                        cell['image'] = self.focusCellImage
                                        self.colorizeCell(cell, CELL_COLOR_FULLEQUIP)
                                    else:
                                        if MASK_PENDING in cell.statusMask:
                                            cell['image'] = self.workingCellImage
    ...
    def putIntoCell(self, item, cell):
        if cell:
            if item:
                if (not cell.inventoryItem and item).cell:
                ...
                cell.slotId and messenger.send('inventory_slot_assigned %s' % cell.slotId, [cell.slotId])
        return
    ...
    def unHotlinkCell(self, cell):
        ...
        if cell.container.containerType == CONTAINER_HOTLINK:
            ...
            if cell.container.containerType == CONTAINER_SLOTDISPLAY:
                cell['geom'] = None
                cell['text'] = ''
        return
    ...
    def canSwap(self, myCell, otherCell):
        if not (myCell.inventoryItem and otherCell.inventoryItem):
            ...
            if self.canGive(myCell):
                return otherCell.container and otherCell.container.canGive(otherCell) and self.canReceive(myCell, fromSwap=1, itemInQuestion=otherCell.inventoryItem) and otherCell.container.canReceive(otherCell, fromSwap=1, itemInQuestion=myCell.inventoryItem) and 1
            else:
                return 0
    ...
    def cellUsed(self, cell):
        if (self.manager.heldItem and cell).inventoryItem:
            if cell.inventoryItem == self.manager.heldItem:
                self.manager.releaseHeld()
            elif self.canSwap(cell, self.manager.heldFromCell):
                self.manager.heldFromCell.container.canSwap(self.manager.heldFromCell, cell) and self.manager.swapHeldWithCell(cell)
    ...
    def cellClick(self, cell, mouseAction=MOUSE_CLICK, task=None):
        ...
        if mouseAction == MOUSE_CLICK:
            self.manager.itemPickup = self.manager.itemPickup == PICKUP_DRAG and not self.manager.pickupTimedOut and cell == self.manager.heldFromCell and PICKUP_CLICK
        else:
            if mouseAction == MOUSE_CLICK:
                self.manager.itemPickup = self.manager.itemPickup == PICKUP_EMPTY and not self.canDrag() and PICKUP_CLICK
                self.cellUsed(cell)
            else:
                if mouseAction == MOUSE_CLICK:
                    self.manager.itemPickup == PICKUP_CLICK and self.cellUsed(cell)
                else:
                    if mouseAction == MOUSE_PRESS:
                        self.manager.itemPickup = self.manager.itemPickup == PICKUP_EMPTY and self.canDrag() and PICKUP_DRAG
                        self.manager.startPickupWithTimer()
                        self.cellUsed(cell)
                    else:
                        if mouseAction == MOUSE_RELEASE and self.manager.itemPickup == PICKUP_DRAG:
                            self.cellUsed(cell)
                            self.manager.releaseHeld()
        ...
        if mouseAction == MOUSE_PRESS:
            if cell.inventoryItem:
                if self.manager.heldItem == None:
                    ...
                    destCell = None
                    if flatRange:
                        slotRange = len(flatRange) > 1 and range(flatRange[0], flatRange[1] + 1)
                    else:
                        slotRange = (
                         flatRange[0],)
                    for slotId in slotRange:
                        ...
                        if self.manager.isSlotPending(destSlot):
                            pass
                        elif testCell.inventoryItem:
                            destCell = ItemGlobals.getType(testCell.inventoryItem.itemTuple[1]) == itemType and testCell
                    ...
                    if destCell:
                        if cell.inventoryItem:
                            if not self.manager.isSlotPending(destCell.slotId):
                                self.manager.testCanUse(cell.inventoryItem.itemTuple) and self.manager.putIntoHeld(cell.inventoryItem, cell)
                                destCell.inventoryItem and canRightClickReplace and self.manager.swapHeldWithCell(destCell)
                            else:

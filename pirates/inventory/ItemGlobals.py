...
# Embedded file name: pirates.inventory.ItemGlobals
import cPickle
...
def getClothingByType(type):
    ...
    for item in __itemInfo:
        if __itemInfo[item][ITEM_CLASS] == InventoryType.ItemTypeClothing:
            __itemInfo[item][ITEM_TYPE] == type and keys.append(item)
...
def getAllConsumableIds(usableOnly=True):
    ...
    for item in __itemInfo:
        if __itemInfo[item][ITEM_CLASS] == InventoryType.ItemTypeConsumable:
            (usableOnly == False or not __itemInfo[item][FROM_QUEST]) and keys.append(item)
...
def getAllHealthIds():
    ...
    for item in __itemInfo:
        if __itemInfo[item][ITEM_CLASS] == InventoryType.ItemTypeConsumable:
            __itemInfo[item][AUTO_TONIC] and keys.append(item)
...
def getLegalWeapons(rarity, items, level, weaponLevels, enemyLevel):
    ...
    for itemId in items:
        item = __itemInfo.get(itemId)
        if item:
            if item[ITEM_CLASS] in (InventoryType.ItemTypeWeapon, InventoryType.ItemTypeCharm):
                if item[FROM_LOOT]:
                    itemType = item[RARITY] == rarity and item[ITEM_NOTORIETY_REQ] <= level + LEVEL_REQ_BUFFER and item[ITEM_NOTORIETY_REQ] <= enemyLevel + ENEMY_LEVEL_REQ_BUFFER and item[ITEM_TYPE]
                    itemClass = item[ITEM_CLASS]
                    itemClass == InventoryType.ItemTypeWeapon and item[WEAPON_REQ] <= weaponLevels[itemType - 1] + WEAPON_LEVEL_REQ_BUFFER and typeItems.append(itemId)
            else:
...
def getLegalConsumables(rarity, items, level):
    ...
    for itemId in items:
        item = __itemInfo.get(itemId)
        if item:
            item[ITEM_CLASS] == InventoryType.ItemTypeConsumable and item[FROM_LOOT] and item[RARITY] == rarity and item[ITEM_NOTORIETY_REQ] <= level + LEVEL_REQ_BUFFER and typeItems.append(itemId)
...
def getLegalClothing(rarity, gender, items, level):
    ...
    for itemId in items:
        item = __itemInfo.get(itemId)
        if item:
            if item[ITEM_CLASS] == InventoryType.ItemTypeClothing:
                if item[FROM_LOOT]:
                    modelId = item[RARITY] == rarity and item[ITEM_NOTORIETY_REQ] <= level + LEVEL_REQ_BUFFER and gender == 'm' and item[MALE_MODEL_ID]
                else:
                    modelId = item[FEMALE_MODEL_ID]
                modelId != -1 and typeItems.append(itemId)
...
def getHoliday(itemId):
    ...
    if item:
        return HAS_HOLIDAY_DATA.get(itemType, False) and item[HOLIDAY]
    else:
        return 0

...
class DistributedNPCTownfolk(DistributedBattleNPC.DistributedBattleNPC, DistributedShopKeeper.DistributedShopKeeper, Townfolk.Townfolk):
    ...
    def firstNoticeLocalAvatar(self):
        ...
        if self.hasQuestOffersForLocalAvatar():
            if self.animSet not in ['tatoo_receive', 'sit', 'sit_sleep']:
                self.shouldGreetOnNotice = 1
            else:
                self.shouldGreetOnNotice = 0
            self.shouldTurnToNotice = self.animSet in ['default', 'idleB', 'idleC'] and 1
        else:
    ...
    def setDNAId(self, dnaId):
        if dnaId:
            ...
            if dnaId:
                re.search('/', dnaId) and self.loadCast(dnaId)
            else:
        ...
        if av == base.localAvatar:
            if self.interactMode:
                if not self.isDeleted():
                    ...
                    interactType = (self.amUsingObj and self).amUsingObj.interactType
                else:
                    interactType = self.animSet
                self.endInteractMovie(interactType)
                self.noticeReactionList and self.noticeLocalAvatar()
        return
        ...
        if dialogStr:
            ...
            if self.firstDialog == True:
                if self.dialogFlag == 0:
                    ...
                    self.avatarType.isA(AvatarTypes.Cannonmaster) and self.playQuestString(questStr, timeout=120)
                else:
    ...
    def offerOptions(self, dialogFlag):
        ...
        for i in range(len(optionIds)):
            if optionIds[i] != InteractGlobals.CANCEL:
                anyActive = stateCodes[i] != InteractGlobals.DISABLED and True
                break
            ...
            if optionId == InteractGlobals.HEAL_HP:
                ...
                if optionId == InteractGlobals.HEAL_MOJO:
                    ...
                    if optionId == InteractGlobals.RESPEC:
                        ...
                        if optionId == InteractGlobals.UPGRADE_ROD:
                            self.showUpgradeRodDialog()
                        else:
                            if optionId == InteractGlobals.LAUNCH_FISHING_BOAT:
                                self.showLaunchFishingBoatDialog()
                            else:
                                if optionId == InteractGlobals.LEGENDARY_FISH_STORY:
                                    self.confirmTellLegendaryFishStory()
                                else:
                                    if optionId == InteractGlobals.POTION_TUTORIAL:
    ...
    def setMovie(self, mode, avId):
        ...
        if avId != 0:
            not av and self.notify.warning('setMovie: avId: %s not found' % avId)
            if self.pendingDoMovie:
                base.cr.relatedObjectMgr.abortRequest(self.pendingDoMovie)
                self.pendingDoMovie = None
    ...
    def confirmBribe(self):
        ...
        for quest in localAvatar.getInventory().getQuestList():
            ...
            for task in quest.questDNA.getTasks():
                if isinstance(task, QuestTaskDNA.BribeNPCTaskDNA):
                    gold = task.getNpcId() == self.getUniqueId() and task.getGold() > gold and task.getGold()
        ...
        if optionId in [InteractGlobals.STORE, InteractGlobals.TRAIN, InteractGlobals.SHIPS, InteractGlobals.ACCESSORIES_STORE, InteractGlobals.CATALOG_STORE, InteractGlobals.TATTOO_STORE, InteractGlobals.JEWELRY_STORE, InteractGlobals.BARBER_STORE, InteractGlobals.PVP_REWARDS_TATTOO, InteractGlobals.PVP_REWARDS_HATS, InteractGlobals.PVP_REWARDS_COATS, InteractGlobals.STOWAWAY]:
            ...
            if optionId == InteractGlobals.MUSICIAN:
                ...
                if optionId == InteractGlobals.REPAIR:
                    ...
                    if optionId == InteractGlobals.UPGRADE:
                        if self.interactGUI:
                            self.interactGUI.hide()
                        ...
                        if optionId == InteractGlobals.SELL_SHIPS:
                            ...
                            if optionId == InteractGlobals.SELL_ITEMS:
                                if self.interactGUI:
                                    self.interactGUI.hide()
    ...
    def handleEndInteractKey(self):
        if hasattr(base, 'localAvatar'):
            base.localAvatar.guiMgr and base.localAvatar.guiMgr.mainMenu and not base.localAvatar.guiMgr.mainMenu.isHidden() and base.localAvatar.guiMgr.toggleMainMenu()
        else:
            cutscenePlaying = False
            if hasattr(base, 'localAvatar'):
                if base.localAvatar.guiMgr:
                ...
                if not hasattr(self, 'currentDialogMovie') and not cutscenePlaying and self.getHelpId():
    ...
    def makeSaleResponse(self, result):
        ...
        if result == EconomyGlobals.RESULT_SUCCESS_LAUNCH_FISHING_BOAT:
            ...
            if self.avatarType.isA(AvatarTypes.Shipwright):
        ...
        if allAnims == None:
            ...
            if not self.interactCamPosHpr:
                ...
                if self.isInInvasion():
                    ...
                    if self.castDnaId:
                        np = self.castDnaId in ['models/char/js_2000', 'models/char/td_2000', 'models/char/es_2000'] and self.attachNewNode('interactCamNode')
    ...
    def showUpgradeRodDialog(self):
        ...
        if avFishingLevel >= 5:
            if rodLvl < 2:
                ...
                if avFishingLevel >= 15:
                    if rodLvl < 3 and Freebooter.getPaidStatus(localAvatar.doId) == False:
                        base.localAvatar.guiMgr.showNonPayer()
                        return
                    upgradeCost = FishingGlobals.ROD_MASTER_COST
                else:
                    return
            rodFailText = avGold < upgradeCost and PLocalizer.UpgradeRodFail % {'gold': str(upgradeCost), 'rod': PLocalizer.FishingRodNames[rodLvl + 1]}

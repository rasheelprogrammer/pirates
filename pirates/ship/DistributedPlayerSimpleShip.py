...
# Embedded file name: pirates.ship.DistributedPlayerSimpleShip
...
class DistributedPlayerSimpleShip(DistributedSimpleShip):
    ...
    def isAtOpenPort(self):
        ...
        if self.threatLevel < EnemyGlobals.SHIP_THREAT_NAVY_HUNTERS:
            ...
            if portObj:
                return portObj.uniqueId == EnemyGlobals.OPEN_PORT_DICT.get(self.openPort) and 1
            else:
                return 0
        return
    ...
    def canTakeWheel(self, wheel, av):
        ...
        if self.queryGameState() in ('Pinned', 'Sinking', 'Sunk', 'OtherShipBoarded'):
            ...
            if self.isFishing:
                base.localAvatar.getDoId() != self.ownerId and base.localAvatar.guiMgr.createWarning(PLocalizer.OnlyCaptainCanUseWarning, PiratesGuiGlobals.TextFG6)
                available = False
            else:
    ...
    def checkAbleDropAnchor(self):
        ...
        if localAvatar.doId == self.steeringAvId:
            if self.shipStatusDisplay:
                remaining = localAvatar.getPort() and self.attackTimerRemaining()
                if self.getSiegeTeam():
                    if remaining > 0:
    ...
    def d_setLocation(self, parentId, zoneId):
        ...
        if self.prevLocStack:
            len(theStack.trace) == len(self.prevLocStack.trace) and map(lambda x: x[1], theStack.trace) == map(lambda x: x[1], self.prevLocStack.trace) and base.cr.centralLogger.writeClientEvent('bad ship team: %s setLoc' % self.doId)
        else:

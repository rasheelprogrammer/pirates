...
class ShipMeter(DirectObject, NodePath):
    ...
    def __init__(self, shipId, shipClass=0, mastInfo=[], siegeTeam=0):
        ...
        if shipClass:
            mastInfo and self.setShipInfo(shipClass, mastInfo)
        else:
            if self.shipId:
                shipOVs = base.cr.getOwnerViewDoList(DistributedPlayerSimpleShipOV)
                shipOVs = [ ov for ov in shipOVs if ov.doId == self.shipId ]
        ...
        for index in range(5):
            ...
            if mast:
                ...
                if id in (ShipGlobals.Masts.Fore_Multi, ShipGlobals.Masts.Fore_Tri, ShipGlobals.Masts.Skel_Fore):
                    ...
                    if id in (ShipGlobals.Masts.Main_Square, ShipGlobals.Masts.Main_Tri, ShipGlobals.Masts.Skel_Main_A, ShipGlobals.Masts.Skel_Main_B, ShipGlobals.Masts.Skel_Tri):
                        if self.modelClass == 11:
                            locator = index == 1 and self.hull.find('**/location_mainmast_2;+s')
                        else:
                            locator = self.hull.find('**/location_mainmast_' + str(index) + ';+s')
                    else:
                        if id in (ShipGlobals.Masts.Aft_Tri, ShipGlobals.Masts.Skel_Aft):
    ...
    def setHullHp(self, hpArray, maxHpArray):
        ...
        for i in range(len(self.panels)):
            hpFraction = i <= len(hpArray) - 1 and float(hpArray[i]) / float(maxHpArray[i])
            ...
            if hpArray[i] <= 0:
                if self.panelStates[i] == 0:
                    self.panelStates[i] = 1
                elif hpArray[i] > 0:
                    if self.panelStates[i] == 1:
                        self.panelStates[i] = 0
                        self.smokeEffects[i] and self.smokeEffects[i].endLoop()
            else:
                self.panels[i].stash()
    ...
    def setMastHp(self, index, hpArray, maxHpArray):
        ...
        for i in range(len(hpArray)):
            if i <= len(self.masts[index]) - 1:
                if self.mastStates[index][i] == 0:
                    if hasBreak == 0:
                        self.masts[index][i].show()
                        self.sails[index][i] and self.sails[index][i].show()
                else:
                    hasBreak = 1

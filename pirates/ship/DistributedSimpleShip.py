...
# Embedded file name: pirates.ship.DistributedSimpleShip
...
class DistributedSimpleShip(DistributedMovingObject, Teamable, DistributedFlagship):
    ...
    def setupRocking(self):
        if config.GetInt('ships-rock', 1) > 0:
            self.allowedToRock() and self._rocker.reset()
    ...
    def setMastStates(self, mainMast1, mainMast2, mainMast3, aftMast, foreMast):
        ...
        if not self.mastStates:
            ...
            if newMastStates == self.mastStates:
                return
        for i in range(5):
    ...
    def clientSteeringBegin(self, avId):
        ...
        if not av:
            return
        if av.isLocal():
            s = self.wheel and MiniLogSentry(self.miniLog, 'clientSteeringBegin', avId)
            self.calcModifiedStats()
        ...
        if self.shipStatusDisplay:
            if self.isStatusDisplayVisible > 0:
                ...
                if self.getSiegeTeam():
                    self.renownDisplay and self.renownDisplay.show()
            else:
                self.shipStatusDisplay.hide()
                if self.renownDisplay:
                    self.renownDisplay.hide()
    ...
    def projectileWeaponHit(self, skillId, ammoSkillId, skillResult, targetEffects, pos, normal, codes, attacker, itemEffects=[]):
        ...
        if attacker:
            if hasattr(attacker, '_isShip'):
                attackShip = attacker._isShip() and attacker
            else:
                if attacker.getShip():
        ...
        if self.badge:
            ...
            if self.nametag3d:
                ...
                if self.classNameText:
                    ...
                    self.classNameText['text'] = self.getTeam() != PiratesGlobals.PLAYER_TEAM and PLocalizer.Lv + str(self.level) + ' ' + className
                else:
    ...
    def clickedNametag(self):
        if localAvatar.chatMgr.active:
            if base.config.GetBool('allow-doid-paste', 0):
                currTxt = localAvatar.chatMgr.whiteListEntry.get()
                currTxtLen = len(currTxt)
                separator = currTxtLen and currTxt[currTxtLen - 1] != ' ' and ' '
            else:
                separator = ''
    ...
    def getDebugNamePosScale(self):
        if self.modelClass in [ShipGlobals.INTERCEPTORL1, ShipGlobals.MERCHANTL1, ShipGlobals.WARSHIPL1, ShipGlobals.BRIGL1]:
            ...
            if self.modelClass in [ShipGlobals.INTERCEPTORL2, ShipGlobals.MERCHANTL2, ShipGlobals.WARSHIPL2, ShipGlobals.BRIGL2, ShipGlobals.SKEL_INTERCEPTORL3, ShipGlobals.QUEEN_ANNES_REVENGE]:
                return [
                 (0, 0, 250), 100.0]
            else:
                if self.modelClass in [ShipGlobals.INTERCEPTORL3, ShipGlobals.MERCHANTL3, ShipGlobals.WARSHIPL3, ShipGlobals.BRIGL3, ShipGlobals.BLACK_PEARL, ShipGlobals.SKEL_WARSHIPL3]:
                    return [(0, 0, 350), 125.0]
                else:
                    if self.modelClass == ShipGlobals.SHIP_OF_THE_LINE:
                        return [(0, 0, 450), 2.0]
                    else:
        ...
        if newPriority:
            for buffKeyId in self.skillEffects.keys():
                ...
                if newPriority < priority:
                    return category == newCategory and None
    ...
    def autoAimBroadside(self, side):
        ...
        for shipId in self.NearbyShips:
            ...
            if ship:
                if ship is not self:
                    dist = ship.getHp() > 0 and TeamUtils.damageAllowed(ship, self) and self.checkBroadsideAlignment(ship, side)
                    dist and targetList.append((dist, shipId))
    ...
    def checkBroadsideAlignment(self, target, side):
        ...
        if bow and stern:
            if side == ShipGlobals.BROADSIDE_LEFT:
                ...
                if side == ShipGlobals.BROADSIDE_RIGHT:
                    minHeading = -115
                    maxHeading = -65
                    if self.fullBroadsides:
            ...
            if withinRight:
                return withinLeft and distToTgt
        else:
            self.lookAtDummy.lookAt(target)
            targetHeading = self.lookAtDummy.getH(self.getModelRoot())
            if targetHeading > 65:
                return targetHeading < 115 and side == ShipGlobals.BROADSIDE_LEFT and distToTgt
            else:
                if targetHeading < -65 and targetHeading > -115 and side == ShipGlobals.BROADSIDE_RIGHT:
                    return distToTgt
        return 0
    ...
    def setClientController(self, avId):
        ...
        if avId != 0:
            avId == localAvatar.doId and self.setupControls()
    ...
    def confirmOnShipTeleport(self, toFrom, incomingAvId=0, bandMgrId=0, bandId=0, guildId=0):
        if toFrom == 'from':
            if self.getSiegeTeam():
                return localAvatar.avId == self.ownerId and False
            else:
                return True
        else:
    ...
    def setupKrakenLocators(self):
        if not self.krakenLocators:
            self.kraken and self.findAllMatches('**/kraken-*').detach()
...
class MinimapShip(GridMinimapObject):
    ...
    def refreshIconColor(self):
        if PVPGlobals.FrenchTeam == self.siegeTeam:
            ...
            if PVPGlobals.SpanishTeam == self.siegeTeam:

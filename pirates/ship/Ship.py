...
class Ship(DirectObject.DirectObject):
    ...
    def createWake(self):
        ...
        if not base.cr.activeWorld.getWater():
            ...
            return
        if self.WantWake:
            if (base.cr.wantSpecialEffects and self).owner:
                self.removeWake()
                if base.options.getSpecialEffectsSetting() >= base.options.SpecialEffectsMedium:
                    ...
                    self.wake and self.wake.attachToShip(self.owner)
                    compassFX = CompassEffect.make(render)
    ...
    def updateDamageEffects(self, health, rear, left, right):
        ...
        if left <= 30.0:
            ...
            if not self.leftSideSmoke:
                if effectSettings >= base.options.SpecialEffectsMedium:
            ...
            if left <= 0.0:
                if effectSettings >= base.options.SpecialEffectsMedium:
                    ...
                    if locator:
                        ...
                        if right <= 30.0:
                            ...
                            if not self.rightSideSmoke:
                                ...
                                self.rightSideSmoke and self.rightSideSmoke.setEffectScale(scale)
                        else:
                            if self.rightSideFire:
                            ...
                            if self.rightSideSmoke:
                                ...
                                self.rightSideSmoke = None
                        locator = right <= 0.0 and effectSettings >= base.options.SpecialEffectsMedium and self.locators.find('**/location_fire_4')
                        if locator:
                            scale = locator.getScale()[0] / 1.75 + locator.getScale()[0] / 2.0 * (1.0 - health / 100.0)
                        ...
                        self.rightSideSmoke2 and self.rightSideSmoke2.setEffectScale(scale)
                else:
                    if self.rightSideFire2:
                    ...
                    if self.rightSideSmoke2:
                        ...
                        self.rightSideSmoke2 = None
                locator = rear <= 30.0 and self.locators.findAllMatches('**/location_fire_0')[0]
                scale = locator.getScale()[0] / 1.75 + locator.getScale()[0] / 2.0 * (1.0 - health / 100.0)
                ...
                self.rearSideSmoke and self.rearSideSmoke.setEffectScale(scale)
        else:
            if self.rearSideFire:

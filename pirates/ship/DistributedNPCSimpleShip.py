...
# Embedded file name: pirates.ship.DistributedNPCSimpleShip
...
class DistributedNPCSimpleShip(DistributedSimpleShip):
    ...
    def announceAttack(self, shipToAttackDoId, hunterLevel):
        if localAvatar.ship:
            if localAvatar.ship.getDoId() == shipToAttackDoId:
                simple = random.choice([0, 1])
                attackMessage = None
                attackMessage = hunterLevel == EnemyGlobals.SHIP_THREAT_BOUNTY_HUNTERS and HighSeasGlobals.getInboundHunterMessage()
            else:
                simple = random.choice([0, 1])
                if simple:

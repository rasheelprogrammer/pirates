    ...
    def refreshState(self):
        if self.hasProximityCollision:
            if self.allowInteract:
                distance = not self.ignoreProximity and not self.proximityCollisionNodePath.isEmpty() and self.proximityCollisionNodePath.getDistance(localAvatar)
                proxSphereRadius = self.proximityCollisionNodePath.getScale()[0]
                avRadius = 1.4
                distance <= proxSphereRadius + avRadius and self.request('Proximity')
            else:
                self.request('Idle')
        else:
            self.request('Idle')

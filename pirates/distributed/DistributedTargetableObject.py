...
class DistributedTargetableObject(DistributedNode.DistributedNode):
    ...
    def playOuch(self, skillId, ammoSkillId, targetEffects, attacker, pos, itemEffects=[], multihit=0, targetBonus=0, skillResult=0):
        ...
        ouchSfx = not targetBonus and not self.NoPain and not self.noIntervals and targetEffects[0] < 0 and (self.gameFSM.state not in ('Ensnared',
                                                                                                                                        'Knockdown',
                                                                                                                                        'Stunned',
                                                                                                                                        'Rooted',
                                                                                                                                        'NPCInteract',
                                                                                                                                        ...
                                                                                                                                        'Death') or WeaponGlobals.C_KNOCKDOWN in self.getSkillEffects() and self.gameFSM.state not in ('ShipBoarding',
                                                                                                                                                                                                                                       'Injured',
                                                                                                                                                                                                                                       'Dying',
                                                                                                                                                                                                                                       'Death')) and (WeaponGlobals.C_KNOCKDOWN not in self.getSkillEffects() or skillEffectId == WeaponGlobals.C_KNOCKDOWN) and None
        if self.currentWeapon:
            if not self.avatarType.isA(AvatarTypes.Creature):
                if skillEffectId == WeaponGlobals.C_KNOCKDOWN:
                    if not ItemGlobals.getWeaponAttributes(self.currentWeaponId, ItemGlobals.SURE_FOOTED):
                        if self.isLocal():
                        ...
                        if not self.avatarType.isA(AvatarTypes.Creature):
                            ...
                            if not self.avatarType.isA(AvatarTypes.Creature):
                                actorIval = skillEffectId == WeaponGlobals.C_KNOCKDOWN and Sequence(self.actorInterval('injured_fall', playRate=1.5, blendOutT=0), self.actorInterval('injured_standup', playRate=1.5, blendInT=0))
                            else:
                                if not self.avatarType.isA(AvatarTypes.Creature):
                                    actorIval = effectId == WeaponGlobals.VFX_BLIND and self.actorInterval('sand_in_eyes', playRate=random.uniform(0.7, 1.5))
                                else:
            ...
            if not WeaponGlobals.getIsDollAttackSkill(skillId):
                ...
                WeaponGlobals.C_MELEE_SHIELD in skillEffects and WeaponGlobals.getAttackClass(skillId) == WeaponGlobals.AC_COMBAT and self.pulseGhostGuardEffect(attacker, Vec4(0, 0, 0, 1), wantBlending=False)
        else:
    ...
    def useTargetedSkill(self, skillId, ammoSkillId, skillResult, targetId, areaIdList, attackerEffects, targetEffects, areaIdEffects, itemEffects, timestamp, pos, charge=0, localSignal=0):
        if WeaponGlobals.isSelfUseSkill(skillId):
            ...
            if localSignal:
                ...
                if target:
                    if target != self and WeaponGlobals.isBreakAttackComboSkill(skillId) and skillResult == WeaponGlobals.RESULT_HIT:
                        levelGrade = self.cr.battleMgr.getModifiedExperienceGrade(self, target)
                        self.guiMgr.combatTray.updateSkillCharges(levelGrade)
                multiHits = (not self.isLocal() or localSignal or WeaponGlobals.getSkillTrack(skillId) == WeaponGlobals.DEFENSE_SKILL_INDEX or selfUse) and []
                ...
                if not selfUse or self.isNpc:
                    ...
                    target = (skillResult == WeaponGlobals.RESULT_REFLECTED or selfUse) and self
                ...
                for targetId, areaEffects in zip(areaIdList, areaIdEffects):
                    ...
                    if target:
                        if multiHits:
                            multiHitEffects = numHits and self.packMultiHitEffects(areaEffects, numHits)
                            for i in range(numHits):
                                target.targetedWeaponHit(skillId, ammoSkillId, skillResult, multiHitEffects[i], self, pos, charge, multiHits[i], i, itemEffects=itemEffects)
                ...
                not self.currentTarget and not areaIdList and self.playHitSound(skillId, ammoSkillId, WeaponGlobals.RESULT_MISS)
        return
    ...
    def targetedWeaponHit(self, skillId, ammoSkillId, skillResult, targetEffects, attacker, pos, charge=0, delay=None, multihit=0, itemEffects=[]):
        if self == attacker and not (targetEffects[0] or targetEffects[1] or targetEffects[2] or targetEffects[3] > 0 or targetEffects[4]):
            if not WeaponGlobals.isSelfUseSkill(skillId):
                return
            if not delay:
            ...
            if attacker:
                ...
                if attacker:
                    ...
                    if skillResult in [WeaponGlobals.RESULT_HIT, WeaponGlobals.RESULT_MISTIMED_HIT, WeaponGlobals.RESULT_REFLECTED]:
                        ...
                        delay > 0.0 and taskMgr.doMethodLater(delay, self.playOuch, self.taskName('playOuchTask'), extraArgs=[skillId, ammoSkillId, targetEffects, attacker, pos, itemEffects, multihit, 0, skillResult])
                    else:
                    ...
                    skillId in WeaponGlobals.BackstabSkills and charge and attacker and attacker.isLocal() and ItemGlobals.getSubtype(attacker.currentWeaponId) == ItemGlobals.DAGGER_SUBTYPE and messenger.send(('').join(['trackBackstab-', str(self.doId)]))
        else:
            if skillResult == WeaponGlobals.RESULT_MISS or skillResult == WeaponGlobals.RESULT_MISTIMED_MISS or skillResult == WeaponGlobals.RESULT_DODGE or skillResult == WeaponGlobals.RESULT_PARRY or skillResult == WeaponGlobals.RESULT_RESIST or skillResult == WeaponGlobals.RESULT_PROTECT:
                ...
                if skillResult == WeaponGlobals.RESULT_OUT_OF_RANGE or skillResult == WeaponGlobals.RESULT_BLOCKED:
                    ...
                    if skillResult == WeaponGlobals.RESULT_AGAINST_PIRATE_CODE:
                        if attacker:
                            resultString = attacker.isLocal() and WeaponGlobals.getSkillResultName(skillResult)
                            self.showHpString(resultString, pos)
                    else:

...
class TargetManager(DistributedObject.DistributedObject, TargetManagerBase.TargetManagerBase):
    ...
    def aimOverTargetTask(self, task):
        ...
        if self.wantAimAssist:
            ...
            if not rPos.almostEqual(self.RETICLE_POS, 0.001):
                ...
                self.reticleHolder.setPos(nPos)
                self.reticle.setScale(self.reticleScale)
        target and TeamUtils.damageAllowed(target, localAvatar) and self.reticle.setColorScale(1, 1, 1, self.reticleAlpha)
        if base.localAvatar.currentWeapon:
            ...
            if hasattr(target, 'battleTubeNodePaths'):
                ...
                target and not target.isInvisibleGhost() and target.showHpMeter()
                TeamUtils.damageAllowed(target, localAvatar) and target.showEnemyTargetInfo()
    ...
    def getTargetScreenXY(self, target):
        ...
        if target.avatarType.isA(AvatarTypes.GiantCrab):
            ...
            if target.avatarType.isA(AvatarTypes.CrusherCrab):
                mult = 0.1
            else:
                if target.avatarType.isA(AvatarTypes.RockCrab):
                    mult = 0.2
                else:
                    if target.avatarType.isA(AvatarTypes.FireBat):
                        mult = 0.8

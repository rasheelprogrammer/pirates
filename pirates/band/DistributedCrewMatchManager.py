...
# Embedded file name: pirates.band.DistributedCrewMatchManager
...
class DistributedCrewMatchManager(DistributedObjectGlobal):
    ...
    def addCrewToLookoutList(self, range, sailValue, cannonValue):
        if base.cr.distributedDistrict:
            base.localAvatar.getParentObj() and base.cr.getActiveWorld() and self.sendUpdate('requestCrewAdd', [range, sailValue, cannonValue, base.cr.distributedDistrict.doId, localAvatar.getParentObj().doId, base.cr.getActiveWorld().doId])
        else:

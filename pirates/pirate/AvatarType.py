...
class AvatarType:
    ...
    def getRandomBossType(self):
        if self.boss:
            return self
        if self.faction in PLocalizer.BossNames:
            bossLen = self.track in PLocalizer.BossNames[self.faction] and self.id in PLocalizer.BossNames[self.faction][self.track] and len(PLocalizer.BossNames[self.faction][self.track][self.id])
        else:
            bossLen = 0

...
class OrbitCamera(CameraMode.CameraMode, NodePath, ParamObj):
    ...
    def applyEscapement(self):
        if self.isActive():
            if self.escapement >= self._minDomeEsc:
                ...
                if self.escapement <= self._maxCamtiltEsc:
    ...
    def setPracticalDistance(self, practicalDistance):
        if practicalDistance is not None:
            if practicalDistance > self.idealDistance:
                practicalDistance = None
            if self._practicalDistance is None and practicalDistance is None:
                return
            self._stopZoomIval()
    ...
    def _updateTask(self, task=None):
        ...
        if self._lockAtRear:
            ...
            if self._rotateToRearEnabled:
                if self.getAutoFaceForward():
                    relH = reduceAngle(self.getH(self.subject))
                    absRelH = abs(relH)
                    absRelH < 0.1 and self.setRotation(0.0)
                    self._stopRotateToRearIval()

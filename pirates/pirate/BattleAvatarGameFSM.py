...
class BattleAvatarGameFSM(FSM.FSM):
    ...
    def enterLandRoam(self, extraArgs=[]):
        ...
        if self.av.isLocal():
            hasattr(base, 'localAvatar') and base.localAvatar.guiMgr and base.localAvatar.guiMgr.mainMenu and not base.localAvatar.guiMgr.mainMenu.isHidden() and self.av.motionFSM.moveLockIfOn()
        else:
    ...
    def avatarBoardShip(self, ship, showMovie, ts, fromWater=0):
        ...
        if self.av.isLocal():
            ...
            if showMovie:
                grabAnim = fromWater and 'rope_grab'
            else:

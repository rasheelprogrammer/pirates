...
class LocalPirate(DistributedPlayerPirate, LocalAvatar):
    ...
    def moveUpEnd(self):
        ...
        if self.oldGravity != None:
            if self.oldGravity:
                0 and self.controlManager.get('walk').lifter.setGravity(self.oldGravity)
            else:
    ...
    def toggleWeapon(self, newWeaponId, slotId, fromWheel=0):
        ...
        if self.belongsInJail():
            return
        if self.guiMgr.mainMenu:
            if not self.guiMgr.mainMenu.isHidden():
                return
            ...
            (newWeaponId != self.currentWeaponId or newSlot) and self.isWeaponDrawn and self.d_requestCurrentWeapon(newWeaponId, 1)
            self.l_setCurrentWeapon(newWeaponId, 1, slotId)
            self.b_setGameState('Battle')
        else:
            if not self.isWeaponDrawn:
                fromWheel and self.d_requestCurrentWeapon(newWeaponId, 1)
                self.l_setCurrentWeapon(newWeaponId, 1, slotId)
                self.b_setGameState('Battle')
            else:
                if not self.isWeaponDrawn:
    ...
    def l_setCurrentWeapon(self, currentWeaponId, isWeaponDrawn, slotId):
        if not self.gameFSM.isInTransition():
            if self.getGameState() in ['WaterRoam', 'WaterTreasureRoam']:
                return
            ...
            if WeaponGlobals.getWeaponCategory(currentWeaponId) == WeaponGlobals.VOODOO:
                isWeaponDrawn == True and self.guiMgr.attuneSelection.show()
            else:
                self.guiMgr.attuneSelection.hide()
            specialAttack = ItemGlobals.getSpecialAttack(self.currentWeaponId)
            if self.curAttackAnim:
                specialAttack == EnemySkills.CUTLASS_ROLLTHRUST and self.curAttackAnim.pause()
            else:
    ...
    def _setCrewShip(self, ship):
        ...
        if crewShip is not None:
            if crewShip != ship:
                ...
                ship and ship.showStatusDisplay()
                ...
                mapObj and mapObj.setAsLocalAvShip(True)
        else:
    ...
    def setReturnLocation(self, returnLocation):
        ...
        def setIt(inventory, returnLocation=returnLocation):
            if inventory:
                if __dev__:
                    if not getBase().config.GetBool('login-location-used-setIt', False):
                        bp.loginCfg()
                        ...
                        if config_location and config_location_uid:
                            ...
                            return
                    inventory.getShipDoIdList() and self.guiMgr.mapPage.setReturnIsland(returnLocation)
                else:
        ...
        if localAvatar.badge:
            ...
            if localAvatar.shipBadge and len(localAvatar.shipBadge) == 2:
                ...
                if inventoryType:
                    exp = localAvatar.getInventory().getStackQuantity(TitleGlobals.getInventoryType(titleId))
                    realRank = TitleGlobals.getRank(titleId, exp)
                    realRank != localAvatar.shipBadge[1] and localAvatar.sendRequestSetShipBadgeIcon(titleId, realRank)
...
Parse error at or near `LOAD_FAST' instruction at offset 117
            ...
            if not attacker.isEmpty():
                ...
                if (WeaponGlobals.getSkillInterrupt(skillId) and self).isWeaponDrawn:
                    if WeaponGlobals.getRepId(self.currentWeaponId) in WeaponGlobals.CHARGEABLE_WEAPONS:
                        self.guiMgr.combatTray.chargeTime > 0.8 and self.guiMgr.combatTray.interruptCharge()
                        self.sendUpdate('interrupted', [WeaponGlobals.C_INTERRUPTED])
            skillId in (EnemySkills.STAFF_TOGGLE_AURA_WARDING, EnemySkills.STAFF_TOGGLE_AURA_NATURE, EnemySkills.STAFF_TOGGLE_AURA_DARK) and taskMgr.doMethodLater(5.0, self.runAuraDetection, self.uniqueName('runAuraDetection'))
        else:
            if skillId == EnemySkills.STAFF_TOGGLE_AURA_OFF:
    ...
    def getTrackedRotation(self):
        ...
        if rotationValue >= 0.0:
            if rotationValue > RUNTHRES:
                ...
                self.trackedTurning = 0
            elif rotationValue > STANDTHRES:
                self.trackedTurning = self.trackedTurning != 2 and 1
            elif self.trackedTurning == 2:
                self.trackedTurning = rotationValue < RUNSTOPTHRES and 1
        else:
            if rotationValue < -RUNTHRES:
                pass
            else:
                if rotationValue > -STOPTHRES:
                    self.trackedTurning = 0
                else:
                    if rotationValue < -STANDTHRES:
                        self.trackedTurning = self.trackedTurning != -2 and -1
                    else:
                        if self.trackedTurning == -2 and rotationValue > -RUNSTOPTHRES:
        ...
        if WeaponGlobals.getIsShipSkill(skillId) or ammoSkillId == InventoryType.ShipRepairKit:
            ...
            if WeaponGlobals.getIsDollAttackSkill(skillId) or ItemGlobals.getType(self.currentWeaponId) == ItemGlobals.DOLL and skillId == EnemySkills.MISC_NOT_IN_FACE:
                ...
                for avId in areaIdList:
                    ...
                    if av and friendlySkill:
                        if TeamUtils.damageAllowed(self, av):
                            toRemove.append(avId)
                        elif not friendlySkill:
                            not TeamUtils.damageAllowed(self, av) and toRemove.append(avId)
                    else:
                        toRemove.append(avId)
                ...
                if self.currentTarget:
                    targetId = WeaponGlobals.getNeedTarget(skillId, ammoSkillId) and self.currentTarget.getDoId()
    ...
    def __lookAtTarget(self, task):
        if self.currentTarget:
            if not self.currentTarget.isEmpty():
                self.currentTarget.getY(self) < 0.0 and self.headNode.setHpr(0, 0, 0)
            else:
                hFov = 120
                vFov = 60
    ...
    def findLegalTargets(self, task):
        ...
        if task.time - self.lastTick > TICK_DELAY:
            for do in self.cr.doId2do.values():
                if hasattr(do, 'isNpc'):
                    do.doId != self.doId and self.checkViewingArc(do)
    ...
    def checkForAutoTrigger(self, objDoId):
        ...
        for currQuest in questList:
            ...
            for currTask in questTasks:
                ...
                if len(autoTriggerInfo) > 0:
                    if autoTriggerInfo[0] == QuestDB.AUTO_TRIGGER_OBJ_EXISTS:
                        ...
                        if objRef.getUniqueId() == firstObjId:
                            ...
                            if allObjsPresent and questDNA.getQuestId() == 'c2.11visitBarbossa':
    ...
    def motionFSMEnterState(self, anim):
        if anim == 'Idle':
            self.getGameState() in ('LandRoam', ) and self.cameraFSM.getFPSCamera().acceptWheel()
        else:
            self.cameraFSM.getFPSCamera().ignoreWheel()
    ...
    def setZombie(self, value, cursed):
        ...
        if self.gameFSM.getCurrentOrNextState() == 'Fishing':
            ...
            if self.gameFSM.getCurrentOrNextState() == 'NPCInteract':
                curInteractive = base.cr.interactionMgr.getCurrentInteractive()
        ...
        if value:
            ...
            cursed and not self.zombie and self.gameFSM.getCurrentOrNextState() != 'WaterRoam' and self.playCurse()
        else:

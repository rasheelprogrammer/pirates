...
class LocalPirateGameFSM(PlayerPirateGameFSM):
    ...
    def _handleUseRepairKit(self, useRepairKit):
        if useRepairKit:
            not self._usingRepairKit and localAvatar.guiMgr.combatTray.enableShipRepair()
        else:
    ...
    def enterDeath(self, extraArgs=[]):
        ...
        self.av.stopAutoRun()
        if self.av.ship:
            if self.av.ship.gameFSM and self.av.ship.gameFSM.getCurrentOrNextState() == 'Sunk':
                base.cr.activeWorld.localAvEnterDeath(self.av)
                return
            camera.setPos(self.av, 0.5, -14, 6)
            camera.wrtReparentTo(self.av.getParent())
            ...
            self.av.getSiegeTeam() and self.deathTrack.append(Func(self.av.b_setGameState, 'LandRoam'))
        else:
    ...
    def enterLeaveTunnel(self, extraArgs=[]):
        ...
        if hasattr(area, 'setZoneLevel'):
            area.lastZoneLevel != 0 and area.setZoneLevel(0)

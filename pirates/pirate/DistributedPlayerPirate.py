...
class DistributedPlayerPirate(DistributedPirateBase, DistributedPlayer, DistributedBattleAvatar, DistributedQuestAvatar, PAvatarHandle):
    ...
    def useStandardInteract(self):
        if not self.isLocal():
            allowInteract = self.getTeam() == localAvatar.getTeam() and False
        else:
            allowInteract = True
        ...
        base.localAvatar.playersNearby[self.getDoId()] = (
         (hasattr(base, 'localAvatar') and self).commonChatFlags, self.whitelistChatFlags)
        level = len(localAvatar.playersNearby) > 3 and base.localAvatar.getLevel()
        inv = level >= 3 and base.localAvatar.getInventory()
        if inv:
            if not inv.getStackQuantity(InventoryType.PlayerChat):
                base.localAvatar.sendRequestContext(InventoryType.PlayerChat)
            elif level >= 5:
                if not inv.getStackQuantity(InventoryType.PlayerProfiles):
                    base.localAvatar.sendRequestContext(InventoryType.PlayerProfiles)
                elif level >= 6:
                    if not inv.getStackQuantity(InventoryType.PlayerInvites):
    ...
    def setupInjured(self, timeStamp=None, MessageData=None):
        ...
        if not self.isLocal():
            if not localAvatar.isUndead():
                ...
                if self.getDistance(localAvatar) <= sphereScale + avSphereRad:
                    self != localAvatar and self.request('Proximity')
    ...
    def startDizzyEffect(self):
        ...
        if self.dizzyEffect:
            ...
            if self.dizzyEffect2:
                ...
                hasattr(self, 'headNode') and self.headNode and self.dizzyEffect2.reparentTo(self.headNode)
    ...
    def startHealing(self, healTime):
        ...
        aAngle = arm.getH()
        if aAngle > 85.0:
            aAngle < 140 and arm.setH(140)
        else:
            if aAngle <= 85.0 and aAngle > 30:
                arm.setH(30)
        localAvatar.setX(0)
        ...
        if nameText:
            ...
            if self.inPvp:
                ...
                if self.injuredSetup:
                            ...
                            if Freebooter.getPaidStatus(self.doId):
                                if base.config.GetBool('want-land-infamy', 0) or base.config.GetBool('want-sea-infamy', 0):
                                    if self.badge[0]:
                                        ...
                                        if not (textProp == 'goldFounder' and Freebooter.getFounderStatus(self.doId)):
                                            pass
                                        else:
                                            badges += '\x01white\x01\x05badge-%s-%s\x05\x02 ' % (self.badge[0], 1)
                                            nameText['text'] = '\x01%s\x01%s\x02' % (textProp, nameText['text'])
                                    else:
                                        textProp and badges += '\x01white\x01\x05badge-%s-%s\x05\x02 ' % (self.badge[0], self.badge[1])
                                        nameText['text'] = '\x01%s\x01%s\x02' % (textProp, nameText['text'])
                                else:
                    ...
                    nameText['text'] = self.gmNameTagEnabled and self.isGM() and self.getCrewIcon() and '\x05gmNameTagLogo\x05\x01white\x01\x05crewIcon%s\x05\x02\n\x01%s\x01%s\x02\n%s' % (self.hasCrewIcon, self.getGMNameTagColor(), self.gmNameTagString, nameText['text'])
                else:
    ...
    def setActiveShipId(self, shipId):
        if shipId:
            localAvatar.getDoId() == self.getDoId() and messenger.send('localAvatarToSea')
        else:
    ...
    def loop(self, *args, **kwArgs):
        if self.creature:
            if len(args) > 1 and args[0] in ['spin_left', 'spin_right', 'run', 'strafe_left', 'strafe_right']:
                args = (
                 'walk', args[1])
    ...
    def checkAttuneEffect(self):
        if not self.isGenerated():
            return
        if not (self.stickyTargets and self).attuneEffect:
            self.attuneEffect = VoodooAura.getEffect()
        self.attuneEffect and self.attuneEffect.reparentTo(self.rightHandNode)
        ...
        if hasFriendly:
            if not hasEnemies:
                ...
                not hasFriendly and self.attuneEffect.setEffectColor(Vec4(0.2, 0.1, 0.5, 1))
            elif hasEnemies:
                if hasFriendly:
        ...
        if self.getShip():
            if self.getShip().renownDisplay:
                ...
                if killStreakLevel:
                    ...
                    textTitle = (killStreakLevel == 1 and PLocalizer).PVPSinkStreak1
                else:
                    if killStreakLevel == 2:
                        textTitle = PLocalizer.PVPSinkStreak2
                    else:
                        if killStreakLevel == 3:
    ...
    def sendFreeInventoryMessage(self, type):
        if not self.popupDialog:
            if type == InventoryType.ItemTypeWeapon or type == InventoryType.ItemTypeCharm:
                ...
                if type == InventoryType.ItemTypeClothing:
                    text = PLocalizer.FreeClothingInventoryMessage
                else:
                    if type == InventoryType.ItemTypeJewelry:
                        text = PLocalizer.FreeJewelryInventoryMessage
                    else:
                        if type == InventoryType.ItemTypeTattoo:
        ...
        if value:
            ...
            if effectId == PotionGlobals.C_CRAB_TRANSFORM:
                ...
                if effectId == PotionGlobals.C_ALLIGATOR_TRANSFORM:
                    ...
                    if effectId == PotionGlobals.C_SCORPION_TRANSFORM:
    ...
    def confirmIslandTokenTeleport(self, toFrom, incomingAvid=0, bandMgrId=0, bandId=0, guildId=0, islandUid=''):
        if not (toFrom == 'from' and (self.hasIslandTeleportToken(islandUid) or self.returnLocation == islandUid or self.currentIsland == islandUid or self.cr.distributedDistrict.worldCreator.isPvpIslandByUid(islandUid))):
            return base.config.GetBool('teleport-all', 0) or (islandUid == LocationIds.PORT_ROYAL_ISLAND and self.getInventory() and not self.getInventory().getShipDoIdList())
        else:
            return True
        ...
        if forwardSpeed == 0.0:
            if slideSpeed == 0.0:
                ...
                if self.waterWake:
                    self.waterWake.stopLoop()
                    self.waterWake = None
                self.waterSplash and self.waterSplash.stopLoop()
                self.waterSplash = None
        else:
            if not self.waterWake:
                self.waterWake = WaterRippleWake.getEffect()
    ...
    def tryOnTattoo(self, tattooItem, location):
        ...
        gender = tattooItem and self.style.getGender()
    ...
    def tryOnJewelry(self, jewelryItem, location):
        ...
        if equipFunction:
            if jewelryItem != None:
                ...
                rarity = ItemGlobals.getRarity(itemId)
                if rarity != ItemConstants.CRUDE:
                    not Freebooter.getPaidStatus(base.localAvatar.getDoId()) and equipFunction(0, 0, 0)
                else:
                    if gender == 'f':
    ...
    def setClothes2(self, *dna):
        if hasattr(self, 'clothingEquipBufferDict'):
            return len(self.clothingEquipBufferDict.keys()) > 0 and None
        else:
            if dna == self.getClothesComposite():
                pass
            else:
    ...
    def initVisibleToCamera(self):
        if self is not localAvatar:
            localAvatar.getSoloInteraction() and self.hideFromCamera()
        else:
            self.showToCamera()
    ...
    def setTempDoubleXPReward(self, value):
        if not self.tempDoubleXPStatusMessaged:
            ...
            if self.getDoId() == localAvatar.getDoId():
                h, m = value != 0 and self.getHoursAndMinutes(value)
    ...
    def removeContext(self, context, number=0):
        if self.guiMgr:
            ...
            if number:
                if contextTutPanel.isFilled():
                    self.guiMgr.contextTutPanel.getContext() == context and self.guiMgr.contextTutPanel.getNumber() == number and contextTutPanel.closePanel()
    ...
    def setAuraActivated(self, auraActivated):
        ...
        if auraActivated == EnemySkills.STAFF_TOGGLE_AURA_WARDING:
            ...
            if auraActivated == EnemySkills.STAFF_TOGGLE_AURA_NATURE:
                ...
                if auraActivated == EnemySkills.STAFF_TOGGLE_AURA_DARK:
    ...
    def rewardNotify(self, rewardCat, rewardId):
        if base.config.GetBool('black-pearl-repeat-reward', 1) == 1:
            ...
            def showRewardPanel(task=None):
                ...
                if rewardCat:
                    rewardText = rewardId and PLocalizer.BlackPearlRewardSuccess % rewardName
                else:
...
#         and (4)
#              0. expr
#                 L.1539     962  LOAD_FAST             2  'textProp'
#              1. jmp_false (2)
#                  0.            965  JUMP_IF_FALSE        62  'to 1030'
#                  1.            968  POP_TOP          
#              2. expr
#                 L.1540     969  LOAD_FAST            10  'badges'
#              3. come_from_opt
#      1. expr
#         binary_expr (3)
#              0. expr
#                            972  LOAD_CONST           30  '\x01white\x01\x05badge-%s-%s\x05\x02 '
#              1. expr
#                 tuple (3)
#                      0. expr
#                         subscript (3)
...
#                                 attribute (2)
#                                      0. expr
#                                                    985  LOAD_FAST             0  'self'
#                                      1.            988  LOAD_ATTR            51  'badge'
#                              1. expr
#                                            991  LOAD_CONST           11  1
#                              2.            994  BINARY_SUBSCR    
#                      2.            995  BUILD_TUPLE_2         2  None
#              2. binary_op
#                            998  BINARY_MODULO    
#      2. inplace_op
#                    999  INPLACE_ADD      
#      3. store
#                   1000  STORE_FAST           10  'badges'
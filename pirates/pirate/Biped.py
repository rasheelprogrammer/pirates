...
class Biped(UsesAnimationMixer, Avatar, UsesEffectNode):
    ...
    def actorInterval(self, *args, **kwargs):
        if hasattr(self, 'undead'):
            return self.undead and UsesAnimationMixer.actorInterval(self.skeleton, *args, **kwargs)
        else:
    ...
    def play(self, *args, **kwArgs):
        if hasattr(self, 'undead'):
            self.undead and UsesAnimationMixer.play(self.skeleton, *args, **kwArgs)
        else:
            UsesAnimationMixer.play(self, *args, **kwArgs)

    def loop(self, *args, **kwArgs):
        if hasattr(self, 'undead'):
            self.undead and UsesAnimationMixer.loop(self.skeleton, *args, **kwArgs)
        else:
            UsesAnimationMixer.loop(self, *args, **kwArgs)

    def stop(self, *args, **kwArgs):
        if hasattr(self, 'undead'):
            self.undead and UsesAnimationMixer.stop(self.skeleton, *args, **kwArgs)
        else:
            UsesAnimationMixer.stop(self, *args, **kwArgs)

    def pose(self, *args, **kwArgs):
        if hasattr(self, 'undead'):
            self.undead and UsesAnimationMixer.pose(self.skeleton, *args, **kwArgs)
        else:
            UsesAnimationMixer.pose(self, *args, **kwArgs)

    def pingpong(self, *args, **kwArgs):
        if hasattr(self, 'undead'):
            self.undead and UsesAnimationMixer.pingpong(self.skeleton, *args, **kwArgs)
        else:
    ...
    def lookAroundTask(self, task):
        if self.isLocal():
            if self.gameFSM.state == 'Battle':
                ...
                dummyNode = len(visibleTargets) > 0 and random.choice((True, True, False)) and self.attachNewNode('dummy')
    ...
    def initSfx(cls):
        for name, effect in cls.SfxNames.iteritems():
            if name not in cls.sfx:
                ...
                if sound:
                    cls.sfx[name] = sound.getActive() and sound

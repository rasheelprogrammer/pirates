...
class PVPGameCTL(PVPGameBase):
    ...
    def handleDeposit(self, eventType, avId, bankId):
        if localAvatar.getDoId() == avId:
            if eventType == 'Team ' + str(localAvatar.getTeam()):
                if localAvatar.lootCarried > 0:
                    base.playSfx(self.depositSound)
                    self.sendUpdate('treasureDeposited', [bankId])
                    localAvatar.gameFSM.getCurrentOrNextState() != localAvatar.gameFSM.defaultState and localAvatar.b_setGameState(localAvatar.gameFSM.defaultState)
    ...
    def updateTreasureProximityText(self, treasure):
        ...
        if baseType:
            treasure.proximityText = (baseType == 'Team ' + str(localAvatar.getTeam()) and PLocalizer).ShipTransferInstructions
        else:

...
class DistributedBank(DistributedInteractive.DistributedInteractive):
    ...
    def enterProximity(self):
        if self.belongsToTeam == PiratesGlobals.INVALID_TEAM or self.belongsToTeam != localAvatar.getTeam():
            localAvatar.lootCarried < self.cr.activeWorld.getMaxCarry() and self.value > 0 and base.cr.interactionMgr.addInteractive(self, InteractiveBase.PROXIMITY)
            messenger.send('enterProximityOfInteractive')
        else:
            if self.belongsToTeam == localAvatar.getTeam():
                localAvatar.lootCarried > 0 and self.cr.activeWorld.handleDeposit('Team ' + str(self.belongsToTeam), localAvatar.getDoId(), self.getDoId())
            else:

...
class Scoreboard(DistributedObject):
    ...
    def getTeamScores(self, team):
        ...
        for id, score in self._scores.iteritems():
            if self._teams[id] == team:
                self._types[id] == PVPGlobals.SHIP_SCORE and scores.append({'ID': id, 'Score': score})
    ...
    def getItemList(self, team):
        ...
        for scoreItemDict in sortedScores:
            ...
            if localAvatar.getShip():
                localAvatar.getShip().doId == id and displayScores.append([self._names[id], stats, ['color', (1, 1, 1, 1)]])
            else:

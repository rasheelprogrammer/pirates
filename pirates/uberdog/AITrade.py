...
# Embedded file name: pirates.uberdog.AITrade
...
class AITrade(AITradeBase):
    ...
    def grantTeleportToken(self, token):
        if token == TortugaTeleportToken:
            ...
            if token == KingsheadTeleportToken:
                self.giveTortugaTeleportToken()
            else:
                if token == PortRoyalTeleportToken:
                    self.givePortRoyalTeleportToken()
                else:
                    if token == CubaTeleportToken:
                        self.giveCubaTeleportToken()
                    else:
                        if token == PadresDelFuegoTeleportToken:
    ...
    def giveReputation(self, category, amount):
        av = self.avatarId and self.air.doId2do.get(self.avatarId)
        inv = av and av.getInventory()

...
# Embedded file name: pirates.uberdog.AITradeBase
...
class AITradeBase(AsyncRequest):
    ...
    def takeLocatable(self, item):
        if item[0] == InventoryType.ItemTypeClothing:
            ...
            if item[0] == InventoryType.ItemTypeWeapon:
                ...
                if item[0] == InventoryType.ItemTypeConsumable:
                    ...
                    if item[0] == InventoryType.ItemTypeJewelry or item[0] == InventoryType.ItemTypeTattoo or item[0] == InventoryType.ItemTypeCharm:
                        self.notify.debug('takeLocatable: %s taking accessory %s' % (self.inventoryId, str(item)))
                        self.takeAccessory(item[0], item[1], item[2])
                    else:
                        if item[0] == InventoryType.ItemTypeMoney:
                ...
                for v in i:
                    ...
                    if InventoryId.isLimitChange(t):
                        ...
                        if giving:
                            ...
                            if equippableInfo == None or filter(lambda x: equippableInfo[x][1] == False, equippableInfo):
    ...
    def _checkRules(self, givingLimitChanges, givingStacks, givingAccumulators, givingDoIds, givingLocatable, takingLimitChanges, takingStacks, takingAccumulators, takingDoIds, takingLocatable):
        ...
        if takingAccumulators:
            ...
            if not (givingStacks or givingDoIds or givingLimitChanges or givingLocatable):
                ...
                if not (takingStacks or takingDoIds or givingLimitChanges or takingLocatable):

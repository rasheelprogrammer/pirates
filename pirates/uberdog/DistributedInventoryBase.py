...
class DistributedInventoryBase:
    ...
    def getInventoryOnlyStacks(cls, inventoryId):
        ...
        if inv is not None:
            return inv.stacksReady and inv
        else:
            return
        return
    ...
    def getInventory(cls, inventoryId, callback, timeout=30):
        ...
        if inv is not None:
            requestId = inv.isReady() and None
    ...
    def setAccumulators(self, accumulatorTypesAndQuantities):
        ...
        for t, q in accumulatorTypesAndQuantities:
            if old.get(t, 0) != q:
                ...
                if t >= InventoryType.begin_Accumulator:
                    t <= InventoryType.end_Accumulator and messenger.send('repChange-%s' % self.doId)

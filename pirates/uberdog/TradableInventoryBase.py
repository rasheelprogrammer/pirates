...
# Embedded file name: pirates.uberdog.TradableInventoryBase
...
class InvItem(tuple):
    ...
    def getLocation(self):
        if self[TradableInventoryBase.ITEM_CAT_IDX] == InventoryType.ItemTypeMoney:
            ...
            if self[TradableInventoryBase.ITEM_CAT_IDX] == InventoryType.ItemTypeMoneyWagered:
                ...
                if itemStoresLocation(self[TradableInventoryBase.ITEM_CAT_IDX]):
                    ...
                    if self[TradableInventoryBase.ITEM_CAT_IDX] == 0:
                        if self[TradableInventoryBase.ITEM_ID_IDX] == 0:
                            return len(self) > TradableInventoryBase.ITEM_LOCATION_IDX and self[TradableInventoryBase.ITEM_LOCATION_IDX]
                    else:
                        return
        return
    ...
    def compare(self, other, excludeLoc=False):
        if self:
            return other and self.getCat() == other.getCat() and isStackableType(self.getCat()) and self.getType() == other.getType() and True
        else:
            if excludeLoc:
                return self and other and self.setLocation(1) == other.setLocation(1) and True
            else:
                return self == other
...
class TradableInventoryBase(DistributedInventoryBase):
        ...
        for currItem in items:
            ...
            if isLocatable(currItem.getCat()):
                if currLoc == Locations.NON_LOCATION or currLoc == None:
                    ...
                    if self.locationAvailable(currLoc, itemType=currItem.getType()) or currItem.getCount(default=1) == 0:
                        location = currLoc
                itemType = currItem.getCat()
            if self.locationIsValid(location, itemType, itemType=currItem.getType(), includeEquip=equippable):
                location not in results and results.append(location)
        ...
        for currItemId in itemIds:
            ...
            if game.process == 'ai':
                ...
                def tradeFail(tradeObj, reason):
                    ...
                    if reason == RejectCode.UNDERFLOW:
    ...
    def getTestItem(self, category, type=None, location=None, count=None, color=None):
        ...
        for currElement in range(locatablesField.getNumElements()):
            ...
            if arrayParam:
                itemStruct = arrayParam.getName() == 'items' and arrayParam.getElementType().asClassParameter().getClass()
            ...
            for currField in range(numFields):
                if currField == 0:
                    ...
                    if currField == 1:
                        ...
                        if currField == 2:
                            newVal = (
                             location != None and location,)
                        else:
                            if currField == 3:
                                newVal = (
                                 count != None and count,)
                            else:
                                if currField == 3:
                                    newVal = (
                                     color != None and color,)
                                else:
...
#                                     attribute (2)
#                                          0. expr
#                                                        333  LOAD_FAST             5  'otherAdd'
#                                          1.            336  LOAD_ATTR            41  'getCount'
#                                  1.            339  CALL_FUNCTION_0       0  None
#                          1. expr
#                                        342  LOAD_CONST            3  0
#                          2.            345  COMPARE_OP            4  '>'
#              1. jmp_false (2)
#                  0.            348  JUMP_IF_FALSE        20  'to 371'
#                  1.            351  POP_TOP          
#              2. expr
#                 L.1084     352  LOAD_FAST             6  'trainingAmt'
#              3. come_from_opt
#      1. expr
#         call (2)
#              0. expr
#                 attribute (2)
#                      0. expr
#                                    355  LOAD_FAST             5  'otherAdd'
#                      1.            358  LOAD_ATTR            41  'getCount'
#              1.            361  CALL_FUNCTION_0       0  None
#      2. inplace_op
#                    364  INPLACE_ADD      
#      3. store
#                    365  STORE_FAST            6  'trainingAmt'
    ...
    def startEffects(self):
        ...
        if self.parent.uniqueId in self.WaterCaves:
            if base.config.GetBool('want-shaders', 1):
                self.water = base.win and base.win.getGsg() and base.win.getGsg().getShaderModel() >= GraphicsStateGuardian.SM20 and Swamp('models/caves/cave_a_water', self.parent, reflection, None, None, shader_water_color)
            ...
            if self.parent.uniqueId in self.LavaCaves:
                if base.config.GetBool('want-shaders', 1):
                    self.water = base.win and base.win.getGsg() and base.win.getGsg().getShaderModel() >= GraphicsStateGuardian.SM20 and Swamp(self.modelPrefix + 'lava', self.parent, reflection, None, None, shader_water_color)

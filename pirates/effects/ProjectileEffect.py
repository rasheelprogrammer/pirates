...
class ProjectileEffect:
    ...
    def __init__(self, cr, attackerId, hitObject, objType, pos, skillId, ammoSkillId, normal=None):
        ...
        if objType == PiratesGlobals.COLL_AV:
            ...
            if objType in (PiratesGlobals.COLL_MONSTER, PiratesGlobals.COLL_MONSTROUS):
                ...
                if objType == PiratesGlobals.COLL_DESTRUCTIBLE:
                    ...
                    if objType == PiratesGlobals.COLL_SHIPPART or objType == PiratesGlobals.COLL_NEWSHIP:
                        self.propHitEffect(hitObject, pos, skillId, ammoSkillId)
                    else:
                                            ...
                                            if objType == PiratesGlobals.COLL_CANNON:
                                                ...
                                                if objType == PiratesGlobals.COLL_FORT:
                                                    ...
                                                    if objType == PiratesGlobals.COLL_DEFENSE_AMMO:
                                                        ...
                                                        if objType == PiratesGlobals.COLL_FLAMING_BARREL:
                                                            pass
                                                        else:
    ...
    def basicHitEffect(self, hitObject, pos, skillId, ammoSkillId):
        ...
        if ammoSkillId in [InventoryType.CannonRoundShot, InventoryType.CannonChainShot, InventoryType.CannonBullet, InventoryType.CannonSkull, InventoryType.CannonBarShot, InventoryType.CannonFury, InventoryType.CannonFirebrand, InventoryType.CannonFlamingSkull, InventoryType.CannonThunderbolt]:
            ...
            if ammoSkillId == InventoryType.CannonExplosive:
                ...
                if base.options.getSpecialEffectsSetting() >= base.options.SpecialEffectsMedium:
                    ...
                    effect = base.options.getSpecialEffectsSetting() >= base.options.SpecialEffectsHigh and FlamingDebris.getEffect(unlimited)
                    effect and effect.wrtReparentTo(base.effectsRoot)
                    effect.setPos(hitObject, pos)
                    effect.duration = 4
                    effect.velocityX = self.projVelocity[i][0]
                    effect.velocityY = self.projVelocity[i][1]
                    effect.play()
        if ammoSkillId == InventoryType.CannonThunderbolt and base.options.getSpecialEffectsSetting() >= base.options.SpecialEffectsLow:
        ...
        if base.options.getSpecialEffectsSetting() >= base.options.SpecialEffectsMedium:
            ...
            if ammoSkillId == InventoryType.CannonFury:
                flashEffect = FlashEffect()
                flashEffect.wrtReparentTo(base.effectsRoot)

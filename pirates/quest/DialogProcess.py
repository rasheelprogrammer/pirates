...
# Embedded file name: pirates.quest.DialogProcess
...
class StepChoice(DialogProcess):
    ...
    def displayStepChoices(self):
        ...
        for i in range(len(self.choices)):
            ...
            if process.avCanParticipate(localAvatar):
                choiceButton = isinstance(process, PlayerDialog) and GuiButton.GuiButton(parent=aspect2d, relief=None, text=self.__getDialogChoiceText(self.choices[i], index), text_font=PiratesGlobals.getPirateFont(), text_shadow=PiratesGuiGlobals.TextShadow, text_wordwrap=None, text_align=TextNode.ALeft, text_scale=0.055, text0_fg=PiratesGuiGlobals.TextFG2, text1_fg=PiratesGuiGlobals.TextFG8, text2_fg=PiratesGuiGlobals.TextFG8, text3_fg=PiratesGuiGlobals.TextFG9, image=choiceIcon, image_scale=0.3, image_pos=(-0.05, 0, 0.007), geom=None, pos=(-1.15, 0, -0.48 - i * 0.075), command=self.buttonClicked, extraArgs=[self.choices[i]])

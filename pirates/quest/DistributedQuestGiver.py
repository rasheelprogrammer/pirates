...
class DistributedQuestGiver(Avatar.Avatar):
    ...
    def hasQuestOffers(self):
            ...
            for prereq in prereqs:
                ...
                if questDNA.getVelvetRoped():
                    ...
                    if questDNA.getHoliday() is not None:
                        ...
                        passed = base.cr.newsManager and not base.cr.newsManager.getHoliday(holidayId) and False
                        break

            if prereqs:
                passed and AvailableQuests.append(questDNA)
        ...
        for quest in localAvatar.getQuests():
            if quest:
                return quest.getTimeLimit() and quest.canBeReturnedTo(self.getQuestGiverId()) and True
    ...
    def setQuestOffer(self, offers):
        ...
        self.offers = offers
        for quest in localAvatar.getQuests():
            if quest.getTimeLimit():
                if quest.canBeReturnedTo(self.getQuestGiverId()):
                ...
                questOffer = branchParent and branchParent.getGiverId() == self.getQuestGiverId() and QuestOffer.QuestBranchResetOffer.create(quest.getQuestId(), localAvatar, branchReset=True)
                offers.append(questOffer)
    ...
    def presentQuestOffer(self, offers, ladder=False):
        ...
        def describeQuest(offer):
            ...
            if questDNA:
                if isinstance(offer, QuestOffer.QuestTimerResetOffer):
                    ...
                    if isinstance(offer, QuestOffer.QuestBranchResetOffer):
        ...
        if not hasStoryQuest:
            self.questMenuGUI = numWorkQuests > QuestConstants.MAXIMUM_MERC_WORK and PDialog.PDialog(text=PLocalizer.QuestFull, style=OTPDialog.Acknowledge, command=questFull)
        else:
        ...
        if not dialogId:
            ...
            if npcDialogs:
                for dialogId in npcDialogs:
                    ...
                    if firstDialogProcess.prereq:
                        firstDialogProcess.avCanParticipate(localAvatar) and availableDialogs.append(dialogId)
    ...
    def hasQuestOffersForLocalAvatar(self):
        ...
        if inventory:
            ...
            for quest in inventory.getQuestList():
                ...
                isComplete = quest.canBeReturnedTo(selfId) and False
                if quest.isComplete():
                    container = localAvatar.questStatus.getContainer(quest.getQuestId())
                elif container:
                    if container.parent:
                        if container.parent.isChoice():
                            isComplete = True
                            for q in container.parent.getContainers():
                                if q.quest:
                                    isComplete = not q.quest.isComplete() and False
                ...
                for prereq in questDNA.prereqs:
                    ...
                    if questDNA.getHoliday() is not None:
                        ...
                        if base.cr.newsManager:
                            passed = not base.cr.newsManager.getHoliday(holidayId) and False
                            break

                questDNA.prereqs and passed and fromQuests.append(questDNA)
            ...
            if inventory:
                ...
                for questDNA in fromQuests:
                    ...
                    if not av.questStatus.hasLadderQuestId(questId):
                        questId not in [ x.questId for x in questList ] and questsLeft.append(questDNA)

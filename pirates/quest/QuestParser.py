...
class NPCMoviePlayer(DirectObject.DirectObject):
    ...
    def finishUpAll(self):
        ...
        for currTrack in trackList:
            if self.cleanedUp == False:
                self.currentTrack = len(self.chapterDict[currTrack]) > 0 and self.chapterDict[currTrack].pop(0)
                ...
                if command == 'WAIT':
                    ...
                    if command == 'CHAT':
                        ...
                        if command == 'CLEAR_CHAT':
                            ...
                            if command == 'INTERACTIONAL_CHAT':
                                iList.append(self.parseInteractionalChat(line))
                                return (
                                 chapterList, resultingNextEvent)
                            else:
                                if command == 'CLEAR_INTERACTIONAL_CHAT':
    ...
    def parseExtraChatArgs(self, args):
        ...
        for arg in args:
            if type(arg) == type(0):
                ...
                if len(arg) > 2:
                    extraChatFlags = arg[:2] == 'CF' and eval(arg)
                else:
    ...
    def parseSetEquippedWeapons(self, line):
        ...
        return [Func(localAvatar.guiMgr.cleanupEquippedWeapons), Func(localAvatar.guiMgr.setEquippedWeapons, weapons)]

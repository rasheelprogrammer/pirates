...
class RecoverShipItemTaskDNA(QuestTaskDNA):
    ...
    def handleShipDefeat(self, questEvent, taskState):
        ...
        if self.level > 0:
            if questEvent.level < self.level:
                return False
        if self.isFlagship == True:
            if questEvent.isFlagship == False:
                return False
            if self.getLocation():
                if not self.locationMatches(questEvent):
                    return False
            ...
            if attempts < self.maxAttempts:
                attempts += 1
                taskState.setAttempts(attempts)
            found = attempts == self.maxAttempts and True
        else:
            if attempts > self.maxAttempts:
                found = True
            else:
...
class SmuggleItemTaskDNA(DeliverItemTaskDNA):
    ...
    def handleDockedAtPort(self, questEvent, taskState):
        if not self.npcId:
            if self.getLocation():
                if self.locationMatches(questEvent):
                    return questEvent.getDeployLocation() and questEvent.getLocation() != questEvent.getDeployLocation() and True
        return False

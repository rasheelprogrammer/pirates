...
class DistributedInteractiveProp(DistributedInteractive.DistributedInteractive, DistributedTargetableObject.DistributedTargetableObject, Actor.Actor, InteractivePropBase.InteractivePropBase):
    ...
    def targetedWeaponHit(self, skillId, ammoSkillId, skillResult, targetEffects, attacker, pos, charge=0, delay=None, multihit=0, itemEffects=[]):
        if skillResult in [WeaponGlobals.RESULT_HIT, WeaponGlobals.RESULT_MISTIMED_HIT]:
            if skillId == InventoryType.CutlassSlash or skillId == InventoryType.CutlassSweep:
                if base.config.GetBool('want-easy-combos', 1):
                    skillResult == WeaponGlobals.RESULT_HIT and self.gameFSM.request('Death')
            else:
    ...
    def getLocatorOffset(self, avParent):
        ...
        if self.modelPath == 'models/props/chair_fancy':
            ...
            if self.modelPath == 'models/props/chair_bank':
                dummyNode.setX(1.75)
                dummyNode.setY(-1.25)
                dummyNode.setH(180)
            else:
                if self.modelPath == 'models/props/chair_shanty':
                    dummyNode.setX(2.0)
                    dummyNode.setY(-0.5)
                    dummyNode.setH(180)
                else:
                    if self.modelPath == 'models/props/chair_bar':
                        dummyNode.setX(1.75)
                        dummyNode.setY(-0.5)
                        dummyNode.setH(180)
                    else:

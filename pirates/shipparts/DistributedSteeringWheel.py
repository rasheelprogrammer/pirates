...
class DistributedSteeringWheel(DistributedInteractive.DistributedInteractive, DistributedShippart.DistributedShippart):
    ...
    def checkInUse(self):
        if self.userId:
            base.localAvatar.getDoId() != self.ship.ownerId and localAvatar.getDoId() != self.userId and self.setAllowInteract(0)
        else:
            if self.userId:
                self.proximityText = (base.localAvatar.getDoId() == self.ship.ownerId and PLocalizer).InteractWheelCaptain
                self.setAllowInteract(1)
            else:

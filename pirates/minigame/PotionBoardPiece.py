...
# Embedded file name: pirates.minigame.PotionBoardPiece
...
class PotionBoardPiece(NodePath):
    ...
    def showName(self, show=True):
        if self.colorIndex >= 0:
            nameText = self.colorIndex < 6 and self.level > 0 and self.level <= 6 and PLocalizer.PotionIngredients[self.colorIndex][self.level - 1]
        else:
    ...
    def testConnection(self, gameBoard, col, row):
        if col >= 0:
            if col < gameBoard.numColumns:
                if row >= 0 and row < gameBoard.numRows:
                    if gameBoard.boardPieces[col][row] is not None:
                        gameBoard.boardPieces[col][row].colorIndex == self.colorIndex and gameBoard.boardPieces[col][row].level == self.level and self.connections.append(gameBoard.boardPieces[col][row])
                    else:
    ...
    def testPreviewConnection(self, gameBoard, col, row):
        if col >= 0:
            if col < gameBoard.numColumns:
                if row >= 0:
                    if row < gameBoard.numRows:
                        if gameBoard.boardPieces[col][row] is not None:
                            ownLevel = (gameBoard.boardPieces[col][row].colorIndex == self.colorIndex and self not in gameBoard.removeList and gameBoard.boardPieces[col][row] not in gameBoard.removeList and self).level
                            ...
                            if gameBoard.boardPieces[col][row] in gameBoard.upgradeList:
                                otherLevel += 1
                            ownLevel == otherLevel and self.pendingConnections.append(gameBoard.boardPieces[col][row])
        return
    ...
    def updateDisplay(self):
        if self.colorIndex >= 0:
            if self.colorIndex <= 5:
                if self.level > 0:
                    if self.level <= 6:
                        texName = self.colorIndex == 0 and 'pir_t_gui_pot_tok_red_%i' % self.level
                    else:
                    ...
                    self.nameCheck is not None and self.nameCheck.stash()
            else:

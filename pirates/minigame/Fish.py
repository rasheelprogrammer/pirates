...
# Embedded file name: pirates.minigame.Fish
import random, math
...
class Fish(NodePath):
    ...
    def turnAround(self, nextState, shouldMoveRight):
        if self.velocity[0] < 0:
            self.velocity[0] = shouldMoveRight and -self.velocity[0]
        else:
            if self.velocity[0] > 0 and not shouldMoveRight:
    ...
    def update(self, dt, index, lurePos):
        ...
        if currentState in ['ScareAway', 'Swimming', 'Flee', 'TurnAround']:
            ...
            if currentState in ['Hooked', 'AboutToFight', 'HookedFighting']:
    ...
    def makeFishRunFromMe(self, otherFish):
        if otherFish.fsm.getCurrentOrNextState() == 'Flee' or otherFish.fsm.getCurrentOrNextState() == 'TurnAround':
            return
        if otherFish.getX() < self.getX(self.fishManager.gameObject.fishingSpot):
            otherFish.movingRight and otherFish.fsm.request('TurnAround', 'Flee', False)
        else:
            if otherFish.getX() > self.getX(self.fishManager.gameObject.fishingSpot):
                not otherFish.movingRight and otherFish.fsm.request('TurnAround', 'Flee', True)
            else:
    ...
    def updateBasedOnBehavior(self, dt, lurePos):
        ...
        newZ = self.getZ()
        for fish in self.closeFish:
            if self.myData['size'] == 'small':
                if fish.myData['size'] == 'large':
                    if self.checkForEating(fish):
                        return
                self.avoidingFish = True
                if fish.velocity[1] > 0.0:
                    self.velocity[1] = fish.avoidingFish and -FishingGlobals.fishAvoidYVelocity
                else:
                    self.velocity[1] = FishingGlobals.fishAvoidYVelocity
                self.velocity[1] = abs(fish.getY() - self.getY()) > self.myData['collisionBoxSize'][1] + fish.myData['collisionBoxSize'][1] and 0.0

        if len(self.closeFish) == 0:
            if abs(self.getY()) > FishingGlobals.fishYTolerance:
                self.avoidingFish = False
                if self.getY() > 0:
                    self.velocity[1] = -FishingGlobals.fishAvoidYVelocity
                else:
            ...
            newX, newZ = currentState in ['Swimming', 'TurnAround', 'Flee', 'ScareAway'] and currentState == 'ScareAway' and self.performScareAwayBehavior(dt, self.velocity, self.accel)
        else:
        ...
        if newX < FishingGlobals.leftFishBarrier:
            if currentState == 'ScareAway':
                ...
                if currentState != 'TurnAround':
                    if not self.movingRight:
                        self.fsm.request('TurnAround', 'Swimming', True)
                else:
                    if newX > FishingGlobals.rightFishBarrier:

...
# Embedded file name: pirates.minigame.PotionGameBoard
...
class PotionGameBoard(DirectFrame):
    ...
    def moveDown(self):
        self.disableInputEvents()
        if self.boardPieces[self.leftPlayerPiece.column][self.leftPlayerPiece.row - 1] is None:
            self.boardPieces[self.leftPlayerPiece.column][self.leftPlayerPiece.row] = (self.boardPieces[self.rightPlayerPiece.column][self.rightPlayerPiece.row - 1] is None and self).leftPlayerPiece
                    ...
                    if screenx > 0:
                        if screeny > -0.8:
                            if screenx < 0.88 and screeny < 0.92:
                                self.moveDown()
                        recipePos = self.dragPiece is not None and False and self.potionGame.currentRecipe.getRelativePoint(render2d, (screenx, 0, screeny))
    ...
    def handleMouseLoc(self, task):
        if base.mouseWatcherNode.hasMouse():
            ...
            if self.lastDragPos is None:
                if self.leftPlayerPiece is not None:
                    if self.rightPlayerPiece is not None and localMousePos[0] < self.leftPlayerPiece.Xpos:
                        self.jumpLeft()
                    localMousePos[0] > self.rightPlayerPiece.Xpos and self.jumpRight()
            elif self.lastDragPos is not None:
                self.dragDist += (localMousePos - self.lastDragPos).length()
                self.lastDragPos = localMousePos
                if self.dragDist > 0.01:
                    globalMousePos = self.dragPiece is not None and self.potionGame.background.getRelativePoint(render2d, (screenx, 0, screeny))
    ...
    def checkAvailableMoves(self):
        ...
        for columnIndex in range(self.numColumns - 1):
            if self.boardPieces[columnIndex][self.numRows - 2] is None:
                movesLeft = self.boardPieces[columnIndex + 1][self.numRows - 2] is None and True

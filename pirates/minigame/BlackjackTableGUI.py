...
# Embedded file name: pirates.minigame.BlackjackTableGUI
...
class BlackjackStatusPanel(DirectFrame, SplitBase):
    ...
    def displayAction(self, text, table, seat):
        ...
        actor = None
        if seat >= 0:
            if seat < len(table.actors):
                actor = table.actors[seat]
            else:
                actor = table.dealer
            if actor:
                name = actor.getName()
            self.actionLabel['text'] = text == '' and text
        else:
...
class BlackjackTableGUI(DirectFrame, TableGUI, SplitBase):
    ...
    def playerAction(self, action):
        ...
        if action == PlayingCardGlobals.CardSwap:
            ...
            if action == PlayingCardGlobals.Split:
                ...
                if length > 0:
                    if self.table.localAvatarSeat > 0 and self.table.localAvatarSeat < length:
                        hand = self.hands[self.table.localAvatarSeat]
                        hand and len(hand) == 2 and self.splitCardsArray.append(hand[1])
                    ...
                    if handValue == 21:
                        if len(newHand) == 2:
                            ...
                            if handValue > 21:
                                ...
                                handNameLabel['text'] = str(handValue)
                        handNameLabel.show()
                        handArray = i == self.table.localAvatarSeat and allHands[i]
                        total_hands = len(handArray)
                        if total_hands > 1:
                        ...
                        handValue >= 21 and self.disableAllPlayButtons()
                    else:
        ...
        if len(action) >= 2:
            ...
            if seatIndex == self.table.localAvatarSeat:
                ...
                if action[0] == PlayingCardGlobals.Split:
                    time = PlayingCardGlobals.AskCardTimeout
                    self.disableAllPlayButtons()
                time > 0.0 and self.startTimer(time)
    ...
    def splitableHand(self, card1, card2):
        ...
        if rank1 == rank2:
            ...
            if rank1 >= 8 and rank1 <= 11 and rank2 >= 8 and rank2 <= 11:
                state = True
        return state

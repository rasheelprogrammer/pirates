...
# Embedded file name: pirates.minigame.RepairGridPiece
import math
...
class RepairGridPiece(DirectButton, FSM.FSM):
    ...
    def checkMovePiece(self, isPush=False):
        ...
        if absZ > absX:
            moveDirection = diff.getZ() > 0.0 and absZ > threshold and directions[1]
        else:
            if absZ > absX:
                moveDirection = diff.getZ() < 0.0 and absZ > threshold and directions[0]
            else:
                if absX > absZ:
                    moveDirection = diff.getX() > 0.0 and absX > threshold and directions[2]
                else:
                    if absX > absZ and diff.getX() < 0.0 and absX > threshold:

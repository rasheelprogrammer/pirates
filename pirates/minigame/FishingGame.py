...
# Embedded file name: pirates.minigame.FishingGame
import random, math
...
class FishingGame(DirectObject.DirectObject):
    ...
    def fishingHandleTurning(self, dt, currentState):
        ...
        if not self.halfwayAround:
            if self.lastRodRotationValue > 120.0:
                self.halfwayAround = newR < 120.0 and True
        else:
            if self.lastRodRotationValue < 0.0:
                weight = (newR > 0.0 and newR < 90.0 and self).fishManager.activeFish.weight
    ...
    def updateFishingGame(self, task):
        ...
        if self.fsm.getCurrentOrNextState() == 'LegendaryFish':
            ...
            if currentState in ['ReelingFish']:
                ...
                if int(self.lure.getX()) <= int(FishingGlobals.leftLureBarrier):
                    if int(self.lure.getZ()) >= int(self.waterLevel):
                ...
                if currentState in ['Fishing', 'Reeling', 'QuickReel', 'ReelingFish', 'Lose', 'FishOnHook']:
                    ...
                    if newZ < -self.castDistance or currentState == 'Fishing' and newZ < FishingGlobals.fishingLevelBoundaries[self.getRodLevel() - 1]:
                        ...
                        if self.boundaryTarget[1] > 0:
                            self.boundaryTarget[1] = max(0, self.boundaryTarget[1] - 0.3)
                    self.lure.setPos(newX, -1.0, newZ)
                    if int(self.lure.getX()) <= int(FishingGlobals.leftLureBarrier):
                        int(self.lure.getZ()) >= int(self.waterLevel) and self.sfx['lureOut'].play()
                    ...
                    if currentState in ['LureSink']:
                        ...
                        if currentState in ['FishFighting']:
            ...
            goalPos = self.oceanEye and Point3(*FishingGlobals.oceanEyeCameraPosition)
            camRange = (0, 1, 2)
        else:
            if FishingGlobals.stateToCameraOffsetInfo[currentState][2]:
                goalPos = self.lure.getPos()
                goalPos[1] = cameraOffset[1]
                camRange = (0, 1, 2)
            else:
                if FishingGlobals.stateToCameraOffsetInfo[currentState][1]:
                    if self.fishManager.activeFish is not None:
                        goalPos = self.fishManager.activeFish.mouthJoint.getPos(self.fishingSpot)
                        goalPos[1] = self.fishManager.activeFish.myData['size'] == 'large' and cameraOffset[1] - 5.0
                    else:
                        if self.fishManager.activeFish.myData['size'] == 'medium':
                            goalPos[1] = cameraOffset[1]
                        else:
    ...
    def checkForHooked(self):
        if self.fsm.getCurrentOrNextState() == 'LegendaryFish':
            ...
            if self.fsm.getCurrentOrNextState() == 'FishBiting':
                if self.hookedIt:
                    self.hookedIt = False
                    self.attachFishToLure()
                    if base.mouseWatcherNode.isButtonDown(MouseButton.one()):
    ...
    def chooseLure(self, type):
        if type == InventoryType.RegularLure:
            ...
            if type == InventoryType.LegendaryLure:
    ...
    def transitionBackdrop(self, stateId=None, stateDuration=0.0, elapsedTime=0.0, transitionTime=0.0):
        ...
        if usesShader:
            usesShader = self.distributedFishingSpot.onABoat and 2
        else:
            if usesShader and localAvatar.getParentObj().getUniqueId() == LocationIds.DEL_FUEGO_ISLAND:
                usesShader = 3

...
# Embedded file name: pirates.minigame.DistributedLiarsDice
import random
...
class DistributedLiarsDice(DistributedDiceGame.DistributedDiceGame):
    ...
    def extraGuiCallback(self, action):
        ...
        if action == 'dice up':
            ...
            return 1
        if action == 'dice down' and self.betDice > 1 and self.betDice > self.oldDice:
            self.sendChat(DiceGlobals.CHAT_DICEDOWN)
            self.betDice -= 1
            if self.betDice == 9:
                self.diceNumLabel.setPos(-0.28, 0, -0.7)
            self.diceNumLabel['text'] = '%d' % self.betDice
            if self.betDice == self.oldDice:
                if self.betValue < self.oldValue:
                    self.betValue = self.oldValue
                    self.diceValLabel['text'] = '%d' % self.betValue
            return 1
        else:
            if action == 'make claim':
                self.makeClaim()
                return 1
            else:
                if action == 'call bluff':
                    self.callBluff()
                    return 1
                else:
                    if action == 'cheat':
    ...
    def callBluff(self):
        if self.oldDice == 1:
            self.gui.gameStatus['text'] = (self.oldValue == 1 and PLocalizer).DiceText_FirstClaim
        else:

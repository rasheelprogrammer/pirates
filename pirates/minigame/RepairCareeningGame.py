...
# Embedded file name: pirates.minigame.RepairCareeningGame
import random, math
        ...
        for b in self.barnacles:
            if i < self.barnacleCount:
                ...
                if x - self.config.mossEdgeRestrictionAmount <= self.config.xRange[0] or x + self.config.mossEdgeRestrictionAmount >= self.config.xRange[1]:
                    ...
                    if y - self.config.mossEdgeRestrictionAmount <= self.config.yRange[0] or y + self.config.mossEdgeRestrictionAmount >= self.config.yRange[1]:
                        closeToEdge = True
                if not moss.isEmpty():
                    closeToEdge and moss.stash()
                else:
                    if not moss.isEmpty():
                        moss.unstash()
    ...
    def updateTask(self, task):
        ...
        if self.isMouseDown:
            ...
            if self.scrubMeterBar.getSz() < 1:
                ...
                if self.scrubMeterBar.getSz() != 1.0:
        ...
        if base.mouseWatcherNode.hasMouse():
            ...
            if self.brush.isStashed():
                self.brush.unstash()
            if self.isMouseDown:
                randomR = self.scrubMeterBar.getSz() > 0 and random.random() * 40.0 - 20.0

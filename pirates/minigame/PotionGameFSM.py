...
# Embedded file name: pirates.minigame.PotionGameFSM
                            ...
                            if len(self.gameObject.animationList) > 0:
                                ...
                                if self.gameObject.currentRecipe.complete:
                                    ...
                                    if len(self.gameObject.animationList) > 0:
                                        ...
                                        if len(self.gameObject.animationList) > 0:
                                            if not self.gameObject.soulMatch:
                                                if not self.gameObject.soulMade:
                                                    self.matchMade = True
                                                (self.gameObject.soulMade and self.gameObject.hintScreen.show('SoulMade') or self.gameObject.hintScreen.show('MatchMade')) and self.demand('Tutorial')
                                            else:
                                                self.demand('Anim')
                                            self.gameObject.soulMade = False
                                        else:
                                            if not self.gameObject.gameBoard.playerPieceActive():
                                                self.gameObject.gameBoard.addNewPiece()
                                                self.demand('Anim')
                                            else:
                                                self.demand('Input')

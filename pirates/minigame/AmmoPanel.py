...
# Embedded file name: pirates.minigame.AmmoPanel
...
class AmmoPanel(DirectFrame):
    ...
    def onAmmoClick(self, skillId):
        ...
        if button.isLocked():
            if not Freebooter.getPaidStatus(base.localAvatar.doId):
                if skillId > CannonDefenseGlobals.FREEBOOTER_LAST_AMMO_AVAILABLE:
            ...
            if button.canPurchase(self._bankNotes):
                if self.addAmmoSkill(skillId):
                    ...
                    self.updateBankNoteText()
                    button.disablePurchase()
                    self.sfxAmmoBought and base.playSfx(self.sfxAmmoBought)
    ...
    def removeAmmoSkill(self, skillId, useDefaultAmmo=False):
        ...
        self.updateQuantities()
        if useDefaultAmmo:
            not self.hasAmmo() and self.onAmmoClick(InventoryType.DefenseCannonRoundShot)
        else:
            if skillId == base.localAvatar.guiMgr.combatTray.getLastAmmoSkillId():

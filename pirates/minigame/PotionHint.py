...
# Embedded file name: pirates.minigame.PotionHint
...
class PotionHint(DirectFrame):
    ...
    def noHintsCheckCB(self, val):
        self.hintCB = val
        if self.hintCB:
            len(self.messageQueue) > 0 and self.bAccept.unstash()
            self.bClose.stash()
        else:
            self.bAccept.stash()
            self.bClose.unstash()
    ...
    def show(self, hintKey):
        ...
        if self.showHints:
            if not self.hintShown[hintKey] and self.potionGame.closeCurrentDialog is not None:

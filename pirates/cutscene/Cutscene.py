...
class Cutscene(NodePath, DirectObject):
    ...
    def initialize(self, doneCallback=None, giverId=None, patch=False):
        ...
        if originNode is not None:
            ...
            if giverObject:
                giverObject.getParent().getParent().getName() == 'GameArea' and self.reparentTo(giverObject.getParent().getParent())
            else:
                if 'localAvatar' in __builtins__:
                    if localAvatar.ship:
                        self.reparentTo(render)
    ...
    def getEnvEffects(self):
        ...
        if self.cr:
            ...
            if ga:
                self.envEffects = (hasattr(ga, 'envEffects') and ga).envEffects
                self.gameArea = ga
        else:

...
class ShipUpgradeInterface(DirectFrame):
    ...
    def filloutOwned(self):
        ...
        costDict = None
        inv = localAvatar.getInventory()
        if self.previewValue != None:
            if self.previewCat != None and not self.viewingCurrent and not self.viewingDowngrade:
                typeDict = ShipUpgradeGlobals.UPGRADE_BY_CATINDEX.get(self.previewCat, {}).get(self.previewValue, {})
                if typeDict:
                    costDict = typeDict.get('Cost', None)
            ownedString = costDict and self.ship and inv and '\n' + PLocalizer.ShipUgradeCostsOwned + '\n\n'
            for currency in ShipUpgradeGlobals.COST_LIST:
                fixedCost = 0
                relativeCost = 0
                if costDict.get('Fixed'):
    ...
    def filloutHullDetails(self, hullInfo=None, oldHullInfo=None):
        ...
        if oldHullInfo != None:
            ...
            if hullInfo:
                ...
                broadsideType = hullInfo.get('BroadsideType', 0) and hullInfo.get('BroadsideType')
    ...
    def setShip(self, shipIndex=0, shipId=None, changed=0):
        ...
        if self.shipId != shipId or changed:
            ...
            self.shipId = shipId
        if self.ship == None:
            shipOV = self.shipId and base.cr.getOwnerView(self.shipId)
            hullMaterial = None
            sailMaterial = None
            sailPattern = None
            invertLogo = False
    ...
    def confirmPurchase(self):
        ...
        if self.waitingForUpgrade:
            return
        if self.previewCat != None:
            if self.previewValue != None:
                if not Freebooter.getPaidStatus(localAvatar.getDoId()):
                    localAvatar.guiMgr.showNonPayer()
                else:
                    self.confirmPanel = self.viewingDowngrade and ShipUpgradeConfirm.ShipUpgradeConfirm(PLocalizer.ShipUpgradeConfirmTitleDown, PLocalizer.ShipUpgradeConfirmAskDown, self.panelConfirmOkay, self.panelConfirmCancel)
            else:
                if self.viewingSidegrade:

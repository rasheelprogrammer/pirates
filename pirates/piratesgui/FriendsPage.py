...
class FriendsPage(SocialPage.SocialPage):
    ...
    def __init__(self, showAvatar=1, showPlayer=1, showPlayerFriendAvatars=0):
        ...
        if self.showAvatar:
            myTitle = (self.showPlayer and PLocalizer).FriendsListLabel
        else:
            if self.showAvatar:
    ...
    def updatePlayerFriendAvatar(self, playerId, info):
        if info.isOnline():
            info.avatarId and self.membersList.updateOrAddMember(info.avatarId, playerId, PirateMemberList.MODE_FRIEND_PLAYER_AVATAR, info)
        else:

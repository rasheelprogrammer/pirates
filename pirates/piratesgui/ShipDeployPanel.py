...
class ShipDeployPanel(ShipSelectionPanel):
    ...
    def __init__(self, title, doneCallback, siegeTeam=0):
        ...
        if localAvatar.style.getTutorial() < PiratesGlobals.TUT_GOT_SHIP:
            text = localAvatar.getCurrentIsland() != QuestConstants.LocationIds.PORT_ROYAL_ISLAND and PLocalizer.DinghyNeedFirstShip % PLocalizer.LocationNames[QuestConstants.LocationIds.PORT_ROYAL_ISLAND]
        else:
    ...
    def setPage(self, pageId):
        ...
        if self.shipFrames[pageId] or pageId == self.OWN and localAvatar.style.getTutorial() < PiratesGlobals.TUT_GOT_SHIP:
            if localAvatar.getCurrentIsland() != QuestConstants.LocationIds.PORT_ROYAL_ISLAND:
                ...
                if self._siegeTeam:
                    if localAvatar.getSiegeTeam():
                        if localAvatar.getSiegeTeam() != self._siegeTeam:
                            self.noShipHint['text'] = localAvatar.getSiegeTeam() == PVPGlobals.FrenchTeam and PLocalizer.DinghyWrongSiegeShip % PLocalizer.PVPFrench
                        else:

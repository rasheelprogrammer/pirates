...
class ReputationMeterDial(DirectFrame):
    ...
    def update(self, value, updateLocal=0):
        ...
        if self.category == InventoryType.OverallRep:
            ...
            self.level = level
            self.levelLabel['text_fg'] = (level == levelCap and PiratesGuiGlobals).TextFG4
        ...
        if self.lastExp:
            ...
            self.lastLevel = self.lastLevel != level and level
            ...
            if leftoverValue:
                expUpIval = float(self.max) / float(leftoverValue) > 2.0 and Sequence(LerpColorScaleInterval(objFrameA, 0.2, glowFrameColor, blendType='easeInOut'), Wait(0.2), LerpColorScaleInterval(objFrameA, 0.5, startFrameColorA, blendType='easeInOut'), LerpColorScaleInterval(objFrameA, 0.1, glowFrameColor, blendType='easeInOut'), Wait(0.1), LerpColorScaleInterval(objFrameA, 0.3, startFrameColorA, blendType='easeInOut'))
            else:

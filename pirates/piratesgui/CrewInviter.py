...
class CrewInviter(DirectFrame):
    ...
    def enterBegin(self):
        if self.avId == localAvatar.doId:
            ...
            if self.bandId and self.bandId[0] != 0 and self.bandId[1] != 0 or DistributedBandMember.DistributedBandMember.getBandMember(self.avId):
                ...
                if len(localcrew) >= BandConstance.MAX_BAND_MEMBERS:
                    ...
                    if len(localcrew) > 0:
                        DistributedBandMember.DistributedBandMember.IsLocalAvatarHeadOfBand() and self.fsm.request('checkAvailability')
                    else:
                        if len(localcrew) > 0:
    ...
    def __crewRejectInvite(self, avId, reason):
        if reason == BandConstance.outcome_not_online:
            ...
            if reason == BandConstance.outcome_already_invited:
                ...
                if reason == BandConstance.outcome_recently_invited:

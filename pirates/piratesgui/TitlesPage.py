...
# Embedded file name: pirates.piratesgui.TitlesPage
    ...
    def refresh(self):
        ...
        if inv:
            ...
            if self.titleId == TitleGlobals.ShipPVPTitle:
                ...
                if self.titleId == TitleGlobals.LandPVPTitle:
        ...
        if self.expMeter:
            ...
            if self.rank < self.maxRank:
                if self.expTarget - self.expBase > 0:
            ...
            if self.expMeterText:
                ...
                if self.iconFrame:
                    ...
                    img = icName and self.iconModel.find('**/' + icName)
                else:
                    img = None
            ...
            (self.rank == 0 or not Freebooter.getPaidStatus(localAvatar.doId)) and self.landButton.hide()
            self.seaButton.hide()
        else:
...
class TitlesPage(InventoryPage.InventoryPage):
    ...
    def shouldShowTitle(self, titleId):
        if titleId == TitleGlobals.FounderTitle:
            return not localAvatar.getFounder() and 0
        else:
            if titleId == TitleGlobals.ShipPVPTitle:
                return not base.config.GetBool('want-sea-infamy', 0) and 0
            else:
                if titleId == TitleGlobals.LandPVPTitle and not base.config.GetBool('want-land-infamy', 0):
                    return 0
        return 1

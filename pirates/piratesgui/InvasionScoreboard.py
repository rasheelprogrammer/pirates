...
# Embedded file name: pirates.piratesgui.InvasionScoreboard
...
class InvasionScoreboard(DirectFrame):
    ...
    def __init__(self, holidayId, wonInvasion, reputationEarned, enemiesKilled, barricadesSaved, wavesCleared):
        ...
        if localAvatar.getParentObj():
            if localAvatar.getParentObj().getUniqueId() == InvasionGlobals.getIslandId(self.holidayId):
                ...
                waxSealImage = self.onIsland and self.wonInvasion and OnscreenImage(parent=self, image=wax_seal, pos=(1.27, 0, -0.95), scale=1.0)
                self.fireSeq = None
            else:

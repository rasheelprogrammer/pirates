...
class GuiManager(FSM.FSM):
    ...
    def doInstructionMessageQueue(self, task=None):
        ...
        if len(self.instructionMessageQueue) > 0:
            ...
            if not base.firstMateVoiceOn:
                if messageCategory in [MessageGlobals.MSG_CAT_THREAT_LEVEL, MessageGlobals.MSG_CAT_NO_PORT, MessageGlobals.MSG_CAT_TELL_PORT, MessageGlobals.MSG_CAT_ANNOUNCE_ATTACK, MessageGlobals.MSG_CAT_SUNK_SHIP]:
                    pass
                else:
                ...
                self.instructionMessageIcon['image'] = messageImage and messageImage
    ...
    def lockInstructionMessage(self, lockingObj, messageText, messageSoundList=[], messageGraphic=None, graphicScale=1.0, messageCategory=MessageGlobals.MSG_CAT_DEFAULT):
        ...
        if lockingObj:
            if self.instructionMessageLockObj == None:
            ...
            self.instructionMessageBoxText.setColorScale(msgCatInfo['text_fg'])
            self.instructionMessageText['text_wordwrap'] = self.iMBsizeX * 0.9 / msgCatInfo['text_scale']
            self.instructionMessageIcon['image'] = messageGraphic and messageImage
            self.instructionMessageIcon['image_scale'] = messageImageScale * 0.15
            self.instructionMessageIcon['image_pos'] = (0.08, 0.0, self.iMBsizeZ * 0.5)
            self.instructionMessageIcon.show()
            self.instructionMessageText['text_pos'] = (self.iMBsizeX * 0.57, 0.12)
    ...
    def showTrays(self):
        ...
        if self.getTutorialStatus() >= PiratesGlobals.TUT_GOT_SEACHEST:
            ...
            if self.contextTutPanel.isFilled():
                ...
                self.prevTag and not Freebooter.AllAccessHoliday and self.prevTag.show()
        if self.getTutorialStatus() >= PiratesGlobals.TUT_GOT_COMPASS:
            self.radarGui.show()
    ...
    def togglePage(self, page, args=None):
        ...
        if self.getTutorialStatus() < PiratesGlobals.TUT_MET_JOLLY_ROGER:
            ...
            if self.ignoreAllButLookoutHotKey and page != self.lookoutPage:
                return False
            self.hideMinimap()
            self.pageOpen(page) and self.hideSeaChest()
            return False
        else:
    ...
    def pageOpen(self, page):
        if self.chestPanel.getCurPage() == page:
            return self.chestPanel.isActive() and 1
        else:
            return 0
 ...
 True, True]):
    ...
    def toggleMainMenu(self, args=None):
        ...
        if args:
            if args == 'escape' or args == 'Esc':
                if self.ignoreEscapeHotKey or self.av.gameFSM.state in ('Battle', 'Cutscene',
                                                                        'Cannon',
                                                                        'Dialog',
                                                                        'NPCInteract',
                                                                        'ShipPilot',
                                                                        ...
                                                                        'Fishing'):
                    return
                if self.av.gameFSM.state in ('TeleportIn', 'TeleportOut', 'EnterTunnel',
                                             'Digging', 'Searching', 'ShipRepair',
                                             'BenchRepair', 'DoorKicking'):
                    return
                ...
                if not self.mainMenu:
                    width = base.config.GetBool('want-custom-keys', 0) and 1.8
                else:
                    width = 1.6
                height = 1.6
                x = -width / 2
                ...
                self.mainMenu.hide()
            self.mainMenu.isHidden() and self.socialPanel.hide()
            self.chatPanel.hide()
            self.av.chatMgr.stop()
            self.av.stopChat()
            self.av.stopAutoRun()
    ...
    def handleMinimapKeyDown(self):
        if base.config.GetBool('want-momentary-minimap', 1):
            if self.minimap:
                self.minimap.allowOnScreen() and self.minimap.request('Opaque')
            else:
                self.createWarning(PLocalizer.MinimapNotAvailable, duration=3)
        else:
            self.nextMinimap()
    ...
    def toggleGuiForNpcInteraction--- This code section failed: ---

4252       0  LOAD_FAST             1  'state'
           3  LOAD_CONST            1  0
           6  COMPARE_OP            2  '=='
           9  JUMP_IF_FALSE       287  'to 299'
          12  POP_TOP          

4253      13  LOAD_FAST             0  'self'
          16  LOAD_ATTR             2  'hideSeaChest'
          19  CALL_FUNCTION_0       0  None
          22  POP_TOP          

4254      23  LOAD_FAST             0  'self'
          26  LOAD_ATTR             3  'gameGui'
          29  LOAD_ATTR             4  'hide'
          32  CALL_FUNCTION_0       0  None
          35  POP_TOP          

4255      36  LOAD_FAST             0  'self'
          39  LOAD_ATTR             5  'chatPanel'
          42  LOAD_ATTR             4  'hide'
          45  CALL_FUNCTION_0       0  None
          48  POP_TOP          

4256      49  LOAD_FAST             0  'self'
          52  LOAD_ATTR             6  'radarGui'
          55  LOAD_ATTR             4  'hide'
          58  CALL_FUNCTION_0       0  None
          61  POP_TOP          

4257      62  LOAD_FAST             0  'self'
          65  LOAD_ATTR             7  'combatTray'
          68  LOAD_ATTR             4  'hide'
          71  CALL_FUNCTION_0       0  None
          74  POP_TOP          

4258      75  LOAD_FAST             0  'self'
          78  LOAD_ATTR             8  'moneyDisplay'
          81  LOAD_ATTR             4  'hide'
          84  CALL_FUNCTION_0       0  None
          87  POP_TOP          

4259      88  LOAD_FAST             0  'self'
          91  LOAD_ATTR             9  'chestTray'
          94  LOAD_ATTR            10  'hideChestButton'
          97  CALL_FUNCTION_0       0  None
         100  POP_TOP          

4260     101  LOAD_FAST             0  'self'
         104  LOAD_ATTR            11  'contextTutPanel'
         107  LOAD_ATTR             4  'hide'
         110  CALL_FUNCTION_0       0  None
         113  POP_TOP          

4261     114  LOAD_FAST             0  'self'
         117  LOAD_ATTR            12  'socialPanel'
         120  LOAD_ATTR            13  'isHidden'
         123  CALL_FUNCTION_0       0  None
         126  JUMP_IF_TRUE         26  'to 155'
       129_0  THEN                     156
         129  POP_TOP          

4262     130  LOAD_FAST             0  'self'
         133  LOAD_ATTR            12  'socialPanel'
         136  LOAD_ATTR             4  'hide'
         139  CALL_FUNCTION_0       0  None
         142  POP_TOP          

4264     143  LOAD_CONST            2  1
         146  LOAD_FAST             0  'self'
         149  STORE_ATTR           14  '_putBackSocialPanel'
         152  JUMP_FORWARD          1  'to 156'
       155_0  COME_FROM           126  '126'
         155  POP_TOP          
       156_0  COME_FROM           152  '152'

4265     156  LOAD_FAST             0  'self'
         159  LOAD_ATTR            15  'tmButtonQuick'
         162  JUMP_IF_FALSE        17  'to 182'
       165_0  THEN                     183
         165  POP_TOP          

4266     166  LOAD_FAST             0  'self'
         169  LOAD_ATTR            15  'tmButtonQuick'
         172  LOAD_ATTR             4  'hide'
         175  CALL_FUNCTION_0       0  None
         178  POP_TOP          
         179  JUMP_FORWARD          1  'to 183'
       182_0  COME_FROM           162  '162'
         182  POP_TOP          
       183_0  COME_FROM           179  '179'

4267     183  LOAD_FAST             0  'self'
         186  LOAD_ATTR            16  'tmButtonSearch'
         189  JUMP_IF_FALSE        17  'to 209'
       192_0  THEN                     210
         192  POP_TOP          

4268     193  LOAD_FAST             0  'self'
         196  LOAD_ATTR            16  'tmButtonSearch'
         199  LOAD_ATTR             4  'hide'
         202  CALL_FUNCTION_0       0  None
         205  POP_TOP          
         206  JUMP_FORWARD          1  'to 210'
       209_0  COME_FROM           189  '189'
         209  POP_TOP          
       210_0  COME_FROM           206  '206'

4269     210  LOAD_FAST             0  'self'
         213  LOAD_ATTR            17  'questPage'
         216  JUMP_IF_FALSE        14  'to 233'
       219_0  THEN                     234
         219  POP_TOP          

4270     220  LOAD_FAST             0  'self'
         223  LOAD_ATTR            18  'hideTrackedQuestInfo'
         226  CALL_FUNCTION_0       0  None
         229  POP_TOP          
         230  JUMP_FORWARD          1  'to 234'
       233_0  COME_FROM           216  '216'
         233  POP_TOP          
       234_0  COME_FROM           230  '230'

4271     234  LOAD_FAST             0  'self'
         237  LOAD_ATTR            19  'crewHUD'
         240  LOAD_ATTR            20  'hudOn'
         243  JUMP_IF_FALSE        26  'to 272'
       246_0  THEN                     273
         246  POP_TOP          

4272     247  LOAD_FAST             0  'self'
         250  LOAD_ATTR            19  'crewHUD'
         253  LOAD_ATTR            21  'setHUDOff'
         256  CALL_FUNCTION_0       0  None
         259  POP_TOP          

4274     260  LOAD_GLOBAL          22  'True'
         263  LOAD_FAST             0  'self'
         266  STORE_ATTR           23  'crewHUDTurnedOff'
         269  JUMP_FORWARD          1  'to 273'
       272_0  COME_FROM           243  '243'
         272  POP_TOP          
       273_0  COME_FROM           269  '269'

4275     273  LOAD_FAST             0  'self'
         276  LOAD_ATTR            24  'hideMinimap'
         279  CALL_FUNCTION_0       0  None
         282  POP_TOP          

4276     283  LOAD_FAST             0  'self'
         286  LOAD_ATTR            25  'profilePage'
         289  LOAD_ATTR             4  'hide'
         292  CALL_FUNCTION_0       0  None
         295  POP_TOP          
         296  JUMP_FORWARD        357  'to 656'
       299_0  COME_FROM             9  '9'
         299  POP_TOP          

4280     300  LOAD_FAST             0  'self'
         303  LOAD_ATTR             2  'hideSeaChest'
         306  CALL_FUNCTION_0       0  None
         309  POP_TOP          

4281     310  LOAD_FAST             0  'self'
         313  LOAD_ATTR             3  'gameGui'
         316  LOAD_ATTR            26  'show'
         319  CALL_FUNCTION_0       0  None
         322  POP_TOP          

4282     323  LOAD_FAST             0  'self'
         326  LOAD_ATTR             5  'chatPanel'
         329  LOAD_ATTR            26  'show'
         332  CALL_FUNCTION_0       0  None
         335  POP_TOP          

4283     336  LOAD_FAST             0  'self'
         339  LOAD_ATTR             7  'combatTray'
         342  LOAD_ATTR            26  'show'
         345  CALL_FUNCTION_0       0  None
         348  POP_TOP          

4284     349  LOAD_FAST             0  'self'
         352  LOAD_ATTR             9  'chestTray'
         355  LOAD_ATTR            27  'showChestButton'
         358  CALL_FUNCTION_0       0  None
         361  POP_TOP          

4285     362  LOAD_FAST             0  'self'
         365  LOAD_ATTR            11  'contextTutPanel'
         368  LOAD_ATTR            28  'isFilled'
         371  CALL_FUNCTION_0       0  None
         374  JUMP_IF_FALSE        17  'to 394'
       377_0  THEN                     395
         377  POP_TOP          

4286     378  LOAD_FAST             0  'self'
         381  LOAD_ATTR            11  'contextTutPanel'
         384  LOAD_ATTR            26  'show'
         387  CALL_FUNCTION_0       0  None
         390  POP_TOP          
         391  JUMP_FORWARD          1  'to 395'
       394_0  COME_FROM           374  '374'
         394  POP_TOP          
       395_0  COME_FROM           391  '391'

4287     395  LOAD_FAST             0  'self'
         398  LOAD_ATTR            14  '_putBackSocialPanel'
         401  JUMP_IF_FALSE        26  'to 430'
         404  POP_TOP          

4288     405  LOAD_FAST             0  'self'
         408  LOAD_ATTR            12  'socialPanel'
         411  LOAD_ATTR            26  'show'
         414  CALL_FUNCTION_0       0  None
         417  POP_TOP          

4289     418  LOAD_CONST            1  0
         421  LOAD_FAST             0  'self'
         424  STORE_ATTR           14  '_putBackSocialPanel'
         427  JUMP_ABSOLUTE       435  'to 435'
       430_0  COME_FROM           401  '401'
         430  POP_TOP          
         431  JUMP_FORWARD          1  'to 435'
         434  POP_TOP          
       435_0  COME_FROM           431  '431'

4290     435  LOAD_FAST             0  'self'
         438  LOAD_ATTR            29  'getTutorialStatus'
         441  CALL_FUNCTION_0       0  None
         444  LOAD_GLOBAL          30  'PiratesGlobals'
         447  LOAD_ATTR            31  'TUT_GOT_COMPASS'
         450  COMPARE_OP            5  '>='
         453  JUMP_IF_FALSE       118  'to 574'
       456_0  THEN                     575
         456  POP_TOP          

4291     457  LOAD_FAST             0  'self'
         460  LOAD_ATTR             6  'radarGui'
         463  LOAD_ATTR            26  'show'
         466  CALL_FUNCTION_0       0  None
         469  POP_TOP          

4292     470  LOAD_FAST             0  'self'
         473  LOAD_ATTR            15  'tmButtonQuick'
         476  JUMP_IF_FALSE        17  'to 496'
       479_0  THEN                     497
         479  POP_TOP          

4293     480  LOAD_FAST             0  'self'
         483  LOAD_ATTR            15  'tmButtonQuick'
         486  LOAD_ATTR            26  'show'
         489  CALL_FUNCTION_0       0  None
         492  POP_TOP          
         493  JUMP_FORWARD          1  'to 497'
       496_0  COME_FROM           476  '476'
         496  POP_TOP          
       497_0  COME_FROM           493  '493'

4294     497  LOAD_FAST             0  'self'
         500  LOAD_ATTR            16  'tmButtonSearch'
         503  JUMP_IF_FALSE        17  'to 523'
       506_0  THEN                     524
         506  POP_TOP          

4295     507  LOAD_FAST             0  'self'
         510  LOAD_ATTR            16  'tmButtonSearch'
         513  LOAD_ATTR            26  'show'
         516  CALL_FUNCTION_0       0  None
         519  POP_TOP          
...
4296     524  LOAD_GLOBAL          32  'len'
         527  LOAD_FAST             0  'self'
         530  LOAD_ATTR            33  'trackedQuestLabel'
         533  LOAD_CONST            3  'text'
         536  BINARY_SUBSCR    
         537  JUMP_IF_FALSE        10  'to 550'
         540  POP_TOP          
         541  LOAD_FAST             0  'self'
         544  LOAD_ATTR            34  'av'
         547  LOAD_ATTR            35  'activeQuestId'
       550_0  COME_FROM           537  '537'
         550  CALL_FUNCTION_1       1  None
         553  JUMP_IF_FALSE        14  'to 570'
       556_0  THEN                     571
         556  POP_TOP          

4297     557  LOAD_FAST             0  'self'
         560  LOAD_ATTR            36  'showTrackedQuestInfo'
         563  CALL_FUNCTION_0       0  None
         566  POP_TOP          
         567  JUMP_ABSOLUTE       575  'to 575'
       570_0  COME_FROM           553  '553'
         570  POP_TOP          
         571  JUMP_FORWARD          1  'to 575'
       574_0  COME_FROM           453  '453'
         574  POP_TOP          
       575_0  COME_FROM           571  '571'

4298     575  LOAD_FAST             0  'self'
         578  LOAD_ATTR            23  'crewHUDTurnedOff'
         581  JUMP_IF_FALSE        71  'to 655'
         584  POP_TOP          
         585  LOAD_FAST             0  'self'
         588  LOAD_ATTR            29  'getTutorialStatus'
         591  CALL_FUNCTION_0       0  None
         594  LOAD_GLOBAL          30  'PiratesGlobals'
         597  LOAD_ATTR            37  'TUT_MET_JOLLY_ROGER'
         600  COMPARE_OP            5  '>='
         603  JUMP_IF_TRUE         23  'to 629'
         606  POP_TOP          
         607  LOAD_FAST             0  'self'
         610  LOAD_ATTR            38  'forceLookout'
         613  JUMP_IF_TRUE         13  'to 629'
         616  POP_TOP          
         617  LOAD_FAST             0  'self'
         620  LOAD_ATTR            19  'crewHUD'
         623  LOAD_ATTR            39  'crew'
       626_0  COME_FROM           613  '613'
       626_1  COME_FROM           603  '603'
         626  JUMP_IF_FALSE        26  'to 655'
       629_0  THEN                     656
         629  POP_TOP          

4301     630  LOAD_FAST             0  'self'
         633  LOAD_ATTR            19  'crewHUD'
         636  LOAD_ATTR            40  'setHUDOn'
         639  CALL_FUNCTION_0       0  None
         642  POP_TOP          

4303     643  LOAD_GLOBAL          41  'False'
         646  LOAD_FAST             0  'self'
         649  STORE_ATTR           23  'crewHUDTurnedOff'
         652  JUMP_FORWARD          1  'to 656'
       655_0  COME_FROM           626  '626'
       655_1  COME_FROM           581  '581'
         655  POP_TOP          
       656_0  COME_FROM           652  '652'
       656_1  COME_FROM           296  '296'

Parse error at or near `JUMP_ABSOLUTE' instruction at offset 427
        ...
        if localAvatar.activeQuestId:
            ...
            if quest:
                ...
                for currQuestTask in questTasks:
                    if isinstance(currQuestTask, DowsingRodTaskDNA):
                        not quest.isComplete() and self.addDowsingButton()
                        break

                if quest.getQuestId() == 'rc.ghosts.clubhearts.disguise':
                    if not quest.isComplete():
                        self.addCheckPortraitButton()
            text and not self.gameGui.isHidden() and self.showTrackedQuestInfo()
        else:
            if not text:
                self.hideTrackedQuestInfo()

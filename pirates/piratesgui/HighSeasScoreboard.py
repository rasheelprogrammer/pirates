...
class HighSeasScoreboard(GuiPanel.GuiPanel):
    ...
    def getCargoResults(self):
        ...
        if base.localAvatar.ship:
            ...
            len(pCargo) == 0 and self.results.append({'Type': 'Entry', 'Text': PLocalizer.NoCargoLooted, 'Value1': '', 'UnwrapMode': 1})
        else:
            cargoDict = {}
    ...
    def getLootLabel(self, lootType):
        ...
        if lootType == PiratesGlobals.ITEM_SAC:
            ...
            if lootType == PiratesGlobals.TREASURE_CHEST:
                gridText = PLocalizer.LootContainerTreasureChest
            else:
                if lootType == PiratesGlobals.RARE_CHEST:
                    gridText = PLocalizer.LootContainerRareChest
                else:
                    if lootType == PiratesGlobals.UPGRADE_CHEST:
                        gridText = PLocalizer.LootContainerUpgradeChest
                    else:
                        if lootType == PiratesGlobals.RARE_UPGRADE_CHEST:
    ...
    def checkForAddLootInfoText(self):
        if not self.addedLootInfoText:
            self.addedLootInfoText = self.rightPanel and 1
                ...
                for cell in grid.cellList:
                    if cell.inventoryItem:
                        if cell.inventoryItem.itemTuple[0] in checkList:
                            itemInfo = ((not stackOnly or cell.inventoryItem.canStack) and cell).inventoryItem.itemTuple
    ...
    def requestItem(self, item):
        ...
        for lootBox in pLootBoxes:
            for lootInfo in lootBox[1]:
                if item[0] == lootInfo[0]:
                    item[1] == lootInfo[1] and item[2] == lootInfo[2] and base.cr.lootMgr.d_requestItemFromContainer(lootBox[0], item)
                    return
    ...
    def requestItems(self, items):
        ...
        for item in items:
            for lootBox in pLootBoxes:
                for lootInfo in lootBox[1]:
                    if item[0] == lootInfo[0]:
                        if item[1] == lootInfo[1]:
                            item[2] == lootInfo[2] and lootBox[0] in containers and containers[lootBox[0]].append(item)
                        else:

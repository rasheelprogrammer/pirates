...
class HoverFrame(DirectFrame):
    ...
    def hoverTextUpdated(self):
        if self.helpBox:
            self.helpBox['text'] != self['hoverText'] and self.helpBox.destroy()
            self.createHelpBox()
        else:
            if self['hoverText']:
                self.createHelpBox()
...
class CrewHUD(SocialPage.SocialPage):
        ...
        if self.crew or self.startACrewState:
            if not self.initialStateSwitch:
                if DistributedBandMember.DistributedBandMember.IsLocalAvatarHeadOfBand():
                    ...
                    if localAvatar.guiMgr.crewInviter and not localAvatar.guiMgr.crewInviter.isEmpty():
    ...
    def updateActionIcons(self, avId, action, siege, vitaeLevel):
        ...
        if member:
            if member[0].hudOnline:
                if not member[0].inPvp:
                    if not member[0].inParlorGame:
                        if not member[0].potentialMember:
                            skillFrame = member[2]
                            ...
                            if siege:
                                siegeFrame['image'] = (siege == 2 and self).spanishFlag
                            ...
                            siegeFrame.hide()
                            siegeFrameSea.hide()
                        vitaeLevel and clampFrame.show()
                        clampFrameSea.show()
                    else:
                        clampFrame.hide()
                        clampFrameSea.hide()
                    newIcon = HUD_ICONS[action]
                    skillFrame['image'] = newIcon == None and None
                    skillFrameSea['image'] = None
                    skillFrame['text'] = (action in [12] and PLocalizer).CrewHUDAFK
        ...
        if not self.crew:
            ...
            if not self.joinACrewStatus:
                if not self.joinACrewStatusPVP:
                    self.canShowButtons() and self.atSea and self.startCrewButtonSea.show()
        ...
        if DistributedBandMember.DistributedBandMember.IsLocalAvatarHeadOfBand():
            if flag:
                if self.canShowButtons():
                    ...
                    self.lookoutButtonSea.show()
                    self.lookoutFrameSea.show()
                self.lookoutButton['image'] = (self.recruitCrewMatesStatus and self.topGui.find('**/pir_t_gui_but_circle'), self.topGui.find('**/pir_t_gui_but_circle'), self.topGui.find('**/pir_t_gui_but_circle_over'), self.topGui.find('**/pir_t_gui_but_circle'))
            ...
            for avId, hudButton in self.crew.iteritems():
                if not hudButton[0].potentialMember:
                    (not hudButton[0].hudOnline or hudButton[0].inPvp or hudButton[0].inParlorGame) and hudButton[10].show()
                    hudButton[11].show()

        else:
            if not flag or not self.canShowButtons():
                self.lookoutButton.hide()
                self.lookoutFrame.hide()
                self.lookoutButtonSea.hide()
                self.lookoutFrameSea.hide()
    ...
    def b_activateCrewLookout(self, range, sailValue=0, cannonValue=0):
        if self.crew or self.startACrewState:
            self.canShowButtons() and base.cr.crewMatchManager.addCrewToLookoutList(range, sailValue, cannonValue)

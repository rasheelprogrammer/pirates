    ...
    def determineOptionsButtonsState(self):
        ...
        if self.addCrewLookout:
            if self.crewHUD.crew:
                ...
                self.crewHUDToggleButton['state'] = (not self.crewHUD.crew and self.crewHUD.inTM and DGG).DISABLED
            else:
                self.crewHUDToggleButton['state'] = DGG.NORMAL
            self.selectCrewIcon['state'] = (self.crewHUD.crew and DGG).NORMAL

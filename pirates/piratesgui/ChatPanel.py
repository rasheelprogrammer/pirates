...
# Embedded file name: pirates.piratesgui.ChatPanel
...
class ChatPanel(DirectFrame, FSM):
        ...
        if wantReceiver:
            ...
            if message.getTalkType() == INFO_OPEN:
                ...
                if message.getTalkType() == INFO_DEV:
                    ...
                    if message.getTalkType() in (TALK_OPEN, TALK_WHISPER, TALK_ACCOUNT, AVATAR_THOUGHT):
                            ...
                            if message.getTalkType() == TALK_GUILD:
                                ...
                                if message.getTalkType() == UPDATE_GUILD:
                                    ...
                                    if message.getTalkType() == TALK_PARTY:
                                        ...
                                        if message.getTalkType() == UPDATE_PARTY:
                                            useName = '\x01Super\x01.\x02 [' + PLocalizer.TalkCrewLabel + '] ' + plainName
                                        else:
                                            if message.getTalkType() == TALK_PVP:
                                                useName = '[' + extraInfo + '] ' + plainName + divider
                                            else:
                                                if message.getTalkType() == UPDATE_PVP:
    ...
    def decodeOpenMessage(self, message):
        ...
        if message.getSenderAvatarId() == localAvatar.doId:
            ...
            if message.getTalkType() == TALK_ACCOUNT:
                if message.getSenderAccountId():
                    buttonCommand = (message.getSenderAccountId() == base.cr.accountDetailRecord.playerAccountId and self).handlePlayerPress
        ...
        if message.getTalkType() == INFO_GUILD:
            ...
            chatString = ''
            someMessage = '%s' in message.getBody() and message.getTalkType() == INFO_OPEN and message.getBody() % messageName
        else:
            if '%s' in message.getBody() and message.getTalkType() == INFO_GUILD:
                if wantTwoNames:
                    if extraInfo:

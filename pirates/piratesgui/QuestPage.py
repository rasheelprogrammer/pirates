...
class QuestPage(InventoryPage.InventoryPage):
    ...
    def updateQuestTitles(self, quest=None, newQuest=False, findNewTrackable=True):
        ...
        if not self.detailId:
            ...
            if self.detailId not in questIds:
                self.detailId = questIds and None
            ...
            if not self.detailFrame.hasQuestDetails() or localAvatar.activeQuestId and self.detailId != localAvatar.activeQuestId:
    ...
    def addTreasureMapButtons(self, tm, buttonOffset):
        ...
        if __debug__:
            if base.config.GetBool('enable-bp-solo', False):
            ...
            base.cr.teleportMgr.inInstanceType == PiratesGlobals.INSTANCE_TM and self.disableTreasureMapButtons()
        else:

...
def ImageScale(repId):
    if repId == InventoryType.PistolRep:
        ...
        if repId == InventoryType.SailingRep:
            ...
            if repId == InventoryType.CannonRep:
                return 0.14
            else:
...
def getSkillIconName(skillId, frame):
    ...
    if skillId == InventoryType.CutlassRep:
        ...
        if skillId == InventoryType.SailingRep:
            return 'sail_full_sail'
        else:
            if skillId == InventoryType.CannonRep:
                return 'pir_t_ico_can_single'
            else:
                if skillId == InventoryType.WandRep:
                    return 'pir_t_ico_stf_dark_a'
                else:
                    if skillId == InventoryType.MeleeRep:
                        return 'pir_t_ico_swd_broadsword_b'
                    else:
                        if skillId == InventoryType.DaggerRep:
                            return 'pir_t_ico_knf_small'
                        else:
                            if skillId == InventoryType.GrenadeRep:
                                return 'pir_t_ico_bom_grenade'
                            else:
                                if skillId == InventoryType.PistolRep:
                                    return 'pir_t_ico_gun_pistol_a'
                                else:
                                    if skillId == InventoryType.DollRep:
                                        if frame == 0:
                ...
                if repId == InventoryType.GrenadeRep:
                    ...
                    if repId == InventoryType.DollRep:
                        ...
                        if repId == InventoryType.WandRep:
                            ...
                            if repId == InventoryType.KettleRep:
                                ...
                                if repId == InventoryType.PistolRep:
                                    ...
                                    if repId == InventoryType.CannonRep:
                                        ...
                                        if repId == InventoryType.SailingRep:
                                            ...
                                            if repId == InventoryType.DefenseCannonRep:
...
class RadialMenu:
    ...
    def __init__(self, rep, weaponMode):
        ...
        if self.rep == InventoryType.DollRep:
            asset = localAvatar.hasStickyTargets() and getSkillIconName(self.rep, 1)
        else:
    ...
    def radialMenuHeartBeat(self, task):
        ...
        for skillId in self.radialSkillMap:
            ...
            if self.rep == InventoryType.DollRep:
                ...
                if haveFriendly:
                    if not haveHostile:
                        greyOut = WeaponGlobals.isFriendlyFire(self.radialSkillMap[count - 1]) or 1
                else:
                    if not haveFriendly:
                        if haveHostile:
                            if WeaponGlobals.isFriendlyFire(self.radialSkillMap[count - 1]):
                                greyOut = 1
            elif self.rstatus[count] != greyOut:
...
class SkillTray:
    ...
    def showSkillTray(self, task=None):
        if localAvatar.isWeaponDrawn == False:
            if localAvatar.gameFSM.state != 'ShipPilot':
                if not localAvatar.cannon:
                    return
            return self.skillTrayState and None
        else:
    ...
    def updateSkillTray(self, rep, weaponMode, callback=None, hideFirst=True):
        ...
        if self.skillTrayState:
            ...
            weaponMode == WeaponGlobals.DEFENSE_CANNON and self.setMoveUpValue(0.345)
        else:
            self.resetMoveUpVale()
        self.showSkillTray()
        return
            ...
            if rep == InventoryType.DollRep:
                ...
                if haveFriendly:
                    if not haveHostile:
                        ...
                        self.isPowerRecharged and self.continuePowerRechargeEffect()
    ...
    def updateSkillIval(self, skillId):
        for button in self.tray:
            if isinstance(self.tray[button], SkillButton):
                if not self.tray[button].isEmpty():
                    self.tray[button].skillId == skillId and (self.tray[button].skillRingIval.isPlaying() or self.tray[button].skillRingIval.start())
    ...
    def addPowerRechargeEffect(self):
        ...
        for button in self.tray:
            if isinstance(self.tray[button], SkillButton):
                if not self.tray[button].isEmpty():
                    (WeaponGlobals.getIsShipSkill(self.tray[button].skillId) or WeaponGlobals.getIsCannonSkill(self.tray[button].skillId)) and self.tray[button].skillId != InventoryType.SailPowerRecharge and self.tray[button].quickGlowImpulse()
    ...
    def continuePowerRechargeEffect(self):
        for button in self.tray:
            if isinstance(self.tray[button], SkillButton):
                if not self.tray[button].isEmpty():
                    (WeaponGlobals.getIsShipSkill(self.tray[button].skillId) or WeaponGlobals.getIsCannonSkill(self.tray[button].skillId)) and self.tray[button].skillId != InventoryType.SailPowerRecharge and self.tray[button].startPowerImpulse()
    ...
    def removePowerRechargeEffect(self):
        ...
        for button in self.tray:
            if isinstance(self.tray[button], SkillButton):
                if not self.tray[button].isEmpty():
                    (WeaponGlobals.getIsShipSkill(self.tray[button].skillId) or WeaponGlobals.getIsCannonSkill(self.tray[button].skillId)) and self.tray[button].skillId != InventoryType.SailPowerRecharge and self.tray[button].stopPowerImpulse()
                    self.tray[button].updateSkillRingIval()
    ...
    def updateCharmSkills(self):
        ...
        for button in self.tray:
            if isinstance(self.tray[button], SkillButton):
                if not self.tray[button].isEmpty():
                    (WeaponGlobals.getIsShipSkill(self.tray[button].skillId) or WeaponGlobals.getIsCannonSkill(self.tray[button].skillId)) and self.tray[button].skillId != InventoryType.SailPowerRecharge and self.tray[button].updateSkillRingIval()

...
def getGeomScale(repId, skillId=0):
    if repId == InventoryType.PistolRep:
        return WeaponGlobals.getSkillType(skillId) == WeaponGlobals.AMMO_SKILL and 0.18
    else:
        return 0.12
...
class SkillButton(DirectFrame):
    ...
    def __init__(self, skillId, callback, quantity=0, skillRank=0, showQuantity=False, showHelp=False, showRing=False, hotkey=None, name='', showIcon=True, showLock=False, rechargeSkillId=False, isWeaponSkill=False, assocAmmo=[]):
        ...
        if self.skillId >= InventoryType.begin_Consumables:
            if self.skillId <= InventoryType.end_Consumables:
                self.totalRechargeTime = not WeaponGlobals.getSkillEffectFlag(skillId) and base.cr.battleMgr.getModifiedRechargeTime(localAvatar, InventoryType.UseItem)
            ...
            if not (showRing and self).isBreakAttackSkill:
                self.createSkillRingIval()
            timeSpentRecharging = self.tonicId and localAvatar.skillDiary.getTimeSpentRecharging(InventoryType.UseItem)
        else:
            timeSpentRecharging = localAvatar.skillDiary.getTimeSpentRecharging(self.skillId)
        if self.isBreakAttackSkill:
            if timeSpentRecharging < self.totalRechargeTime:
                self.updateRechargeRing()
            elif not self.isBreakAttackSkill:
                self.totalRechargeTime and timeSpentRecharging and not timeSpentRecharging > self.totalRechargeTime and self.skillRingIval.start(startT=timeSpentRecharging)
            else:
    ...
    def checkAmount(self):
        if not self.quantity:
            if self.showQuantity:
                self.showRing and not self.skillRingIval.isPlaying() and self.setGeomColor(0.5, 0.5, 0.5, 1.0)
    ...
    def updateSkillRingIval(self):
        if self.showRing:
            ...
            self.createSkillRingIval()
            if playing:
                timeSpentRecharging and self.skillRingIval.start(startT=timeSpentRecharging)
    ...
    def updateRechargeRing(self):
        ...
        self.totalRechargeTime = base.cr.battleMgr.getModifiedRechargeTime(localAvatar, self.skillId)
        if timeSpentRecharging >= self.totalRechargeTime:
            self.rechargeFilled = not self.rechargeFilled and 1
            if self.skillRingIval:
                self.skillRingIval.pause()
                self.skillRingIval = None
            self.skillRing.meterFaceHalf1.setR(0)
        ...
        if self.showQuantity:
            ...
            if self.isBreakAttackSkill:
                ...
                if self.isDefenseSkill:
                    ...
                    if skillId == ItemGlobals.getSpecialAttack(localAvatar.currentWeaponId):
                        specialIconId = 3
            if specialIconId:
    ...
    def createHelpFrame(self, args=None):
        ...
        try:
            ...
            if self.showUpgrade:
                ...
                if self.skillId in SkillComboReq:
                    if SkillComboReq[self.skillId] and inv.getStackQuantity(self.skillId - 1) < 2:
                        color = '\x01red\x01'
                        if rank == 0:
                            color = '\x01red\x01'
                            upgradeInfo = ''
                    ...
                    damage and loDamage and stats.append(abs(loDamage))
                ...
                if skillInfo[4]:
                    ...
                    if upgradeAmt < 1.0 and upgradeAmt > 0:
                        bonus *= 100
                    self.skillId == InventoryType.SailTreasureSense and bonus /= 2.0
                else:
                    if self.skillId == InventoryType.CutlassParry:
    ...
    def updateQuantity(self, quantity):
        ...
        if quantity == WeaponGlobals.INF_QUANT:
            ...
            if self.assocAmmo:
                ...
                text = 'x%s' % quantity
        if self.quantityLabel:
            self.quantityLabel['text'] = not self.quantityLabel.isEmpty() and text
        else:
...
#                             attribute (2)
#                                  0. expr
#                                     L. 862    2232  LOAD_FAST             0  'self'
#                                  1.           2235  LOAD_ATTR            13  'skillId'
#                          1. expr
#                             attribute (2)
#                                  0. expr
#                                               2238  LOAD_GLOBAL          39  'InventoryType'
#                                  1.           2241  LOAD_ATTR           107  'SailTreasureSense'
#                          2.           2244  COMPARE_OP            2  '=='
#              1. jmp_false (2)
#                  0.           2247  JUMP_IF_FALSE        14  'to 2264'
#                  1.           2250  POP_TOP          
#              2. expr
#                 L. 863    2251  LOAD_FAST            20  'bonus'
#              3. come_from_opt
#      1. expr
#                   2254  LOAD_CONST           20  2.0
#      2. inplace_op
#                   2257  INPLACE_DIVIDE   
#      3. store
#                   2258  STORE_FAST           20  'bonus'
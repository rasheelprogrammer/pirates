...
class CombatTray(GuiTray):
    ...
    def toggleWeapon(self, weaponId, slotId, args=None, fromWheel=0):
        ...
        if weaponVolley:
            ...
            if state in ('Spawn', 'Stunned', 'Death', 'WaterRoam', 'WaterTreasureRoam',
                         'Cutscene', 'MakeAPirate', 'TeleportOut', 'TeleportIn',
                         'ParlorGame', 'NPCInteract', 'ShipBoarding', 'Ensnared',
                         'Thrown', 'Knockdown', 'Unconcious', 'Injured', 'Dying',
                         'ThrownInJail', 'DoorKicking', 'EnterTunnel', 'LeaveTunnel',
                         'PVPWait', 'ShipPilot', 'Cannon', 'Fishing'):
                return
            if self.isMouseMode:
                return
            if not weaponId:
            ...
            if self.isDrawingWeapon or self.isUsingSkill or self.isCharging:
                ...
                return
            if fromWheel:
                if weaponId == localAvatar.currentWeaponId and localAvatar.currentWeaponSlotId == slotId:
                    self.skillTray.showSkillTray()
                    return
                EconomyGlobals.getItemCategory(weaponId) == ItemType.CONSUMABLE and WeaponGlobals.getSkillEffectFlag(skillId) and self.trySkill(InventoryType.UsePotion, weaponId, 0)
            else:
    ...
    def setCurrentWeapon(self, currentWeaponId, isWeaponDrawn, slotId=1):
        ...
        if currentWeaponId:
            if not self.skillsHidden:
                self.acceptInput()
            if ItemGlobals.getWeaponAttributes(currentWeaponId, ItemGlobals.BLOOD_FIRE):
                ...
                if currentWeaponId and isWeaponDrawn:
                    if not Freebooter.getPaidStatus(base.localAvatar.getDoId()):
                        Freebooter.allowedFreebooterWeapon(self.rep) and ItemGlobals.getClass(currentWeaponId) == InventoryType.ItemTypeWeapon and self.skillTray.updateSkillTray(repId, self.weaponMode, self.__skillTrayCallback)
            else:
            ...
            if self.rep == 0 or self.rep == InventoryType.MeleeRep:
                ...
                if ItemGlobals.getType(localAvatar.currentWeaponId) == ItemGlobals.QUEST_PROP:
                    ...
                    if rep:
                        ...
                        leftSkills = RadialMenu.ComboSkills(self.rep, 2)
                        rightSkills = RadialMenu.ActiveSkills(self.rep, 2)
            self.weaponMode = (self.rep == InventoryType.SailingRep and WeaponGlobals).SAILING
        else:
    ...
    def trySkill(self, skillId, ammoSkillId, combo=0, charge=0):
                    ...
                    if skillId in range(InventoryType.begin_WeaponSkillFishingRod, InventoryType.end_WeaponSkillFishingRod):
                        if localAvatar.fishingGameHook:
                            ...
                            if localAvatar.fishingGameHook.fsm.getCurrentOrNextState() in ['FishOnHook', 'ReelingFish', 'FishFighting']:
                                ...
                                if skillId in [InventoryType.FishingRodPull, InventoryType.FishingRodHeal, InventoryType.FishingRodTug]:
            ...
            if ammoItemId in tonicIds or skillId == EnemySkills.MISC_FIRST_AID:
                if localAvatar.getHp() == localAvatar.getMaxHp():
                    ...
                    if localAvatar.getGameState() == 'Cannon':
                        ...
                        return 0
                    localAvatar.getGameState() == 'ParlorGame' and localAvatar.guiMgr.createWarning(PLocalizer.TonicParlorGameWarning, PiratesGuiGlobals.TextFG6)
                    return 0
        ...
        if WeaponGlobals.getIsShipSkill(skillId):
            if localAvatar.ship:
                if skillId == InventoryType.SailRammingSpeed:
                    if localAvatar.ship.queryGameState() in 'InBoardingPosition':
                        localAvatar.guiMgr.createWarning(PLocalizer.RamWhileBoardingWarning, PiratesGuiGlobals.TextFG6)
                        return 0
                    if localAvatar.ship.isRamming():
                        if skillId != InventoryType.SailBroadsideLeft:
                            if skillId != InventoryType.SailBroadsideRight:
                                return 0
                    if skillId not in self.IGNORES_INPUT_LOCK:
                        if not WeaponGlobals.getIsShipSkill(skillId):
                            ...
                            if self.rep != skillRepCategory and skillRepCategory > 0:
                                return 0
                            ammoRepCategory = ammoSkillId and WeaponGlobals.getSkillReputationCategoryId(ammoSkillId)
                            return self.rep != ammoRepCategory and ammoRepCategory > 0 and 0
                skillEffects = localAvatar.getSkillEffects()
                self.skillQueue = (
                 WeaponGlobals.C_STUN in skillEffects and (skillId == EnemySkills.PISTOL_RELOAD or skillId == EnemySkills.GRENADE_RELOAD) and skillId, ammoSkillId, combo)
            else:
                localAvatar.guiMgr.createWarning(PLocalizer.StunWarning, PiratesGuiGlobals.TextFG6)
            return 0
        else:
            if WeaponGlobals.C_KNOCKDOWN in skillEffects:
        ...
        if not WeaponGlobals.getIsInstantSkill(skillId, ammoSkillId):
            if self.isUsingSkill:
                if skillId not in self.IGNORES_INPUT_LOCK:
                    if self.weaponMode == WeaponGlobals.COMBAT or self.weaponMode == WeaponGlobals.THROWING:
                        if skillId != EnemySkills.DOLL_UNATTUNE:
                            if not self.skillQueue:
                                if not self.weaponQueue:
                                    self.skillQueue = (
                    ...
                    if WeaponGlobals.getNeedSight(skillId, ammoSkillId):
                        ...
                        if rechargingTime > 0:
                            if skillId == InventoryType.UseItem:
                                ...
                                if skillId not in self.BASIC_ATTACKS:
                                    localAvatar.guiMgr.createWarning(PLocalizer.SkillRechargingWarning, PiratesGuiGlobals.TextFG6)
                            return 0
                        if ammoSkillId:
                            if not WeaponGlobals.isInfiniteAmmo(ammoSkillId):
                                if not WeaponGlobals.canUseInfiniteAmmo(localAvatar.currentWeaponId, ammoSkillId):
                                    inv = not WeaponGlobals.canUseInfiniteAmmo(localAvatar.getCurrentCharm(), ammoSkillId) and localAvatar.getInventory()
                                    if inv is None:
                                        return 0
                                    itemCat = ammoItemId
                                    if skillId in (InventoryType.UseItem, InventoryType.UsePotion):
                                    ...
                                    amt < 1 and EconomyGlobals.getItemCategory(ammoSkillId) == ItemType.AMMO and localAvatar.guiMgr.createWarning(PLocalizer.OutOfAmmoWarning, PiratesGuiGlobals.TextFG6)
                                else:
                    ...
                    if skillId:
                        if not WeaponGlobals.isInfiniteAmmo(skillId) and not WeaponGlobals.canUseInfiniteAmmo(localAvatar.currentWeaponId, skillId) and not WeaponGlobals.canUseInfiniteAmmo(localAvatar.getCurrentCharm(), skillId):
                            ...
                            amt < 1 and EconomyGlobals.getItemCategory(skillId) == ItemType.AMMO and localAvatar.guiMgr.createWarning(PLocalizer.OutOfAmmoWarning, PiratesGuiGlobals.TextFG6)
                        else:
                            localAvatar.guiMgr.createWarning(PLocalizer.OutOfItemWarning, PiratesGuiGlobals.TextFG6)
                        return 0
            ...
            if skillId == InventoryType.SailBroadsideLeft or skillId == InventoryType.SailBroadsideRight:
                if localAvatar.ship and not localAvatar.ship.broadside:
                    ...
                    return 0
            self.currentSkill = skillId
            visSkillId = ammoSkillId and skillId != EnemySkills.PISTOL_RELOAD and skillId != EnemySkills.GRENADE_RELOAD and WeaponGlobals.getSkillType(skillId) != WeaponGlobals.WEAPON_SKILL and ammoSkillId
        else:
            visSkillId = skillId
        prevSkillId = 0
        time = 0
        ...
        if localAvatar.wantComboTiming:
            if combo == -1:
                ...
                if prevSkillId in self.L1_COMBO_ATTACKS:
                    if skillId == prevSkillId:
                        if time <= self.BUTTON_MASH_WINDOW:
                            self.activeName['text'] = PLocalizer.Mistimed
                        else:
                        ...
                        if combo == -1:
                            ...
                            if prevSkillId in self.L1_COMBO_ATTACKS:
                                self.activeName['text'] = skillId == prevSkillId and time <= self.BUTTON_MASH_WINDOW and PLocalizer.InventoryTypeNames[visSkillId] + ' - ' + PLocalizer.Mistimed
                                localAvatar.mistimedAttack = 1
                            else:
                                if visSkillId in self.COMBO_ATTACKS:
                                    self.activeName['text'] = visSkillId not in self.L1_COMBO_ATTACKS and PLocalizer.InventoryTypeNames[visSkillId] + ' - ' + PLocalizer.Perfect
                                else:
                                    self.activeName['text'] = PLocalizer.InventoryTypeNames[visSkillId]
                        ...
                        for target in allTargets:
                            ...
                            for buffId in target.skillEffects:
                                ...
                                if newPriority < priority:
                                    category == newCategory and localAvatar.guiMgr.createWarning(PLocalizer.BuffPriorityWarning, PiratesGuiGlobals.TextFG6)
                                    return 0
                    ...
                    if localAvatar.hasStickyTargets():
                        ...
                        if target:
                                ...
                                if not WeaponGlobals.isSelfUseSkill(skillId):
                                    ...
                                    if distance < deadzone:
                                        if WeaponGlobals.getAttackClass(skillId) == WeaponGlobals.AC_MISSILE:
                                            if not ItemGlobals.getSubtype(localAvatar.currentWeaponId) == ItemGlobals.BAYONET:
                                                localAvatar.guiMgr.createWarning(PLocalizer.TooCloseWarning, PiratesGuiGlobals.TextFG6)
                                                target = None
                                                ammoSkillId = 0
                                    if target:
                                        if distance < deadzone:
                                            if ItemGlobals.getSubtype(localAvatar.currentWeaponId) == ItemGlobals.BAYONET and WeaponGlobals.getAttackClass(skillId) == WeaponGlobals.AC_MISSILE:
                                            ...
                                            localAvatar.monstrousTarget = (localAvatar.currentTarget and localAvatar.currentTarget.hasNetPythonTag('MonstrousObject') and localAvatar).currentTarget
                                        else:
                                        ...
                                        if skillId in WeaponGlobals.BackstabSkills:
                                            ...
                                            if diff < WeaponGlobals.BACKSTAB_ANGLE:
                                                ...
                                                self.chargeTime = 0
                                        self.comboLevel += 1
                                        combo == -1 and localAvatar.wantComboTiming and self.ignoreInput()
                                    else:
                                        self.acceptInput()
                                else:
                                    self.acceptInput()
                            ...
                            ammoItemId in tonicIds and self.tonicButton.startRecharge()
                            inv = localAvatar.getInventory()
                            if inv is None:
                                return 0
                            itemCat = 0
                        ...
                        ammoSkillId and self.skillTray.decrementSkillTrayAmount(ammoSkillId)
                    else:
                    ...
                    skillId in (InventoryType.UseItem, InventoryType.UsePotion) and messenger.send(WeaponGlobals.LocalAvatarUseItem, [
                     skillId, ammoSkillId, combo, charge])
                else:
                    if WeaponGlobals.isProjectileSkill(skillId, ammoSkillId):
                        posHpr = [
                         0, 0, 0, 0, 0, 0]
                        ...
                        if WeaponGlobals.getIsShipSkill(skillId):
                            not WeaponGlobals.getIsShout(skillId) and messenger.send(WeaponGlobals.LocalAvatarUseShipSkill, [
                             skillId, ammoSkillId])
                        else:
                            if skillId in WeaponGlobals.BackstabSkills:
                ...
                if skillId:
                    if self.weaponMode in (WeaponGlobals.COMBAT, WeaponGlobals.MELEE, WeaponGlobals.VOODOO, WeaponGlobals.THROWING, WeaponGlobals.SAILING):
                        ...
                        rechargeT = skillTrack == WeaponGlobals.RADIAL_SKILL_INDEX and base.cr.battleMgr.getModifiedRechargeTime(localAvatar, skillId)
    ...
    def triggerSkillTraySkill(self, skillId):
        if skillId > 0 and localAvatar.guiMgr.combatTray.skillDisabled(skillId):
            localAvatar.guiMgr.createWarning(PLocalizer.NotWhileFishingWarning, PiratesGuiGlobals.TextFG6)
            return
        ...
        if self.weaponMode == WeaponGlobals.STAFF:
            if self.chargeTime >= self.CHARGE_MODE_TIME_THRESHOLD:
                ...
                return
            if self.skillTray.traySkillMap:
                if WeaponGlobals.getChargeSkill(skillId):
                    if skillId in self.skillTray.traySkillMap:
                        for i in range(len(self.skillTray.traySkillMap)):
                            if self.skillTray.traySkillMap[i] == skillId:
            ...
            if self.weaponMode in (WeaponGlobals.FIREARM, WeaponGlobals.STAFF, WeaponGlobals.CANNON, WeaponGlobals.GRENADE, WeaponGlobals.DEFENSE_CANNON):
                ...
                if self.weaponMode == WeaponGlobals.VOODOO and not WeaponGlobals.getIsDollAttackSkill(skillId):
                    if skillId != EnemySkills.MISC_NOT_IN_FACE:
                        self.trySkill(skillId, 0, combo)
                    else:
                        haveFriendly = localAvatar.hasStickyTargets() and localAvatar.getFriendlyStickyTargets()
            ...
            if toggleButton:
                ...
                if self.weaponMode == WeaponGlobals.FIREARM or self.weaponMode == WeaponGlobals.GRENADE or self.weaponMode == WeaponGlobals.CANNON:
        ...
        if self.chargeTime > 0:
            if self.weaponMode == WeaponGlobals.FIREARM or self.weaponMode == WeaponGlobals.GRENADE:
                ...
                if WeaponGlobals.C_STUN in skillEffects or WeaponGlobals.C_KNOCKDOWN in skillEffects:
                    ...
                    if localAvatar.curAttackAnim and not localAvatar.curAttackAnim.isPlaying():
                        if self.weaponMode == WeaponGlobals.GRENADE:
                        ...
                        if not self.thresholdHit:
                            ...
                            if self.ammoSkillId:
                                if not WeaponGlobals.isInfiniteAmmo(self.ammoSkillId):
                                    if not WeaponGlobals.canUseInfiniteAmmo(localAvatar.currentWeaponId, self.ammoSkillId):
                                        ammoInvId = not WeaponGlobals.canUseInfiniteAmmo(localAvatar.getCurrentCharm(), self.ammoSkillId) and WeaponGlobals.getSkillAmmoInventoryId(self.ammoSkillId)
                                        inv = localAvatar.getInventory()
                                        amt = inv.getStackQuantity(ammoInvId)
                                        amt < 1 and self.endButtonCharge()
                                        return Task.done
                    if self.weaponMode != WeaponGlobals.VOODOO:
                        self.chargeMeter.show()
                if self.chargeTime > 0:
                    ...
                    if self.chargeTime >= self.maxCharge:
                        self.chargeMeterBar['barColor'] = self.weaponMode == WeaponGlobals.STAFF and Vec4(0.7, 0.7, 1.0, 1.0)
                    else:
                        self.chargeMeterBar['barColor'] = Vec4(1.0, 0.6, 0.3, 1.0)
            else:
                self.chargeMeterBar['value'] = 0
            self.chargeMeterBar['range'] = self.maxCharge
            self.chargeTime = (self.InstantCast and self).maxCharge
        else:
    ...
    def processInput(self, button):
        if localAvatar.getGameState() == 'Battle':
            if not localAvatar.checkForWeaponInSlot(localAvatar.currentWeaponId, localAvatar.currentWeaponSlotId):
                        ...
                        if localAvatar.currentTarget:
                            ...
                            if not base.cr.battleMgr.obeysPirateCode(localAvatar, localAvatar.currentTarget):
                                if localAvatar.guiMgr.codeShown:
                                    self.tryShoot += 1
                                    self.tryShoot = self.tryShoot > 5 and 0
                ...
                if button == 'action':
                        ...
                        if self.weaponMode == WeaponGlobals.FIREARM:
                            ...
                            if inventory.getStackQuantity(InventoryType.PistolTakeAim) > 1:
                                if weaponType == ItemGlobals.GUN:
                                    weaponSubType == ItemGlobals.BLUNDERBUSS and self.beginButtonCharge(blunderbuss=1)
                                else:
                                    self.beginButtonCharge()
                            else:
                                if localAvatar.currentTarget:
                            ...
                            if self.weaponMode == WeaponGlobals.GRENADE:
                                ...
                                if self.weaponMode == WeaponGlobals.MELEE:
                    ...
                    if self.weaponMode == WeaponGlobals.COMBAT or self.weaponMode == WeaponGlobals.VOODOO or self.weaponMode == WeaponGlobals.THROWING:
                        ...
                        if self.combatChainLvl > len(skillTrack) - 1:
                            localAvatar.wantComboTiming and self.ignoreInput()
                            return
                        else:
                            if self.combatChainLvl == 0 and not combo == -1:
                                self.resetComboLevel()
                        if self.isUsingSkill:
                            if localAvatar.currentTarget:
                                if self.combatChainLvl <= 1:
                                    if combo <= 0:
                                        if localAvatar.wantComboTiming:
                                            self.resetComboLevel()
                                            messenger.send('tooFast')
                                            ...
                                            combo = 0
                                            return
                            self.onLastAttack = self.combatChainLvl == len(skillTrack) - 1 and 1
                        else:
                            self.onLastAttack = 0
    ...
    def triggerSkillQueue(self):
        if self.actionPressed:
            self.isUsingSkill = self.weaponMode == WeaponGlobals.FIREARM and (not self.skillQueue or self.skillQueue[0] != EnemySkills.PISTOL_RELOAD) and 0
            self.endButtonCharge()
            messenger.send('action')
        else:
            if self.skillQueue:
                self.isUsingSkill = not self.weaponQueue and 0
    ...
    def resetComboLevel(self, args=None, skillId=None):
        ...
        if self.weaponMode == WeaponGlobals.FIREARM or self.weaponMode == WeaponGlobals.GRENADE:
            ...
            if weaponVolley > 0:
                if self.volley >= weaponVolley:
                    if WeaponGlobals.C_QUICKLOAD in localAvatar.getSkillEffects() or self.usedBayonetSkill or skillId and (skillId in WeaponGlobals.SpecialSkills or WeaponGlobals.getSkillReputationCategoryId(skillId) not in (InventoryType.PistolRep, InventoryType.GrenadeRep)):
                        self.usedBayonetSkill = False
                        self.volley = 0
                        skillId == EnemySkills.PISTOL_RELOAD and self.acceptInput()
                else:
                    self.reloadWeapon()
            else:
                self.acceptInput()
    ...
    def reloadWeapon(self):
        if self.ammoSkillId:
            ...
            if not maxQuant == WeaponGlobals.INF_QUANT and not WeaponGlobals.canUseInfiniteAmmo(localAvatar.currentWeaponId, self.ammoSkillId):
                if not WeaponGlobals.canUseInfiniteAmmo(localAvatar.getCurrentCharm(), self.ammoSkillId):
                    ...
                    if not inv:
                        ...
                        if ammoAmt <= 0:
                            self.outOfAmmo()
                            return
            self.weaponMode == WeaponGlobals.FIREARM and self.trySkill(EnemySkills.PISTOL_RELOAD, 0)
        else:
            if self.weaponMode == WeaponGlobals.GRENADE:

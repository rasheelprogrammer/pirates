...
class GuildPage(SocialPage.SocialPage):
    ...
    def determineButtonState(self):
        ...
        if self.guildId:
            ...
            if hasattr(base, 'localAvatar'):
                ...
                if inv:
                    if not inv.getStackQuantity(InventoryType.NewGuild):
            ...
            if self.nameLabel:
                if self.guildName:
                    if self.guildName != PLocalizer.GuildNoGuild:
                        ...
                        ranktxt = (rank == 1 and PLocalizer).GuildRankMember
                    else:
                        if rank == 2:
                            ...
                            if rank == 3:
                                ...
                                if rank == 4:
                                    ...
                                    ranktxt = None
                    self.rankLabel['text'] = self.rankLabel and rank and ranktxt and ranktxt
                    self.rankLabel.show()
                else:
                    if self.rankLabel:
                ...
                self.leaveButton['state'] = (self.guildRank > 0 and DGG).NORMAL
            ...
            if Freebooter.FreeGuildRestrict:
                if not Freebooter.getPaidStatus(base.localAvatar.getDoId()):
                    if self.createButton:
                        self.createButton['state'] = DGG.DISABLED
                    self.renameButton['state'] = (self.renameButton and DGG).DISABLED
        return
    ...
    def leaveGuild(self):
        if localAvatar.getGuildRank() == GUILDRANK_GM:
            self.confirmBox = len(base.cr.guildManager.id2Name.keys()) > 1 and PiratesInfo.PiratesInfo(PLocalizer.GuildPageLeaveGuild, [PLocalizer.GuildAskLeaveGM])
        else:

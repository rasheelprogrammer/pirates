...
class GameOptionsGui(DirectFrame):
    ...
    def setupDisplayFrame(self):
        ...
        if self.gameOptions:
            ...
            if base.appRunner is not None:
                if base.appRunner.windowProperties is not None:
                    y += oy
                text = PLocalizer.GameOptionsWindowedResolutions
                self.windowed_resolutions = []
                resolution_table = (hasattr(base, 'windowed_resolution_table') and base).windowed_resolution_table
            else:
            ...
            resolution_table = (hasattr(base, 'fullscreen_resolution_table') and base).fullscreen_resolution_table
        else:
            resolution_table = self.resolution_table
    ...
    def setupGraphicsFrame(self):
        ...
        self.textureDetailVar = [
         0]
        if self.gameOptions is not None:
            if self.gameOptions.options.texture_scale_mode == False:
                sx = 0.13
            ...
            y += oy
            text = self.gameOptions is not None and self.gameOptions.shader_support and PLocalizer.GameOptionsShaderLevel + ' ' + self.gameOptions.shader_model.__repr__() + ' *'
            self.create_label(x, y, text, parent, sl)
    ...
    def initResolutionSettings(self):
        ...
        if base.inAdFrame:
            ...
            for button_index in range(total_modes):
                ...
                if current_mode['want_size_x'] == m['want_size_x']:
                    current_mode['want_size_y'] == m['want_size_y'] and self.windowedResolutionMenu.set(button_index, False)
    ...
    def windowRadioButtonCB(self, var):
        ...
        if self.windowVar[0] == PLocalizer.GameOptionsWebEmbeddedMode:
            ...
            if self.windowVar[0] == PLocalizer.GameOptionsWindowedMode:
                ...
                if self.windowVar[0] == PLocalizer.GameOptionsFullscreenMode:

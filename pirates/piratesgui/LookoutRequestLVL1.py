...
class LookoutRequestLVL1(InventoryPage, InventoryRequestGameType):
    ...
    def updateMode(self, mode):
        ...
        if self.currMode == PiratesGuiGlobals.SEARCH_MODE or self.currMode == PiratesGuiGlobals.INVITE_MODE or self.currMode == PiratesGuiGlobals.CHALLENGE_MODE or self.currMode == PiratesGuiGlobals.INVITE_ACCEPTED_MODE:
            ...
            if self.currMode == PiratesGuiGlobals.REQUEST_CAT_MODE:
                self.hideRequestCat()
            else:
        ...
        if mode == PiratesGuiGlobals.SEARCH_MODE or mode == PiratesGuiGlobals.INVITE_MODE or mode == PiratesGuiGlobals.INVITE_ACCEPTED_MODE or mode == PiratesGuiGlobals.CHALLENGE_MODE:
            ...
            if mode == PiratesGuiGlobals.REQUEST_CAT_MODE:
                self.showRequestCat()
            else:
    ...
    def updateActiveLookout(self, task=None):
        ...
        if self.iconAnimType == self.ICON_2D_ROTATE:
            ...
            if lookoutImage.sourceImage is imageTray.lookoutButtonNormal:
    ...
    def submitRequest(self):
        ...
        if gameCat == PiratesGlobals.GAME_TYPE_TM:
            if gameType == PiratesGlobals.GAME_STYLE_TM_BLACK_PEARL and quickStart:
            ...
            inviting and options.append([str(GameTypeGlobals.GAME_OPTION_VIP_PASS), str(localAvatar.doId)])
        else:
            if self.invited:
        ...
        def teleportConfirmation(confirmed):
            if confirmed:
                ...
                if self.searchParams:
                    if self.searchParams.has_key('catId'):
                        if self.searchParams['catId'] == PiratesGlobals.GAME_TYPE_PVP:
                            ...
                            base.localAvatar.guiMgr.crewHUD.crew and base.localAvatar.guiMgr.crewHUD.leaveCrew()
                if self.confirmMsg:
                    self.confirmMsg.messageDone()
                    self.confirmMsg = None
            return
    ...
    def createNewItem(self, item, parent, itemType=None, columnWidths=[], color=None):
        ...
        if item['Value'] == PiratesGlobals.GAME_TYPE_CREW:
            ...
            if item['Value'] == PiratesGlobals.GAME_TYPE_PRIV:
                ...
                if item['Value'] == PiratesGlobals.GAME_TYPE_CREW or item['Value'] == PiratesGlobals.GAME_TYPE_PRIV or item['Value'] == PiratesGlobals.GAME_TYPE_PVP:
    ...
    def nextClick(self, gameType=None):
        ...
        def buildLvl2(itemList):
            if itemList:
                gameCatStr = len(itemList) > 0 and GameTypeGlobals.getGameTypeString(gameCat, 'type')
    ...
    def createButtonAndText(self, imageInfo=None, textInfo=None):
        if imageInfo == None:
            ...
            for currImagePostfix in imagePostfixes:
                ...
                if imageFound:
                    not imageFound.isEmpty() and images.append(imageFound)
                else:
                    images.append(None)
        ...
        if self.currMode == PiratesGuiGlobals.SEARCH_MODE or self.currMode == PiratesGuiGlobals.INVITE_MODE or self.currMode == PiratesGuiGlobals.CHALLENGE_MODE or self.currMode == PiratesGuiGlobals.INVITE_ACCEPTED_MODE:
            ...
            if inviting == False:
                if self.guiMgr.isSeaChestAllowed():
                    if self.searchParams:
                        description = (self.searchParams.has_key('cat') and self.searchParams['cat'] == PLocalizer.TMGame and self.guiMgr.crewHUD.crew and PLocalizer).LookoutFoundCrewMsg
                    else:
    ...
    def matchFailed(self, restartRequest):
        if self.currMode == PiratesGuiGlobals.REQUEST_FOUND_MODE or self.currMode == PiratesGuiGlobals.REQUEST_JOIN_MODE or self.currMode == PiratesGuiGlobals.SEARCH_MODE or self.currMode == PiratesGuiGlobals.REQUEST_TYPE_DIRECT_MODE:
            if restartRequest:
                self.searchParams and self.updateMode(PiratesGuiGlobals.SEARCH_MODE)
    ...
    def matchChance(self, matchChance):
        if self.foundChance:
            ...
            if matchChance <= GameTypeGlobals.MATCH_CHANCE_LOW:
                ...
                if matchChance <= GameTypeGlobals.MATCH_CHANCE_MODERATE:
    ...
    def requestInvite(self, inviterName, activityCategory, activityType, options):
        ...
        if inviterName == '':
            if activityCategory == PiratesGlobals.GAME_TYPE_TM:
                description = self.guiMgr.crewHUD.crew and PLocalizer.LookoutInviteCrewMsg % gameCatDesc
            else:
                description = PLocalizer.LookoutInviteMsg % gameCatDesc
        else:
            if activityCategory == PiratesGlobals.GAME_TYPE_TM:
                description = self.guiMgr.crewHUD.crew and PLocalizer.LookoutInviteFromCrewMsg % (inviterName, gameCatDesc)
            else:

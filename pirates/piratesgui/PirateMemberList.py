    ...
    def __init__(self, owner, avId, playerId, mode, modeInfo=None, name=None):
        ...
        if self.mode == MODE_CREW_HUD:
            ...
            if self.mode == MODE_CREW_HUD_SEA:
                ...
                self.statusLabel = DirectLabel(parent=self, relief=None, pos=(0.0,
                                                                              0,
                                                                              0), text='', text_align=TextNode.ALeft, text_scale=PiratesGuiGlobals.TextScaleSmall, text_pos=(0.03,
                                                                                                                                                                             0.02), text_fg=self.OnlineSubtextColor, text_shadow=PiratesGuiGlobals.TextShadow, textMayChange=1)
                statusOffset = 0.35
                levelOffset = 0.46
    ...
    def handlePress(self):
        if self.mode == MODE_FRIEND_PLAYER:
            ...
            if self.mode in (MODE_FRIEND_AVATAR, MODE_GUILD, MODE_CREW):
                ...
                if self.mode in (MODE_CREW_HUD, MODE_CREW_HUD_SEA):
...
class PirateMemberList(DirectObject):
    ...
    def getMemberByAvId(self, avId):
        for member in self.members:
            if avId:
                return member.avId == avId and member
    ...
    def getMemberByPlayerId(self, playerId):
        for member in self.members:
            if playerId:
                return member.playerId == playerId and member
    ...
    def updateOrAddMember(self, avId, playerId, mode, modeInfo=None):
        ...
        if tryAv:
            if tryAv.mode == mode:
                ...
                tryAv.mode == MODE_FRIEND_PLAYER_AVATAR and self.addMember(avId, playerId, mode, modeInfo)
            elif mode == MODE_FRIEND_PLAYER_AVATAR:
                return tryAv.mode == MODE_FRIEND_AVATAR and None
        else:
            if tryPlayer:
                tryPlayer.mode == mode and self.updateMemberInfo(avId, playerId, mode, modeInfo)
            else:
    ...
    def updateMemberInfo(self, avId, playerId, mode, modeInfo=None):
        ...
        member = None
        if tryAv:
            member = tryAv.mode == mode and tryAv
        else:
            if tryPlayer and tryPlayer.mode == mode:
                member = tryPlayer
        if member:
    ...
    def removeMember(self, avId, playerId, mode):
        ...
        if not playerId:
            if avId:
                ...
                playerId = (avMemeber and avMemeber).playerId
        else:
            if playerId and not avId:
    ...
    def refillMember(self, avId, playerId, mode):
        if mode == MODE_FRIEND_AVATAR:
            if playerId:
                info = base.cr.playerFriendsManager.isFriend(playerId) and base.cr.playerFriendsManager.getFriendInfo(playerId)
    ...
    def compareMembers(self, first, other):
        if other is None:
            return -1
        if first.online:
            return not other.online and -1
        else:
            if (not first.online and other).online:
                return 1
        ...
        if first.mode == MODE_GUILD:
            if not hasattr(other, 'modeInfo') or not other.modeInfo:
                ...
                if first.modeInfo:
                    if first.modeInfo[2] in (2, 3):
                        return other.modeInfo[2] == 4 and -1
                    elif first.modeInfo[2] == 4:
                        if other.modeInfo[2] in (2, 3):
                            return 1
                        elif first.modeInfo[2] > other.modeInfo[2]:
                            return -1
                        elif first.modeInfo[2] < other.modeInfo[2]:
                            return 1
                return (first.mode == MODE_FRIEND_PLAYER or first.mode == MODE_FRIEND_PLAYER_AVATAR) and first.mode == MODE_FRIEND_AVATAR and -1
        else:
            if first.mode == MODE_FRIEND_AVATAR:
                return (first.mode == MODE_FRIEND_PLAYER or first.mode == MODE_FRIEND_PLAYER_AVATAR) and 1
            else:
                if self.sortMode == 'Online':
                    if first.lastOnline == None:
                        return 1
                    else:
                        if other.lastOnline == None:
                            return -1
                        else:
                            if first.lastOnline > other.lastOnline:
                                return -1
                    return 1
                else:

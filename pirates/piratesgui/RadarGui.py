...
class RadarGui(GuiTray, FSM):
    ...
    def showLocation(self, locationUID):
        ...
        if locText is not None:
            ...
            if base.localAvatar.ship:
                if base.localAvatar.getTutorialState() > 2:
                    ...
                    oceanText = (oceanText is None and PLocalizer).LoadingScreen_Ocean
    ...
    def moveRadarObjToCache(self, objKey, skipCache=False):
        ...
        if objInfo['type'] != RADAR_OBJ_TYPE_DEFAULT:
            objInfo['radarObjNode'] = objInfo['type'] != RADAR_OBJ_TYPE_EXIT and skipCache == False and None
    ...
    def makeRadarNode(self, obj, objType, objDoId=None, teamId=None):
        ...
        if objType != RADAR_OBJ_TYPE_QUEST:
            modelDict = obj.getCollideMask().hasBitsInCommon(PiratesGlobals.RadarShipBitmask) and self.modelDict[RADAR_OBJ_TYPE_SHIP]
        else:
    ...
    def draw(self, task):
        for objId in self.__radarObjects:
            ...
            if not self.checkValidSrcObjNode(srcObjNode, objId):
                return Task.cont
            if radarNode:
                if not srcObjNode.getParent().isEmpty():
                    self.reparentRadarNode(radarNode)
                    dx, dz, relH, inRange = self.getXZHRelativeToAV(srcObjNode)
                    objType == RADAR_OBJ_TYPE_EXIT and self.setExitNode(radarNode, srcObjNode, outOfRangeNode, dx, dz, relH, inRange)
                else:
    ...
    def updateDial--- This code section failed: ---

 993       0  LOAD_FAST             1  'map'
           3  LOAD_ATTR             1  'getRadarAxis'
           6  CALL_FUNCTION_0       0  None
           9  LOAD_GLOBAL           2  'Options'
          12  LOAD_ATTR             3  'RadarAxisMap'
          15  COMPARE_OP            2  '=='
          18  JUMP_IF_FALSE        52  'to 73'
          21  POP_TOP          

 994      22  LOAD_FAST             0  'self'
          25  LOAD_ATTR             5  'dial'
          28  LOAD_ATTR             6  'setR'
          31  LOAD_CONST            1  0
 ...
 995      38  LOAD_FAST             0  'self'
          41  LOAD_ATTR             7  'north'
          44  LOAD_ATTR             6  'setR'
          47  LOAD_CONST            1  0
          50  CALL_FUNCTION_1       1  None
          53  POP_TOP          

 996      54  LOAD_FAST             0  'self'
          57  LOAD_ATTR             8  'objTop'
          60  LOAD_ATTR             6  'setR'
          63  LOAD_CONST            1  0
          66  CALL_FUNCTION_1       1  None
          69  POP_TOP          
          70  JUMP_FORWARD        117  'to 190'
        73_0  COME_FROM            18  '18'
          73  POP_TOP          

 997      74  LOAD_FAST             1  'map'
          77  LOAD_ATTR             1  'getRadarAxis'
          80  CALL_FUNCTION_0       0  None
          83  LOAD_GLOBAL           2  'Options'
          86  LOAD_ATTR             9  'RadarAxisCamera'
          89  COMPARE_OP            2  '=='
          92  JUMP_IF_FALSE        94  'to 189'
        95_0  THEN                     190
          95  POP_TOP          

 999      96  LOAD_GLOBAL          10  'camera'
          99  LOAD_ATTR            11  'getH'
         102  LOAD_GLOBAL          12  'render'
         105  CALL_FUNCTION_1       1  None
         108  STORE_FAST            2  'camH'

1000     111  LOAD_FAST             0  'self'
         114  LOAD_ATTR             5  'dial'
         117  LOAD_ATTR             6  'setR'
         120  LOAD_FAST             2  'camH'
         123  CALL_FUNCTION_1       1  None
         126  POP_TOP          

1001     127  LOAD_FAST             0  'self'
         130  LOAD_ATTR             7  'north'
         133  LOAD_ATTR             6  'setR'
         136  LOAD_FAST             2  'camH'
         139  UNARY_NEGATIVE   
         140  CALL_FUNCTION_1       1  None
         143  POP_TOP          

1002     144  LOAD_FAST             0  'self'
         147  LOAD_ATTR             8  'objTop'
         150  LOAD_ATTR             6  'setR'
         153  LOAD_FAST             2  'camH'
         156  CALL_FUNCTION_1       1  None
         159  POP_TOP          

1003     160  LOAD_FAST             2  'camH'
         163  LOAD_GLOBAL          14  'localAvatar'
         166  LOAD_ATTR            11  'getH'
         169  LOAD_GLOBAL          12  'render'
         172  CALL_FUNCTION_1       1  None
         175  BINARY_SUBTRACT  
         176  LOAD_FAST             0  'self'
         179  STORE_ATTR           15  'camToAvAngle'
         182  JUMP_ABSOLUTE       190  'to 190'
         185  POP_TOP          

1011     186  JUMP_FORWARD          1  'to 190'
       189_0  COME_FROM            92  '92'
         189  POP_TOP          
       190_0  COME_FROM           186  '186'
       190_1  COME_FROM            70  '70'

Parse error at or near `JUMP_ABSOLUTE' instruction at offset 182
    ...
    def flashRadarObject(self, objId, duration=None, scaleMin=Point3(0.5, 0.5, 0.5), scaleMax=Point3(1.0, 1.0, 1.0)):
        if duration != None:
            if self.flashCleanupTasks.has_key(objId):
            ...
            displayIval.loop()
            duration == None and self.addEffectIval([displayIval, objId])
        else:
    ...
    def cleanupEffects(self):
        ...
        for currIval, currObjId in self.effectIvals:
            ...
            if uItem:
                not uItem.isEmpty() and uItem.setScale(1.0, 1.0, 1.0)
    ...
    def restoreOldInfo(self, objId):
        ...
        if objectInfo:
            if not offRadar:
                objectInfo['type'] = objectInfo.has_key('oldInfo') and objectInfo['oldInfo']['type']
...
class MinimapDemoNpc(MinimapDemoObject):
    ...
    def _addedToMap(self, map):
        if self.type == RadarGui.DEMO_FRIEND:
            ...
            if self.type == RadarGui.DEMO_ENEMY:

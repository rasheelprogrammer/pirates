...
# Embedded file name: pirates.audio.MusicManager
        ...
        if self.playlist:
            if self.current:
                ...
                if self.current.looping == False:
                    ...
                    songLength = SoundGlobals.getMusicLength(self.current.name)
                    songLength is not None and taskMgr.doMethodLater(songLength - offset, self.handleCurrentTrackFinished, 'currentTrackFinished', extraArgs=[self.current.name])
        return
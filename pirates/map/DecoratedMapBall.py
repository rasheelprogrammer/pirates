...
class DecoratedMapBall(MapBall):
        ...
        if base.mouseWatcherNode.hasMouse():
            ...
            if item:
                ...
                if task.activeDart and task.activeDart != dart:
                    ...
                    task.activeDart = None
                p = not task.activeDart and dart and Point2()
    ...
    def setReturnIsland(self, islandUid):
        for item in self.placedItems.itervalues():
            if item.hasNetTag('islandUid'):
                if islandUid:
                    islandUid == item.getNetTag('islandUid') and item.setAsReturnIsland(1)
                else:
    ...
    def setPortOfCall(self, islandUid):
        for item in self.placedItems.itervalues():
            if item.hasNetTag('islandUid'):
                if islandUid:
                    islandUid == item.getNetTag('islandUid') and item.setAsPortOfCall(1)
                else:
    ...
    def setCurrentIsland(self, islandUid):
        for item in self.placedItems.itervalues():
            if item.hasNetTag('islandUid'):
                if islandUid:
                    islandUid == item.getNetTag('islandUid') and item.setAsCurrentIsland(1)
                else:
    ...
    def getCurrentIsland(self):
        for item in self.placedItems.itervalues():
            if item.hasNetTag('islandUid'):
                return item.isCurrentIsland() and item
    ...
    def getIsland(self, islandUid):
        ...
        for item in self.placedItems.itervalues():
            if item.hasNetTag('islandUid'):
                if islandUid:
                    island = islandUid == item.getNetTag('islandUid') and item
                    break

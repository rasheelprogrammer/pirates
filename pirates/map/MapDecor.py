...
class Model(Item):
    ...
    def _initModel(self, modelName, scale, modelPath):
        ...
        if geom:
            ...
            if __dev__:
                geom = base.config.GetBool('map-islands-debug', 1) and loader.loadModel('models/misc/smiley')
                self.geom = geom.instanceTo(self)
                scale = 0.05
    ...
    def updateState(self, mouseOver=False):
        ...
        if self.getCanTeleportTo():
            ...
            if not self.isCurrentIsland():
                self.isReturnIsland() and self.setHelpLabel(PLocalizer.MapCanTeleportReturn)
            else:
                if not self.isCurrentIsland():
                    self.isPortOfCall() and self.setHelpLabel(PLocalizer.MapCanTeleportPortOfCall)
                else:
...
class OceanAreaText(Text):
    ...
    def updateZoom(self, zoom):
        ...
        if zoom >= 0.0:
            self.hidden and self.fadeIn()
        else:
            if zoom < 0.0 and not self.hidden:
                self.fadeOut()

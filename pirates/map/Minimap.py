...
# Embedded file name: pirates.map.Minimap
...
class AreaMap(Map):
    ...
    def __init__(self, area):
        ...
        if mapNode:
            if not mapNode.isEmpty():
                ...
                h, w = diff[1], diff[0]
            ratio = h / w
            normalScale = ratio < 0.99 and 2 / w
            screenScale = 1
        else:
            normalScale = 2 / h
            screenScale = 0.75
    ...
    def handleHolidayEnded(self, area, holiday, override=False):
        if holiday in InvasionGlobals.INVASION_IDS:
            not override and taskMgr.doMethodLater(10, self.handleInvasionEnded, 'handleInvasionEnded', extraArgs=[area, holiday])
        else:

...
class DistributedTeleportMgr(DistributedObject.DistributedObject):
    ...
    def localTeleportToId(self, locationId, teleportingObj=None, destPos=None, callback=None, objectLocation=None, showLoadingScreen=True):
        ...
        if locationId in base.cr.doId2do:
            if base.cr.doId2do[locationId].dclass.getName() == 'DistributedOceanGrid':
            ...
            destObj and self._localTeleportToIdInterestComplete()
    ...
    def _localTeleportToIdResponse(self, parentId, zoneId):
        if parentId != 0:
            if zoneId != 0:
                self.cr.doId2do.get(parentId) and localAvatar.setInterest(parentId, zoneId, ['localTeleportToId'], 'localTeleportToIdInterestAddComplete')
    ...
    def failTeleport(self, parentId=None, zoneId=None, message=PLocalizer.TeleportToPlayerFailMessage):
        ...
        if fallbackAreaId != '':
            ...
            if areaDoId:
                ...
                if destPos:
                    self.localTeleportingObj and self.localTeleportToId(areaDoId, self.localTeleportingObj, destPos)
                else:
        ...
        def confirmed(canTeleportTo, avId, failedFlag, avName=avName):
            if canTeleportTo:
                if self.cr.getActiveWorld():
                    self.cr.distributedDistrict and localAvatar.getParentObj() and handle.sendTeleportResponse(PiratesGlobals.TAAvailable, self.cr.distributedDistrict.doId, self.cr.getActiveWorld().doId, localAvatar.getParentObj().doId, sendToId=requesterId)
                else:
    ...
    def forceTeleportStart(self, instanceName, tzDoId, thDoId, worldGridDoId, tzParent, tzZone):
        ...
        if self.fromInstanceType == PiratesGlobals.INSTANCE_PVP:
            ...
            if self.fromInstanceType == PiratesGlobals.INSTANCE_TUTORIAL:
    ...
    def createSpawnInterests(self, parents, callback, destGrid, teleportingObj):
        ...
        if parentsLen == 0:
            ...
            if parentObj:
                ...
                if parentsLen > 2:
                    base.cr.doId2do.has_key(parents[2]) and base.cr.relatedObjectMgr.requestObjects([parents[0]], eachCallback=lambda param1=None, param2=teleportingObj: callback(param1, param2))

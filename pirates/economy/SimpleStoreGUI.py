...
# Embedded file name: pirates.economy.SimpleStoreGUI
...
class SimpleStoreGUI(DirectFrame):
    ...
    def setPreviewItem(self, itemId):
        ...
        if self.previewItem:
            if self.previewItem.uid == itemId:
                return
            if self.previewItem:
            ...
            if self.previewItem.itemClass == InventoryType.ItemTypeClothing:
                if self.previewItem.itemType == ClothingGlobals.SHIRT and gender == 'f':
                    ...
                    vestIdx in [3, 4] and self.showCutOffVestAlert()
    ...
    def handleBuyItem(self, value=DGG.DIALOG_YES):
        ...
        if inventory:
            ...
            if myMoney < totalCost:
                ...
                return
            if self.buyPanelItem.cost < 0:
                if myMoney + totalCost > self.getMaxMoney(inventory):
                    base.localAvatar.guiMgr.createWarning(PLocalizer.CannotHoldGoldWarning, PiratesGuiGlobals.TextFG6)
                    return
                self.buyPanelItem.quantity > freeSpace and base.localAvatar.guiMgr.createWarning(PLocalizer.NoInventorySpaceWarning, PiratesGuiGlobals.TextFG6)
                return
    ...
    def setBuyPanelItem(self, itemId):
        if self.buyPanelItem:
            ...
            isinstance(self.buyPanelItem, SimpleClothingItem) and ItemGlobals.canDyeItem(itemId) and self.colorPanel.selectColor(self.buyPanelItem.colorId)
    ...
    def itemRollIn(self, itemId, task=None):
        ...
        if self.clickedItem:
            itemId == self.clickedItem.uid and self.setBuyPanelItem(itemId)
        else:
            if self.clickedItem:
                not self.rollInItem and self.setBuyPanelItem(itemId)
            else:
                self.setBuyPanelItem(None)
                if self.rollInItemTask:
                    taskMgr.remove(self.rollInItemTask)
    ...
    def itemClicked(self, itemId, task=None):
        if self.clickedItem:
            if itemId == self.clickedItem.uid:
                return
            self.clickedItem = self.stock.get(itemId)
            if isinstance(self.clickedItem, SimpleClothingItem):
                InventoryUIStoreContainer.InventoryUIStoreContainer.detailsHeight = ItemGlobals.canDyeItem(itemId) and 0.22
            else:
            ...
            if not self.clickedItem:
                self.itemCardPlaceholder.show()
            self.setPreviewItem(itemId)
            getBase().config.GetBool('want-simple-buy', 0) and self.showBuyGUI()
        else:

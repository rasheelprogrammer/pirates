...
class JewelryStoreGUI(DirectFrame):
    ...
    def updateButton(self, data, buyState):
        for item in self.buttons:
            if item.jewelryUID == data[1]:
                if item.jewelryLocation == data[2]:
                    item.addToCart['text'] = (self.mode == BUYING and item.jewelryUID not in self.currentWardrobe and PLocalizer).TailorAddToCart
                else:
                    if self.mode == SELLING:
    ...
    def setPage(self, pageName, startIndex=0, refreshWardrobe=True):
        ...
        if self.mode == BUYING:
            ...
            for jewelryId in jewelryIds:
                if ItemGlobals.getType(jewelryId) == self.getItemGlobalsType(jewelryType):
                    if ItemGlobals.getClass(jewelryId) == InventoryType.ItemTypeJewelry:
                        ...
                        location = 0
                        (not holiday or holiday in JewelryStoreGUI.holidayIdList) and jewelry.append([jewelryId, False, cost, id, primary, secondary, holiday, location])
        ...
        for jewel in jewelry:
            if self.jewelryAmount - startIndex < self.buttonsPerPage:
                if self.jewelryAmount >= startIndex:
                    uid = jewel[0]
                    type = jewelryType
                    ...
                    if self.mode == BUYING:
                        jewelryButton.itemText['text'] = (not owned and PLocalizer).TailorPreview
                    else:
                        if self.mode == SELLING:
                            jewelryButton.itemText['text'] = (equipped and PLocalizer).TattooShopOwned
                        else:
                            if questDrop is True:
                                jewelryButton.itemText['text'] = (self.mode == SELLING and not equipped and PLocalizer).ShopQuestItem
                            else:
                    ...
                    data = [
                     type, uid, location]
                    jewelryButton.addToCart.buyState = self.mode == BUYING and owned and 0
                    ...
                    if self.mode == BUYING:
                        jewelryButton.addToCart.buyState = self.purchaseInventory.hasPanel(data) and 0
                        jewelryButton.addToCart['state'] = DGG.NORMAL
                        jewelryButton.addToCart['text'] = PLocalizer.TailorRemove
                    else:
                        if self.mode == SELLING:
                            jewelryButton.addToCart.buyState = self.sellInventory.hasPanel(data) and 0
                            jewelryButton.addToCart['state'] = DGG.NORMAL
                            jewelryButton.addToCart['text'] = PLocalizer.TailorRemove
                        else:
    ...
    def getItemGlobalsType(self, type):
        if type in (JewelryGlobals.LBROW, JewelryGlobals.RBROW):
            ...
            if type in (JewelryGlobals.LEAR, JewelryGlobals.REAR):
                ...
                if type == JewelryGlobals.NOSE:
                    ...
                    if type == JewelryGlobals.MOUTH:
                        ...
                        if type in (JewelryGlobals.LHAND, JewelryGlobals.RHAND):

    ...
    def hasPanel(self, data, mode):
        for panel in self.panels:
            if panel.data[0] == data[0]:
                if panel.data[1] == data[1]:
                    return panel.data[2] == data[2] and (mode == 0 or panel.data[3] == data[3]) and True
...
class AccessoriesStoreGUI(DirectFrame):
    ...
    def showColorFrame(self, button=None, type=None, id=None, tex=None, cost=None, uid=None):
        ...
        def selectColor(id, colorFrame):
            ...
            if button:
                ...
                topColors[0] = self.currentPage == ClothingGlobals.SHIRT and id
            else:
        ...
        for idx in range(0, len(colorSet)):
            ...
            if idx != 0:
                ...
                colorButton['state'] = (idx in colorsOwned and DGG).DISABLED
    ...
    def updateButton(self, data, buyState):
        for item in self.buttons:
            if item.clothModelType == data[0]:
                if item.clothModelId == data[1]:
                    if item.clothTextureId == data[2] and item.clothLocation == data[6] and self.mode == SELLING and item.clothColorId != data[3]:
                        continue
                    item.addToCart['text'] = (self.mode == BUYING and PLocalizer).TailorAddToCart
                else:
                    if self.mode == SELLING:
        ...
        if self.mode == mode:
            if not refresh:
                ...
                if self.mode == mode and refresh:
                    ...
                    return
            self.prevIdx = (self.mode == BUYING and self).buttonIndex
            idx = 0
        else:
            idx = self.prevIdx
        self.mode = mode
    ...
    def equipAccessory(self, accessory):
        ...
        if accessory[0].equippedText.isHidden():
            ...
            if accessory[1] == ClothingGlobals.PANT:
                ...
                accessory[0].addToCart['state'] = (accessory[1] == ClothingGlobals.SHIRT and DGG).DISABLED
        else:
    ...
    def setPage(self, pageName, startIndex=0, refreshWardrobe=True):
        ...
        if self.mode == BUYING:
            ...
            for clothingId in clothingIds:
                if ItemGlobals.getType(clothingId) == clothingTypeId:
                    if ItemGlobals.getClass(clothingId) == InventoryType.ItemTypeClothing:
                        if gender == 'm':
                        ...
                        id != -1 and (not holiday or holiday in AccessoriesStoreGUI.holidayIdList) and clothes.append([clothingId, color, 0, cost, False, id, tex, holiday, 0])
        ...
        for cloth in clothes:
            ...
            if numButtons < self.buttonsPerPage:
                if self.clothingAmount >= startIndex:
                    uid = cloth[0]
                    ...
                    if self.mode == BUYING:
                        ...
                        if self.mode == BUYING:
                            if not owned:
                                ...
                                clothButton.addToCart['state'] = (original and DGG).DISABLED
                                clothButton.addToCart.hide()
                        ...
                        clothButton.addToCart.buyState = self.mode == BUYING and owned and 0
                        clothButton.addToCart['state'] = DGG.DISABLED
                        clothButton['state'] = DGG.DISABLED
                        clothButton.addToCart['text'] = PLocalizer.TailorPurchased
                    else:
                        if self.mode == BUYING:
                            if self.purchaseInventory.hasPanel(data, self.mode):
                                clothButton.addToCart.buyState = 0
                                clothButton.addToCart['state'] = DGG.NORMAL
                                clothButton.addToCart['text'] = PLocalizer.TailorRemove
                                (not self.pvpMode or not (landInfamyRequired or seaInfamyRequired)) and clothButton.addToCart.show()
                        else:
                            if self.mode == SELLING:
                                clothButton.addToCart.buyState = self.sellInventory.hasPanel(data, self.mode) and 0
                                clothButton.addToCart['state'] = DGG.NORMAL
                                clothButton.addToCart['text'] = PLocalizer.TailorRemove
                            else:
                                ...
                                if self.mode == BUYING:
                                    ...
                                    if self.pvpMode:
                                        (landInfamyRequired or seaInfamyRequired) and clothButton.addToCart.hide()
                                else:
                                    if self.mode == SELLING:
                                        clothButton.addToCart['text'] = PLocalizer.TailorSell
                    clothButton.addToCart['state'] = (equipped and type == ClothingGlobals.PANT and DGG).DISABLED
                else:
                    if equipped and gender == 'f' and type == ClothingGlobals.SHIRT:
                        clothButton.addToCart['state'] = DGG.DISABLED

...
class TattooStoreGUI(DirectFrame):
    ...
    def updateButton(self, data, buyState):
        for item in self.buttons:
            if item.tattooUID == data[1]:
                if item.tattooLocation == data[3]:
                    item.addToCart['text'] = (self.mode == BUYING and item.tattooUID not in self.currentWardrobe and PLocalizer).TailorAddToCart
                else:
                    if self.mode == SELLING:
    ...
    def setPage(self, pageName, startIndex=0, refreshWardrobe=True):
        ...
        if self.mode == BUYING:
            ...
            for tattooId in tattooIds:
                if ItemGlobals.getType(tattooId) == self.getItemGlobalsType(tattooType):
                    if ItemGlobals.getClass(tattooId) == InventoryType.ItemTypeTattoo:
                        id = ItemGlobals.getMaleModelId(tattooId)
                        color = 0
                        cost = ItemGlobals.getGoldCost(tattooId)
                        holiday = ItemGlobals.getHoliday(tattooId)
                        location = 0
                        (not holiday or holiday in TattooStoreGUI.holidayIdList) and tattoos.append([tattooId, False, cost, id, holiday, location])
                ...
                if self.tattooAmount >= startIndex:
                    ...
                    if self.mode == BUYING:
                        if not owned:
                            ...
                            if self.mode == SELLING:
                                tattooButton.itemText['text'] = (equipped and PLocalizer).TattooShopOwned
                                tattooButton.itemText['image'] = None
                            else:
                        ...
                        if questDrop is True:
                            ...
                            if self.mode == BUYING and not owned and (self.pvpMode and landInfamyRequired or seaInfamyRequired):
                                ...
                                infamyText = landInfamyRequired and PLocalizer.LandInfamyRequirement % landInfamyLevel
                            else:
                    ...
                    data = [
                     type, uid, tattooId, location]
                    tattooButton.addToCart.buyState = self.mode == BUYING and owned and 0
                    ...
                    if self.mode == BUYING:
                        if self.purchaseInventory.hasPanel(data):
                            ...
                            (not self.pvpMode or not (landInfamyRequired or seaInfamyRequired)) and tattooButton.addToCart.show()
                    else:
                        if self.mode == SELLING:
                            tattooButton.addToCart.buyState = self.sellInventory.hasPanel(data) and 0
                            tattooButton.addToCart['state'] = DGG.NORMAL
                            tattooButton.addToCart['text'] = PLocalizer.TailorRemove
                            tattooButton.addToCart.show()
                        else:
                            ...
                            if self.mode == BUYING:
                                ...
                                if self.pvpMode:
                                    (landInfamyRequired or seaInfamyRequired) and tattooButton.addToCart.hide()
                            else:
                                if self.mode == SELLING:
    ...
    def getItemGlobalsType(self, type):
        if type == TattooGlobals.ZONE1:
            ...
            if type in (TattooGlobals.ZONE2, TattooGlobals.ZONE3):
                return ItemGlobals.ARM
            else:
                if type == TattooGlobals.ZONE4:

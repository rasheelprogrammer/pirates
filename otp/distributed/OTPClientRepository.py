...
class OTPClientRepository(ClientRepositoryBase):
    ...
    def enterNoConnection(self):
        ...
        if self.bootedIndex != None:
            message = OTPLocalizer.CRBootedReasons.has_key(self.bootedIndex) and OTPLocalizer.CRBootedReasons[self.bootedIndex] % {'name': gameUsername}
        else:
    ...
    def detectLeaks(self, okTasks=None, okEvents=None):
        ...
        if leakedTasks or leakedEvents or leakedIvals or leakedGarbage:
            ...
            if errorCode >= OTPLauncherGlobals.NonErrorExitStateStart:
                ...
                allowExit and self.notify.info('Allowing client to leave, panda error code %s' % errorCode)
            else:
                base.userExit()
        else:
        ...
        if not self.SupportTutorial or base.localAvatar.tutorialAck:
            ...
            if base.config.GetBool('force-tutorial', 1):
                if hasattr(self, 'skipTutorialRequest'):
                    self.skipTutorialRequest and self.gameFSM.request('playGame', [hoodId, zoneId, avId])
    ...
    def allowWhiteListChat(self):
        if hasattr(self, 'whiteListChatEnabled'):
            return self.whiteListChatEnabled and True
        else:
            return False
        ...
        if msgType == CLIENT_GO_GET_LOST:
            ...
            if msgType == CLIENT_HEARTBEAT:
                ...
                if msgType == CLIENT_SYSTEM_MESSAGE:
                    ...
                    if msgType == CLIENT_SYSTEMMESSAGE_AKNOWLEDGE:
                                                    ...
                                                    if msgType == CLIENT_DONE_INTEREST_RESP:
                                                        ...
                                                        if msgType == CLIENT_GET_STATE_RESP:
                                                            ...
                                                            if msgType == CLIENT_OBJECT_LOCATION:
                                                                ...
                                                                if msgType == CLIENT_SET_WISHNAME_RESP:

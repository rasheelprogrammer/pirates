...
class Avatar(Actor, ShadowCaster):
            ...
            if self == base.localAvatar:
                ...
                if self.playerType == NametagGroup.CCSuit:
                    ...
                    if self.playerType not in (NametagGroup.CCNormal, NametagGroup.CCFreeChat, NametagGroup.CCSpeedChat):
                        ...
                        if hasattr(base, 'localAvatar'):
                            self.understandable = self.commonChatFlags & base.localAvatar.commonChatFlags & OTPGlobals.CommonChat and 1
                            self.setPlayerType(NametagGroup.CCFreeChat)
                        else:
                            if self.commonChatFlags & OTPGlobals.SuperChat:
                                ...
                                if hasattr(base, 'localAvatar'):
                                    self.understandable = base.localAvatar.commonChatFlags & OTPGlobals.SuperChat and 1
                                    self.setPlayerType(NametagGroup.CCFreeChat)
                                else:
                                    if base.cr.getFriendFlags(self.doId) & OTPGlobals.FriendChat:
                                        ...
                                        if base.cr.playerFriendsManager.findPlayerIdFromAvId(self.doId) is not None:
                                            ...
                                            if hasattr(base, 'localAvatar'):
                                                self.understandable = self.whitelistChatFlags & base.localAvatar.whitelistChatFlags and 1
                                            else:
                                                self.understandable = 0
    ...
    def playCurrentDialogue(self, dialogue, chatFlags, interrupt=1):
        if interrupt:
            if self.__currentDialogue is not None:
                self.__currentDialogue.stop()
            self.__currentDialogue = dialogue
            dialogue and base.playSfx(dialogue, node=self)
        else:
    ...
    def showNametag3d(self):
        if self.__nameVisible:
            not self.ghostMode and self.nametag.getNametag3d().setContents(Nametag.CName | Nametag.CSpeech | Nametag.CThought)
        else:
            self.nametag.getNametag3d().setContents(0)
    ...
    def advancePageNumber(self):
        if self.__chatAddressee == base.localAvatar.doId:
            if self.__chatPageNumber != None and self.__chatPageNumber[0] == self.__chatParagraph:
                ...
                if pageNumber >= 0:
                    ...
                    if pageNumber >= self.nametag.getNumChatPages():
                        pageNumber = -1
                    self.__chatLocal and self.setPageNumber(self.__chatParagraph, pageNumber)
                else:
    ...
    def __updatePageChat(self):
        if self.__chatPageNumber != None:
            if self.__chatPageNumber[0] == self.__chatParagraph:
                pageNumber = self.__chatPageNumber[1]
                if pageNumber >= 0:
                    ...
                    if pageNumber < self.nametag.getNumChatPages():
                        ...
                        if pageNumber > 0:
                            dialogue = len(self.__chatDialogueList) > pageNumber and self.__chatDialogueList[pageNumber]
                        else:
                            dialogue = None

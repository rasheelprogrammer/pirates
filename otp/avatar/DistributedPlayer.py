...
class DistributedPlayer(DistributedAvatar.DistributedAvatar, PlayerBase.PlayerBase):
    ...
    def b_setChat(self, chatString, chatFlags):
        if self.cr.wantMagicWords:
            len(chatString) > 0 and chatString[0] == '~' and messenger.send('magicWord', [chatString])
        else:
            if base.config.GetBool('want-chatfilter-hacks', 0):
    ...
    def setChat(self, chatString, chatFlags, DISLid):
        ...
        if base.cr.avatarFriendsManager.checkIgnored(self.doId):
            return
        if base.localAvatar.garbleChat:
            if not self.isUnderstandable():
                chatString = self.chatGarbler.garble(self, chatString)
            chatFlags &= ~(CFQuicktalker | CFPageButton | CFQuitButton)
            chatFlags & CFThought and chatFlags &= ~(CFSpeech | CFTimeout)
        else:
    ...
    def teleportQuery(self, requesterId):
        ...
        if avatar != None:
            ...
            if self.__teleportAvailable:
                if not self.ghostMode and base.config.GetBool('can-be-teleported-to', 1):
                ...
                self.failedTeleportMessageOk(requesterId) and self.setSystemMessage(requesterId, OTPLocalizer.WhisperFailedVisit % avatar.getName())
...
#                 binary_expr (3)
#                      0. expr
#                         L. 402     138  LOAD_FAST             2  'chatFlags'
#                      1. expr
#                                    141  LOAD_GLOBAL          20  'CFThought'
#                      2. binary_op
#                                    144  BINARY_AND       
#              1. jmp_false (2)
#                  0.            145  JUMP_IF_FALSE        19  'to 167'
#                  1.            148  POP_TOP          
#              2. expr
#                 L. 403     149  LOAD_FAST             2  'chatFlags'
#              3. come_from_opt
#      1. expr
#         unary_expr (2)
#              0. expr
#                 binary_expr (3)
#                      0. expr
#                                    152  LOAD_GLOBAL          21  'CFSpeech'
#                      1. expr
#                                    155  LOAD_GLOBAL          22  'CFTimeout'
#                      2. binary_op
#                                    158  BINARY_OR        
#              1. unary_op
#                            159  UNARY_INVERT     

...
class TalkAssistant(DirectObject.DirectObject):
    ...
    def receiveWhisperTalk(self, avatarId, avatarName, accountId, accountName, toId, toName, message, scrubbed=0):
        ...
        if not avatarName:
            ...
            avatarId == localAvatar.doId and self.addHandle(toId, newMessage)
        else:
    ...
    def receiveOpenSpeedChat(self, type, messageIndex, senderAvId, name=None):
        ...
        if not name:
            ...
            message = type == SPEEDCHAT_NORMAL and self.SCDecoder.decodeSCStaticTextMsg(messageIndex)
        else:
    ...
    def sendOpenTalk(self, message):
        error = None
        if base.cr.wantMagicWords:
            len(message) > 0 and message[0] == '~' and messenger.send('magicWord', [message])
